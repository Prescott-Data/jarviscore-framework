...
def gcs_list_files(auth_info: dict, bucket_name: str, prefix: Optional[str] = None, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        records, status, message, _ = _gcs_list_objects(
            api, bucket, headers, limit, timeout, verify_ssl, prefix=_gcs_norm_prefix(prefix) or None
        )
        if message != "ok":
