...
def gcs_update_file(auth_info: dict, bucket_name: str, object_name: str, fields: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        headers, auth_err = _gcs_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        url = _gcs_object_url(api, bucket, object_name)
        resp = requests.patch(url, headers=headers, json=fields, timeout=timeout, verify=verify_ssl)
