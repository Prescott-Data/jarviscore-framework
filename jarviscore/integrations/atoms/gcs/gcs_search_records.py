...
def gcs_search_records(auth_info: dict, bucket_name: str, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search objects by name prefix (GET .../o?prefix=query). OAuth Bearer token (cloud-platform or devstorage scope). JSON API root required. Official: https://cloud.google.com/storage/docs/json_api/v1"""
    try:
        if not query:
        ...
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        headers, auth_err = _gcs_auth(auth_info)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        records, status, message, _ = _gcs_list_objects(
            api, bucket, headers, limit, timeout, verify_ssl, prefix=str(query).lstrip("/")
        )
        if message != "ok":
