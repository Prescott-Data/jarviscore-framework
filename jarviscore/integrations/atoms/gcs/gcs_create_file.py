...
def gcs_create_file(auth_info: dict, bucket_name: str, object_name: str, content: str, content_type: str = "application/octet-stream", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        headers, auth_err = _gcs_auth(auth_info, content_type=content_type or "application/octet-stream")
        if auth_err:
        ...
        oid = data.get("name") or data.get("id") if isinstance(data, dict) else None
        return {
            "records": records,
            "data_count": len(records),
            "status": status,
            "message": "ok",
            "provision_ids": [str(oid)] if oid not in (None, "") else [],
        }
