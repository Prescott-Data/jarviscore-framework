...
def coda_update_project(auth_info: dict, project_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        headers, auth_err = _coda_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        resp = _coda_patch(f"{api}/docs/{project_id}", headers, payload, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        data = resp.json() if resp.text else {}
        return {"records": [data] if isinstance(data, dict) else [], "data_count": 1 if isinstance(data, dict) else 0, "status": resp.status_code, "message": "ok", "provision_ids": [project_id]}
