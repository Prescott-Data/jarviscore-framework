...
def coda_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        records, status, msg = _coda_paginate(f"{api}/docs", headers, fetch_limit, timeout, verify_ssl)
        if status >= 400:
