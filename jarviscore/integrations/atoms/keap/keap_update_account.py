...
def keap_update_account(auth_info: dict, record_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update Keap account. Official: https://developer.keap.com/docs/restv2/"""
    try:
        if not record_id: return {"records": [], "data_count": 0, "status": 400, "message": "record_id is required"}
        ...
        resp = _kp_patch(f"{api}/companies/{record_id}", headers, payload, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        records = _kp_single(resp.json() if resp.text else {})
