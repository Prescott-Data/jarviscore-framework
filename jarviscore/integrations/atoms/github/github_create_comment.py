async def github_create_comment(owner: str, repo: str, issue_number: int, body: str) -> dict:
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    comment = await _post(f'/repos/{owner}/{repo}/issues/{issue_number}/comments', data={'body': body}, headers={'Accept': 'application/vnd.github+json'})
    return {'id': comment['id'], 'body': comment['body'], 'url': comment['html_url'], 'user': comment['user']['login'], 'created_at': comment['created_at']}
