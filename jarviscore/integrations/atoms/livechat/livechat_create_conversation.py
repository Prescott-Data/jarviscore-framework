...
def livechat_create_conversation(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if body and "chat" not in body and "active" not in body and "continuous" not in body:
            body = {"chat": body}
        resp, aerr = _lc_post(base, "start_chat", body, auth_info, timeout, verify_ssl)
        if aerr:
