...
def livechat_list_conversations(auth_info: dict, limit: int = 25, page_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        while len(records) < min(max(int(limit or 25), 1), 100) and pages < 20:
            ...
            resp, aerr = _lc_post(base, "list_chats", req, auth_info, timeout, verify_ssl)
            if aerr:
                return {"records": [], "data_count": 0, "status": 401, "message": aerr}
            status = resp.status_code
            data = _lc_json(resp)
            if status >= 400:
                return {"records": records, "data_count": len(records), "status": status, "message": _lc_error(data, resp.text)}
