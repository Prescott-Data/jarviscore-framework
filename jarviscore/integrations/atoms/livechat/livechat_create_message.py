...
def livechat_create_message(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, aerr = _lc_post(base, "send_event", body, auth_info, timeout, verify_ssl)
        if aerr:
        ...
        if not result["provision_ids"] and isinstance(data, dict) and data.get("event_id"):
            result["provision_ids"] = [data["event_id"]]
            result["records"] = [{"id": data["event_id"]}]
            result["data_count"] = 1
        return result
