...
def livechat_list_messages(auth_info: dict, conversation_id: str, limit: int = 25, page_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List thread events (messages) via list_threads. Official: https://platform.text.com/docs/messaging/agent-chat-api#list-threads"""
    try:
        ...
        body = {"chat_id": cid, "limit": min(max(int(limit or 25), 1), 100)}
        if page_id:
            body["page_id"] = page_id
        resp, aerr = _lc_post(base, "list_threads", body, auth_info, timeout, verify_ssl)
        if aerr:
        ...
        records = _lc_cap(_lc_flatten_events(data.get("threads") if isinstance(data, dict) else None), limit)
