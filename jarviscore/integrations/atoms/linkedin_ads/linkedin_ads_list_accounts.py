async def linkedin_ads_list_accounts(count: int=25, start: int=0) -> list:
    """Ads list accounts via the linkedin_ads API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/adAccountsV2', params={'q': 'search', 'count': count, 'start': start})
    return [{'id': a.get('id'), 'name': a.get('name'), 'status': a.get('status'), 'type': a.get('type'), 'currency': a.get('currency'), 'reference': a.get('reference')} for a in resp.get('elements', [])]
