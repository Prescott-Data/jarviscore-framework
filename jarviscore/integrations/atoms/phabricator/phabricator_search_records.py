...
def phabricator_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Fulltext project search via project.search constraints.query. Official: https://secure.phabricator.com/conduit/"""
    try:
        if not query:
            return _ph_dataset([], 400, "query is required")
        auth_info = auth_info or {}
        params = {"constraints": {"query": str(query)}, "queryKey": auth_info.get("query_key") or auth_info.get("queryKey") or "all"}
        records, status, msg = _ph_search("project.search", base_url, auth_info, params, limit, timeout, verify_ssl)
