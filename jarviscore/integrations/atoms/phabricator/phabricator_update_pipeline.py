...
def phabricator_update_pipeline(auth_info: dict, pipeline_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a build plan via harbormaster.buildplan.edit. Official: https://secure.phabricator.com/conduit/"""
    try:
        if not pipeline_id:
            return _ph_provision({}, 400, "pipeline_id is required")
        tx, err = _ph_payload_transactions(payload, _PLAN_TX)
        if err:
            return _ph_provision({}, 400, err)
        params = {"objectIdentifier": str(pipeline_id).strip(), "transactions": tx}
        resp, result, status, msg = _ph_conduit("harbormaster.buildplan.edit", params, base_url, auth_info, timeout, verify_ssl)
        if status >= 400:
            return _ph_provision(result or {}, status, msg)
        return _ph_provision(result or {}, status, "ok", fallback_id=pipeline_id)
...
_PLAN_TX = {
    "name": "name",
    "behavior_hold_drafts": "behavior.hold-drafts",
    "behavior_arc_land": "behavior.arc-land",
    "behavior_buildable": "behavior.buildable",
    "behavior_restartable": "behavior.restartable",
    "behavior_runnable": "behavior.runnable",
}
