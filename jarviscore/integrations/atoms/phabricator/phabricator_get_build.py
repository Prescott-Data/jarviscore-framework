...
def phabricator_get_build(auth_info: dict, build_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a build by id or PHID via harbormaster.build.search. Official: https://secure.phabricator.com/conduit/"""
    try:
        if not build_id:
            return _ph_dataset([], 400, "build_id is required")
        params = {"constraints": _ph_id_constraints(build_id), "limit": 1}
        resp, result, status, msg = _ph_conduit("harbormaster.build.search", params, base_url, auth_info, timeout, verify_ssl)
        if status >= 400:
