...
def phabricator_list_pipelines(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        params = {"queryKey": auth_info.get("query_key") or auth_info.get("queryKey") or "active"}
        constraints = auth_info.get("constraints")
        if isinstance(constraints, dict):
            params["constraints"] = constraints
        records, status, msg = _ph_search("harbormaster.buildplan.search", base_url, auth_info, params, limit, timeout, verify_ssl)
