...
def phabricator_get_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a project by id or PHID via project.search constraints. Official: https://secure.phabricator.com/conduit/"""
    try:
        if not project_id:
            return _ph_dataset([], 400, "project_id is required")
        params = {"constraints": _ph_id_constraints(project_id), "limit": 1}
        resp, result, status, msg = _ph_conduit("project.search", params, base_url, auth_info, timeout, verify_ssl)
        if status >= 400:
