...
def plausible_get_report(auth_info: dict, report_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        extra = {"metrics": auth_info.get("metrics") or ["visitors", "pageviews", "visits"]}
        if report_id:
            dim = str(report_id).strip()
            if ":" in dim:
                extra["dimensions"] = [dim]
            else:
                extra["dimensions"] = auth_info.get("dimensions") or ["visit:source"]
