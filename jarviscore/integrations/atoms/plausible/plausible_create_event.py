...
def plausible_create_event(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        ua = auth_info.get("user_agent") or payload.get("user_agent")
        if ua:
            headers["User-Agent"] = str(ua)
        xff = auth_info.get("x_forwarded_for") or payload.get("x_forwarded_for")
        if xff:
        ...
        resp = requests.post(host + "/api/event", headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
