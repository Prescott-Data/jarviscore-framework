...
def plausible_get_event(auth_info: dict, event_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get stats for one event name via Stats v2 filter on event:name. Official: https://plausible.io/docs/stats-api"""
    try:
        if not event_id:
            return _pl_dataset([], 400, "event_id is required")
        auth_info = auth_info or {}
        extra = {
            "metrics": auth_info.get("metrics") or ["events", "visitors"],
            "dimensions": auth_info.get("dimensions") or ["event:name"],
            "filters": auth_info.get("filters") or [["is", "event:name", [str(event_id)]]],
        }
