...
def plausible_list_events(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List custom events via Stats v2 query (dimensions event:name). Official: https://plausible.io/docs/stats-api"""
    try:
        auth_info = auth_info or {}
        extra = {"metrics": auth_info.get("metrics") or ["events"], "dimensions": auth_info.get("dimensions") or ["event:name"]}
