...
def plausible_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search pages/events via Stats v2 contains filter. Official: https://plausible.io/docs/stats-api"""
    try:
        if not query:
            return _pl_dataset([], 400, "query is required")
        auth_info = auth_info or {}
        dimension = auth_info.get("search_dimension") or "event:page"
        extra = {
            "metrics": auth_info.get("metrics") or ["visitors", "pageviews"],
            "dimensions": auth_info.get("dimensions") or [dimension],
            "filters": auth_info.get("filters") or [["contains", dimension, [str(query)]]],
        }
