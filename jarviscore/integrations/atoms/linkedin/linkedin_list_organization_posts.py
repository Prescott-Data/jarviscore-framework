async def linkedin_list_organization_posts(organization_id: str, count: int=25, start: int=0) -> list:
    """List organization posts via the linkedin API."""
    ...
    return [{'id': post.get('id'), 'created_at': post.get('created', {}).get('time'), 'visibility': post.get('visibility', {}).get('com.linkedin.ugc.MemberNetworkVisibility'), 'text': post.get('specificContent', {}).get('com.linkedin.ugc.ShareContent', {}).get('shareCommentary', {}).get('text')} for post in resp.get('elements', [])]
