def linkedin_get_network_size(auth_info: dict, person_id: str) -> dict:
    import requests
    ...
    def _delete(p, headers=None):
        ...
        return _r.json() if _r.content else {}
    resp = _get(
        f"/networkSizes/urn:li:person:{person_id}",
        params={"edgeType": "FirstDegreeHandshake"}
    )
    return {
        "person_id": person_id,
        "first_degree_connections": resp.get("firstDegreeSize"),
        "edge_type": "FirstDegreeHandshake"
    }
