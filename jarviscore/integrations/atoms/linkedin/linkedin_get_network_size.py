async def linkedin_get_network_size(person_id: str) -> dict:
    """Get network size via the linkedin API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get(f'/networkSizes/urn:li:person:{person_id}', params={'edgeType': 'FirstDegreeHandshake'})
    return {'person_id': person_id, 'first_degree_connections': resp.get('firstDegreeSize'), 'edge_type': 'FirstDegreeHandshake'}
