async def linkedin_create_post(author_urn: str, text: str, visibility: str='PUBLIC') -> dict:
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    payload = {'author': author_urn, 'lifecycleState': 'PUBLISHED', 'specificContent': {'com.linkedin.ugc.ShareContent': {'shareCommentary': {'text': text}, 'shareMediaCategory': 'NONE'}}, 'visibility': {'com.linkedin.ugc.MemberNetworkVisibility': visibility}}
    resp = await _post('/ugcPosts', data=payload)
    return {'id': resp.get('id'), 'author': author_urn, 'text': text, 'visibility': visibility}
