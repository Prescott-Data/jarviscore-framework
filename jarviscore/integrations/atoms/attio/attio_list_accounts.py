...
def attio_list_accounts(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        records, status, message = _attio_query_records(
            api_root, "companies", headers, limit, timeout, verify_ssl
        )
        if message != "ok":
