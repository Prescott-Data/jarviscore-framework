...
# PATCH /v2/objects/people/records/{record_id} — https://docs.attio.com/rest-api/endpoint-reference/records/update-a-record
...
def attio_update_contact(auth_info: dict, contact_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update person record (PATCH /v2/objects/people/records/{record_id}). Official: https://docs.attio.com/rest-api/endpoint-reference/records/update-a-record"""
    try:
        if not base_url:
        ...
        resp = requests.patch(
            f"{api_root}/objects/people/records/{contact_id}",
            headers=headers,
        ...
        rec = _attio_record(body) or {"record_id": contact_id}
        records = [rec] if isinstance(rec, dict) else []
        return _attio_provision(records, status, "ok", _attio_provision_ids(body, contact_id))
