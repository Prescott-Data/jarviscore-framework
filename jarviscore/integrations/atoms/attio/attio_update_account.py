...
# PATCH /v2/objects/companies/records/{record_id} — https://docs.attio.com/rest-api/endpoint-reference/records/update-a-record
...
def attio_update_account(auth_info: dict, account_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not account_id:
            return _attio_provision([], 400, "account_id is required")
        ...
        resp = requests.patch(
            f"{api_root}/objects/companies/records/{account_id}",
            headers=headers,
        ...
        rec = _attio_record(body) or {"record_id": account_id}
        records = [rec] if isinstance(rec, dict) else []
        return _attio_provision(records, status, "ok", _attio_provision_ids(body, account_id))
