...
def attio_search_records(auth_info: dict, query: str, object_type: str = "people", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        try:
            object_slug = _attio_object_slug(object_type)
        except ValueError as e:
            return {"records": [], "data_count": 0, "status": 400, "message": str(e)}
        ...
        records, status, message = _attio_query_records(
            ...
            timeout,
            verify_ssl,
            {"filter": _attio_search_filter(query)},
        )
        if message != "ok":
