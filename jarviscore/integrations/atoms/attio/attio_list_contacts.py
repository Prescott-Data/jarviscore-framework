...
async def attio_list_contacts(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Query people records (POST /v2/objects/people/records/query). Official: https://docs.attio.com/rest-api/endpoint-reference/records/query-records"""
    try:
        if not base_url:
        ...
        records, status, message = await _attio_query_records(api_root, 'people', headers, limit, timeout, verify_ssl)
        if message != 'ok':
