...
def liquidplanner_create_task(auth_info: dict, workspace_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create a task with task wrapper body. Official: https://developer.liquidplanner.com/docs/create-and-update-examples"""
    try:
        if not isinstance(payload, dict) or not payload.get("name") and "task" not in payload:
            inner = payload.get("task") if isinstance(payload, dict) else None
            if not (isinstance(inner, dict) and inner.get("name")):
                return {"records": [], "data_count": 0, "status": 400, "message": "payload.name is required", "provision_ids": []}
        ...
        body = _lp_write_body("task", payload)
        resp = requests.post(f"{base}/workspaces/{ws}/tasks", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
        ...
        return _lp_provision(data, resp.status_code)
