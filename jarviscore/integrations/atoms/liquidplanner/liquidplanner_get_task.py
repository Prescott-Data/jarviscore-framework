...
def liquidplanner_get_task(auth_info: dict, workspace_id: str, task_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a task by ID. Official: https://developer.liquidplanner.com/docs/requests-and-responses"""
    try:
        if not task_id:
        ...
        resp = requests.get(f"{base}/workspaces/{ws}/tasks/{task_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        try:
