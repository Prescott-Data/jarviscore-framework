...
def liquidplanner_list_tasks(auth_info: dict, workspace_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List tasks filtered to item_type Task. Official: https://developer.liquidplanner.com/docs/filtering-requests"""
    try:
        ...
        params = [("filter[]", "item_type is Task")]
        resp = requests.get(f"{base}/workspaces/{ws}/tasks", headers=headers, params=params, timeout=timeout, verify=verify_ssl)
