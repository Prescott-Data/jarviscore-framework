...
def liquidplanner_create_project(auth_info: dict, workspace_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create a project with project wrapper body. Official: https://developer.liquidplanner.com/docs/create-and-update-examples"""
    try:
        if not isinstance(payload, dict) or not payload.get("name") and "project" not in payload:
            inner = payload.get("project") if isinstance(payload, dict) else None
        ...
        body = _lp_write_body("project", payload)
        resp = requests.post(f"{base}/workspaces/{ws}/projects", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
