...
# https://developers.niftypm.com/operation/operation-projectapicontroller_getproejcts
...
def nifty_list_projects(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        params = {}
        if auth_info.get("subteam_id"):
            params["subteam_id"] = str(auth_info.get("subteam_id"))
        if auth_info.get("archived") is not None:
            params["archived"] = str(auth_info.get("archived")).lower()
        records, status, msg = _nf_paginate(f"{root}/projects", headers, params, "items", limit, timeout, verify_ssl)
