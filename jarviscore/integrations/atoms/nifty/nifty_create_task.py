...
# https://developers.niftypm.com/operation/operation-taskapicontroller_createtask
...
def nifty_create_task(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        headers, aerr = _nf_auth(auth_info, json_body=True)
        if aerr:
            return _nf_provision([], 400, aerr, [])
        body = payload if isinstance(payload, dict) else {}
        if not body.get("name"):
            return _nf_provision([], 400, "payload.name is required", [])
        resp = requests.post(f"{root}/tasks", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
        ...
        pid = _nf_provision_id(data if isinstance(data, dict) else {})
        rec = data if isinstance(data, dict) and data else ([{"id": pid}] if pid else [])
        records = [rec] if isinstance(rec, dict) else rec
        ids = [pid] if pid not in (None, "") else []
        return _nf_provision(records, resp.status_code, "ok", ids)
