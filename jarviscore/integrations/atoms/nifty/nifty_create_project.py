...
# https://developers.niftypm.com/operation/operation-projectapicontroller_createproject
...
def nifty_create_project(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        form = _nf_form_payload(body)
        resp = requests.post(f"{root}/projects", headers=headers, data=form, timeout=timeout, verify=verify_ssl)
        try:
