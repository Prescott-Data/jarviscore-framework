...
# https://developers.niftypm.com/operation/operation-taskapicontroller_gettasks
...
def nifty_list_tasks(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        for key in ("project_id", "task_group_id", "member_id", "milestone_id", "completed", "archived", "include_archived", "include_subtasks"):
            if auth_info.get(key) not in (None, ""):
                params[key] = str(auth_info.get(key))
        records, status, msg = _nf_paginate(f"{root}/tasks", headers, params, "tasks", limit, timeout, verify_ssl)
