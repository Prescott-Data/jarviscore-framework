...
# https://developers.niftypm.com/operation/operation-taskapicontroller_gettask
...
def nifty_get_task(auth_info: dict, task_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get task by ID. Official: https://developers.niftypm.com/operation/operation-taskapicontroller_gettask"""
    try:
        if not task_id:
        ...
        resp = requests.get(f"{root}/tasks/{task_id}", headers=headers, timeout=timeout, verify=verify_ssl)
