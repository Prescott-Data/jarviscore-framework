...
def backblaze_b2_list_files(auth_info: dict, bucket_id: Optional[str] = None, bucket_name: Optional[str] = None, prefix: str = "", limit: int = 100, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        while len(records) < cap:
            body = {"bucketId": bid, "maxFileCount": min(cap - len(records), 10000)}
            if prefix:
                body["prefix"] = prefix
            if start:
            ...
            for item in batch:
                if isinstance(item, dict) and item.get("action") == "upload":
                    records.append(item)
                    if len(records) >= cap:
                        break
