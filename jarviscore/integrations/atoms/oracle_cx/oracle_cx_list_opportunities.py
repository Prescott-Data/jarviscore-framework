async def oracle_cx_list_opportunities(instance_url: str, limit: int=25, offset: int=0) -> list:
    """Cx list opportunities via the oracle_cx API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/opportunities', params={'limit': limit, 'offset': offset})
    return [{'id': o.get('OptyId'), 'name': o.get('Name'), 'status': o.get('StatusCode'), 'sales_stage': o.get('SalesStage'), 'win_probability': o.get('WinProb'), 'revenue': o.get('Revenue'), 'currency': o.get('CurrencyCode'), 'close_date': o.get('TargetWinProb'), 'owner': o.get('OwnerName'), 'account': o.get('TargetPartyName')} for o in resp.get('items', [])]
