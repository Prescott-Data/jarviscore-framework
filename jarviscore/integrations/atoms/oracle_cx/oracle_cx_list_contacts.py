async def oracle_cx_list_contacts(instance_url: str, limit: int=25, offset: int=0) -> list:
    """Cx list contacts via the oracle_cx API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/contacts', params={'limit': limit, 'offset': offset})
    return [{'id': c.get('PartyId'), 'first_name': c.get('FirstName'), 'last_name': c.get('LastName'), 'email': c.get('EmailAddress'), 'phone': c.get('PhoneNumber'), 'title': c.get('JobTitle'), 'account': c.get('PrimaryOrganizationName'), 'owner': c.get('OwnerName'), 'created_at': c.get('CreationDate')} for c in resp.get('items', [])]
