def oracle_cx_list_accounts(auth_info: dict, instance_url: str, limit: int = 25, offset: int = 0) -> list:
    import requests
    ...
    def _delete(p, headers=None):
        ...
        _r.raise_for_status()
        return _r.json() if _r.content else {}
    resp = _get("/accounts", params={"limit": limit, "offset": offset})
    return [
        {
            "id": a.get("PartyId"),
            "name": a.get("OrganizationName"),
            "type": a.get("Type"),
            "status": a.get("AccountStatus"),
            "industry": a.get("IndustryCode"),
            "revenue": a.get("AnnualRevenue"),
