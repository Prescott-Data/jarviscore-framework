...
def gitlab_update_issue(auth_info: dict, project_id: str, issue_iid: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a project issue. Official: https://docs.gitlab.com/api/issues/#edit-an-issue"""
    try:
        ...
        if not issue_iid:
            return {"records": [], "data_count": 0, "status": 400, "message": "issue_iid is required", "provision_ids": []}
        if not isinstance(payload, dict) or not payload:
            return {"records": [], "data_count": 0, "status": 400, "message": "payload is required", "provision_ids": []}
        headers, auth_err = _gl_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        pid = _gl_path_enc(project_id)
        iid = _gl_path_enc(issue_iid)
        resp = requests.put(
            f"{api}/projects/{pid}/issues/{iid}", headers=headers, json=payload, timeout=timeout, verify=verify_ssl
        )
