...
def gitlab_get_issue(auth_info: dict, project_id: str, issue_iid: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a single project issue. Official: https://docs.gitlab.com/api/issues/#get-a-single-project-issue"""
    try:
        ...
        if not issue_iid:
            return {"records": [], "data_count": 0, "status": 400, "message": "issue_iid is required"}
        headers, auth_err = _gl_auth(auth_info)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        pid = _gl_path_enc(project_id)
        iid = _gl_path_enc(issue_iid)
        records, status, msg = _gl_get_entity(
            f"{api}/projects/{pid}/issues/{iid}", headers, timeout, verify_ssl
        )
