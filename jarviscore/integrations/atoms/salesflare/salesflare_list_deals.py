...
async def salesflare_list_deals(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List opportunities (deals). Official: https://api.salesflare.com/docs"""
    try:
        (records, status, msg) = await _sf_list('/opportunities', 'opportunities', base_url, limit, timeout, verify_ssl)
