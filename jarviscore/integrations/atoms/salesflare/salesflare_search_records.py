...
def salesflare_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        for path, key, rtype in (("/contacts", "contacts", "contact"), ("/accounts", "accounts", "account"), ("/opportunities", "opportunities", "opportunity")):
            if len(records) >= cap:
                break
            chunk, status, msg = _sf_list(path, key, base_url, auth_info, cap - len(records), timeout, verify_ssl, {"search": query})
