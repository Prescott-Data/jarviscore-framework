...
def salesflare_update_account(auth_info: dict, account_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update account. Official: https://api.salesflare.com/docs"""
    try:
        if not account_id:
            return _sf_provision({}, 400, "account_id is required")
        if not isinstance(payload, dict) or not payload:
            return _sf_provision({}, 400, "payload is required", account_id)
        resp, body, status, msg = _sf_request("PUT", f"/accounts/{account_id}", base_url, auth_info, None, payload, timeout, verify_ssl)
        if status >= 400:
            return _sf_provision(body if isinstance(body, dict) else {}, status, msg, account_id)
        return _sf_provision(body if isinstance(body, dict) else {}, status, "ok", account_id)
    except Exception as e:
        return _sf_provision({}, 500, str(e), account_id)
