...
def salesflare_get_contact(auth_info: dict, contact_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get contact. Official: https://api.salesflare.com/docs"""
    try:
        if not contact_id:
            return _sf_dataset([], 400, "contact_id is required")
        resp, body, status, msg = _sf_request("GET", f"/contacts/{contact_id}", base_url, auth_info, None, None, timeout, verify_ssl)
        if status >= 400:
