...
def salesflare_create_account(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create account. Official: https://api.salesflare.com/docs"""
    try:
        if not isinstance(payload, dict) or not payload:
            return _sf_provision({}, 400, "payload is required")
        resp, body, status, msg = _sf_request("POST", "/accounts", base_url, auth_info, None, payload, timeout, verify_ssl)
        if status >= 400:
