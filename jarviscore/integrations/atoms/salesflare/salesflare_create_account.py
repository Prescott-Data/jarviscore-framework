...
async def salesflare_create_account(payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Create account. Official: https://api.salesflare.com/docs"""
    try:
        if not isinstance(payload, dict) or not payload:
            return _sf_provision({}, 400, 'payload is required')
        resp, body, status, msg = await _sf_request('POST', '/accounts', base_url, None, payload, timeout, verify_ssl)
        if status >= 400:
