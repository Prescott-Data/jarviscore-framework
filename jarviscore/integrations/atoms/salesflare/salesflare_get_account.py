...
def salesflare_get_account(auth_info: dict, account_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get account. Official: https://api.salesflare.com/docs"""
    try:
        if not account_id:
            return _sf_dataset([], 400, "account_id is required")
        resp, body, status, msg = _sf_request("GET", f"/accounts/{account_id}", base_url, auth_info, None, None, timeout, verify_ssl)
        if status >= 400:
            return _sf_dataset([], status, msg)
        rec = body if isinstance(body, dict) else {}
        return _sf_dataset([rec] if rec else [], status, msg)
