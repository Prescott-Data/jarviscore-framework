...
async def salesflare_get_account(account_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get account. Official: https://api.salesflare.com/docs"""
    try:
        if not account_id:
            return _sf_dataset([], 400, 'account_id is required')
        (resp, body, status, msg) = await _sf_request('GET', f'/accounts/{account_id}', base_url, None, None, timeout, verify_ssl)
        if status >= 400:
