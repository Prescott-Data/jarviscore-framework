...
def salesflare_create_contact(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create contact. Official: https://api.salesflare.com/docs"""
    try:
        if not isinstance(payload, dict) or not payload:
            return _sf_provision({}, 400, "payload is required")
        resp, body, status, msg = _sf_request("POST", "/contacts", base_url, auth_info, None, payload, timeout, verify_ssl)
        if status >= 400:
            return _sf_provision(body if isinstance(body, dict) else {}, status, msg)
        obj = body if isinstance(body, dict) else {}
        return _sf_provision(obj, status, "ok")
