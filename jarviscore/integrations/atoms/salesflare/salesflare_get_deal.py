...
def salesflare_get_deal(auth_info: dict, deal_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get opportunity. Official: https://api.salesflare.com/docs"""
    try:
        if not deal_id:
            return _sf_dataset([], 400, "deal_id is required")
        resp, body, status, msg = _sf_request("GET", f"/opportunities/{deal_id}", base_url, auth_info, None, None, timeout, verify_ssl)
        if status >= 400:
