...
def square_list_orders(auth_info: dict, location_id: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        loc = location_id or auth_info.get("location_id")
        if not loc: return _sq_dataset([], 400, "location_id is required")
        body = {"location_ids": [loc], "limit": _sq_cap(limit)}
        _, data, status, msg = _sq_request("post", root + "/orders/search", auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
