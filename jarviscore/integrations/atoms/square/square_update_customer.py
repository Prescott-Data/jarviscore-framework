...
def square_update_customer(auth_info: dict, customer_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Square API v2: update customer. Official: https://developer.squareup.com/reference/square/customers-api/update-customer"""
    try:
        if not customer_id: return _sq_provision({}, 400, "customer_id is required")
        ...
        path = "/customers" if "customer" == "customer" else ("/orders" if "customer" == "order" else "/payments")
        body = payload if "customer" in payload else {"customer": payload}
        _, data, status, msg = _sq_request("put", root + path + "/{customer_id}".format(**locals()), auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400: return _sq_provision(data if isinstance(data, dict) else {}, status, msg)
        return _sq_provision(data if isinstance(data, dict) else {}, status, "ok", fallback_id=customer_id)
