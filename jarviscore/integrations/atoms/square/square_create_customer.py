...
def square_create_customer(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Square API v2: create customer. Official: https://developer.squareup.com/reference/square/customers-api/create-customer"""
    try:
        ...
        body = payload if "customer" in payload or "order" in payload or "amount_money" in payload else {"customer": payload, "idempotency_key": str(uuid.uuid4())}
        if "idempotency_key" not in body: body["idempotency_key"] = str(uuid.uuid4())
        _, data, status, msg = _sq_request("post", root + path, auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
