...
def square_get_order(auth_info: dict, order_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Square API v2: get order. Official: https://developer.squareup.com/reference/square/orders-api/retrieve-order"""
    try:
        if not order_id: return _sq_dataset([], 400, "order_id is required")
        ...
        _, data, status, msg = _sq_request("get", root + path + "/{order_id}".format(**locals()), auth_info, timeout=timeout, verify_ssl=verify_ssl)
