...
def square_get_payment(auth_info: dict, payment_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Square API v2: get payment. Official: https://developer.squareup.com/reference/square/payments-api/get-payment"""
    try:
        if not payment_id: return _sq_dataset([], 400, "payment_id is required")
        root, _ = _sq_root(base_url, auth_info)
        path = "/payments" if resource != "payment" else "/payments"
        if resource == "customer": path = "/customers"
        elif resource == "order": path = "/orders"
        elif resource == "payment": path = "/payments"
        _, data, status, msg = _sq_request("get", root + path + "/{payment_id}".format(**locals()), auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400: return _sq_dataset([], status, msg)
        obj = data.get("payment") if isinstance(data, dict) else {}
