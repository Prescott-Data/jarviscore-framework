...
async def square_get_payment(payment_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        if not payment_id:
            return _sq_dataset([], 400, 'payment_id is required')
        ...
        if resource == 'customer':
            ...
            path = '/payments'
        _, data, status, msg = await _sq_request('get', root + path + '/{payment_id}'.format(**locals()), timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
