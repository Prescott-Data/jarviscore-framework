...
def square_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        if not query: return _sq_dataset([], 400, "query is required")
        root, _ = _sq_root(base_url, auth_info)
        cap = _sq_cap(limit)
        # Square SearchCustomers: POST /v2/customers/search with a fuzzy email_address filter.
        body = {"limit": cap, "query": {"filter": {"email_address": {"fuzzy": str(query)}}}}
        _, data, status, msg = _sq_request("post", root + "/customers/search", auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400: return _sq_dataset([], status, msg)
        records = [x for x in (data.get("customers") or []) if isinstance(x, dict)][:cap]
        return _sq_dataset(records, status, "ok")
