...
def square_update_order(auth_info: dict, order_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Square API v2: update order. Official: https://developer.squareup.com/reference/square/orders-api/update-order"""
    try:
        if not order_id: return _sq_provision({}, 400, "order_id is required")
        ...
        _, data, status, msg = _sq_request("put", root + path + "/{order_id}".format(**locals()), auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
