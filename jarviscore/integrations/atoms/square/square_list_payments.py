...
async def square_list_payments(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        _, data, status, msg = await _sq_request('get', root + '/payments', {'limit': _sq_cap(limit)}, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
