...
def square_list_payments(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Square API v2: list payments. Official: https://developer.squareup.com/reference/square/payments-api/list-payments"""
    try:
        root, _ = _sq_root(base_url, auth_info)
        _, data, status, msg = _sq_request("get", root + "/payments", auth_info, {"limit": _sq_cap(limit)}, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400: return _sq_dataset([], status, msg)
        recs = [x for x in (data.get("payments") or []) if isinstance(x, dict)]
