...
def pendo_list_events(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        cap = _pn_cap(limit)
        pipeline = [{"source": _pn_source(auth_info, "trackTypes")}, {"limit": cap}]
        records, status, msg = _pn_aggregate(auth_info, pipeline, request_id="list-track-types", timeout=timeout, verify_ssl=verify_ssl)
