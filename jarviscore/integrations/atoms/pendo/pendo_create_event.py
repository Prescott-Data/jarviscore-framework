...
def pendo_create_event(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        host = _pn_host(base_url, auth_info)
        resp, data, status, err = _pn_request("post", host + "/data/track", auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl, track=True)
        if err:
