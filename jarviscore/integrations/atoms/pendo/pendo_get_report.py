...
def pendo_get_report(auth_info: dict, report_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        root, err = _pn_root(base_url, auth_info)
        if err:
            return _pn_dataset([], 400, err)
        if not report_id:
            return _pn_dataset([], 400, "report_id is required")
        auth_info = auth_info or {}
        fmt = auth_info.get("format") or "json"
        url = root + f"/report/{report_id}/results.{fmt.lstrip('.')}"
        resp, data, status, err = _pn_request("get", url, auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if err:
