...
def wrike_update_task(auth_info: dict, task_id: str, name: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wrike REST: update task. Official: https://developers.wrike.com/api/v4/"""
    try:
        ...
        resp = requests.put(root + "/tasks/" + str(task_id), headers=headers, data={'title': name} if name else {}, timeout=timeout, verify=verify_ssl)
        ...
        return _provision(obj, resp.status_code, 'ok', fallback_id=task_id)
