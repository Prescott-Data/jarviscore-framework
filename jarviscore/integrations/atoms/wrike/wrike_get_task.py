...
def wrike_get_task(auth_info: dict, task_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wrike REST: get task. Official: https://developers.wrike.com/api/v4/"""
    try:
        root, err = _root(base_url, auth_info)
        if err: return _dataset([], 400, err)
        if not task_id: return _dataset([], 400, 'task_id is required')
        headers, aerr = _auth(auth_info)
        if aerr: return _dataset([], 401, aerr)
        resp = requests.get(root + "/tasks/" + str(task_id), headers=headers, timeout=timeout, verify=verify_ssl)
