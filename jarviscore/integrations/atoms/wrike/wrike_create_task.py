...
def wrike_create_task(auth_info: dict, project_id: str, title: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not title: return _provision({}, 400, "title is required")
        ...
        resp = requests.post(root + "/folders/" + str(project_id) + "/tasks", headers=headers, data={'title': title}, timeout=timeout, verify=verify_ssl)
