...
def wrike_create_project(auth_info: dict, name: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wrike REST: create project. Official: https://developers.wrike.com/api/v4/"""
    try:
        if not name: return _provision({}, 400, "name is required")
        root, err = _root(base_url, auth_info)
        if err: return _provision({}, 400, err)
        parent = (auth_info or {}).get("parent_id") or (auth_info or {}).get("folder_id") or (auth_info or {}).get("space_id")
        if not parent: return _provision({}, 400, "auth_info.parent_id (or folder_id/space_id) is required to create a project")
        headers, aerr = _auth(auth_info)
        if aerr: return _provision({}, 401, aerr)
        resp = requests.post(root + "/folders/" + str(parent) + "/folders", headers=headers, data={'title': name, 'project': '{}'}, timeout=timeout, verify=verify_ssl)
