...
def safaricom_mpesa_create_order(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        body_payload, err = _mp_stk_fields(auth_info, payload)
        if err:
        ...
        resp, body, status, msg = _mp_post("/mpesa/stkpush/v1/processrequest", base_url, auth_info, body_payload, timeout, verify_ssl)
        if status >= 400:
