async def bamboo_trigger_build(cloud_id: str, plan_key: str, custom_revision: str=None) -> dict:
    ...
    resp = await _post(f'/rest/api/latest/queue/{plan_key}', data=params or None)
    return {'build_number': resp.get('buildNumber'), 'build_result_key': resp.get('buildResultKey'), 'plan_key': resp.get('planKey'), 'trigger_reason': resp.get('triggerReason'), 'href': resp.get('link', {}).get('href')}
