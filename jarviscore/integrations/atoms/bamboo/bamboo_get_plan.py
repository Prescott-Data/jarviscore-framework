async def bamboo_get_plan(cloud_id: str, plan_key: str) -> dict:
    """Get plan via the bamboo API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    p = await _get(f'/rest/api/latest/plan/{plan_key}')
    return {'key': p['key'], 'name': p['name'], 'project_key': p.get('projectKey'), 'description': p.get('description'), 'enabled': p.get('enabled', True), 'is_building': p.get('isBuilding', False), 'average_build_time_seconds': p.get('averageBuildTimeInSeconds'), 'href': p.get('link', {}).get('href')}
