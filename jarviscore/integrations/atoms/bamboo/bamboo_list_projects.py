async def bamboo_list_projects(cloud_id: str, max_results: int=25) -> list:
    """List projects via the bamboo API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/rest/api/latest/project', params={'max-result': max_results, 'expand': 'projects.project'})
    projects = resp.get('projects', {}).get('project', [])
    return [{'key': p['key'], 'name': p['name'], 'description': p.get('description'), 'href': p.get('link', {}).get('href')} for p in projects]
