...
def workfront_update_project(auth_info: dict, project_id: str, name: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """workfront REST: update project. Official: https://developer.adobe.com/workfront/api-explorer/"""
    try:
        ...
        resp = requests.put(root + "/proj/" + str(project_id), headers=headers, json={'name': name} if name else {}, timeout=timeout, verify=verify_ssl)
        ...
        return _provision(obj if isinstance(obj, dict) else {}, resp.status_code, 'ok', fallback_id=project_id)
