...
def workfront_get_project(auth_info: dict, project_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """workfront REST: get project. Official: https://developer.adobe.com/workfront/api-explorer/"""
    try:
        root, err = _root(base_url, auth_info)
        if err: return _dataset([], 400, err)
        if not project_id: return _dataset([], 400, 'project_id is required')
        headers, aerr = _auth(auth_info)
        if aerr: return _dataset([], 401, aerr)
        resp = requests.get(root + "/proj/" + str(project_id), headers=headers, timeout=timeout, verify=verify_ssl)
