...
def workfront_create_task(auth_info: dict, name: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        proj_id = (auth_info or {}).get('project_id') or (auth_info or {}).get('projectID')
        if not proj_id: return _provision({}, 400, 'auth_info.project_id is required to create a task')
        resp = requests.post(root + "/task", headers=headers, json={'name': name, 'projectID': str(proj_id)}, timeout=timeout, verify=verify_ssl)
