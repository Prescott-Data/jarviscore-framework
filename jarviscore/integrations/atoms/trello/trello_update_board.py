...
def trello_update_board(auth_info: dict, board_id: str, name: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not board_id: return _tr_provision({}, 400, "board_id is required")
        params, aerr = _tr_auth_params(auth_info)
        if aerr: return _tr_provision({}, 401, aerr)
        if name: params["name"] = name
        resp = requests.put(f"{root}/boards/{board_id}", params=params, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _tr_provision({}, resp.status_code, _tr_err(resp))
        data = resp.json() if resp.content else {}
        return _tr_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok", fallback_id=board_id)
