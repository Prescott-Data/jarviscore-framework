...
def trello_get_card(auth_info: dict, card_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not card_id: return _tr_dataset([], 400, "card_id is required")
        params, aerr = _tr_auth_params(auth_info)
        if aerr: return _tr_dataset([], 401, aerr)
        resp = requests.get(f"{root}/cards/{card_id}", params=params, timeout=timeout, verify=verify_ssl)
