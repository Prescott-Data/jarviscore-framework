...
async def trello_get_list(list_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not list_id:
            return _tr_dataset([], 400, 'list_id is required')
        (params, aerr) = _tr_auth_params()
        if aerr:
            return _tr_dataset([], 401, aerr)
        resp = await nexus_call('GET', f'{root}/lists/{list_id}', params=params)
