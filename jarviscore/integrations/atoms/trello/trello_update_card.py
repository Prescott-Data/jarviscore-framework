...
async def trello_update_card(card_id: str, name: str='', timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not card_id:
            return _tr_provision({}, 400, 'card_id is required')
        (params, aerr) = _tr_auth_params()
        if aerr:
            return _tr_provision({}, 401, aerr)
        if name:
            params['name'] = name
        resp = await nexus_call('PUT', f'{root}/cards/{card_id}', params=params)
