...
def trello_create_card(auth_info: dict, name: str, list_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not name or not list_id: return _tr_provision({}, 400, "name and list_id are required")
        params, aerr = _tr_auth_params(auth_info)
        if aerr: return _tr_provision({}, 401, aerr)
        params.update({"name": name, "idList": list_id})
        resp = requests.post(f"{root}/cards", params=params, timeout=timeout, verify=verify_ssl)
