...
async def trello_get_board(board_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not board_id:
            return _tr_dataset([], 400, 'board_id is required')
        params, aerr = _tr_auth_params()
        if aerr:
            return _tr_dataset([], 401, aerr)
        resp = await nexus_call('GET', f'{root}/boards/{board_id}', params=params)
