...
def trello_get_board(auth_info: dict, board_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Trello REST: get board. Official: https://developer.atlassian.com/cloud/trello/rest/"""
    try:
        root, err = _tr_root(base_url, auth_info)
        if err: return _tr_dataset([], 400, err)
        if not board_id: return _tr_dataset([], 400, "board_id is required")
        params, aerr = _tr_auth_params(auth_info)
        if aerr: return _tr_dataset([], 401, aerr)
        resp = requests.get(f"{root}/boards/{board_id}", params=params, timeout=timeout, verify=verify_ssl)
