...
async def trello_create_list(name: str, board_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not name or not board_id:
            return _tr_provision({}, 400, 'name and board_id are required')
        (params, aerr) = _tr_auth_params()
        if aerr:
            return _tr_provision({}, 401, aerr)
        params.update({'name': name, 'idBoard': board_id})
        resp = await nexus_call('POST', f'{root}/lists', params=params)
