...
def trello_create_list(auth_info: dict, name: str, board_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Trello REST: create list. Official: https://developer.atlassian.com/cloud/trello/rest/"""
    try:
        root, err = _tr_root(base_url, auth_info)
        if err: return _tr_provision({}, 400, err)
        if not name or not board_id: return _tr_provision({}, 400, "name and board_id are required")
        params, aerr = _tr_auth_params(auth_info)
        if aerr: return _tr_provision({}, 401, aerr)
        params.update({"name": name, "idBoard": board_id})
        resp = requests.post(f"{root}/lists", params=params, timeout=timeout, verify=verify_ssl)
