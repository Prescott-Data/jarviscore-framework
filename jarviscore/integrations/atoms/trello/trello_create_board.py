...
def trello_create_board(auth_info: dict, name: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not name: return _tr_provision({}, 400, "name is required")
        params, aerr = _tr_auth_params(auth_info)
        if aerr: return _tr_provision({}, 401, aerr)
        params["name"] = name
        resp = requests.post(f"{root}/boards", params=params, timeout=timeout, verify=verify_ssl)
