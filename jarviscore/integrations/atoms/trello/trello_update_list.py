...
def trello_update_list(auth_info: dict, list_id: str, name: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Trello REST: update list. Official: https://developer.atlassian.com/cloud/trello/rest/"""
    try:
        root, err = _tr_root(base_url, auth_info)
        if err: return _tr_provision({}, 400, err)
        if not list_id: return _tr_provision({}, 400, "list_id is required")
        params, aerr = _tr_auth_params(auth_info)
        if aerr: return _tr_provision({}, 401, aerr)
        if name: params["name"] = name
        resp = requests.put(f"{root}/lists/{list_id}", params=params, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _tr_provision({}, resp.status_code, _tr_err(resp))
        data = resp.json() if resp.content else {}
        return _tr_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok", fallback_id=list_id)
