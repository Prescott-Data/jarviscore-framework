...
def less_annoying_crm_list_deals(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        pipeline_ids, status, perr = _lacrm_pipeline_ids(base, headers, timeout, verify_ssl)
        if perr: return {"records": [], "data_count": 0, "status": status, "message": perr}
        if not pipeline_ids: return {"records": [], "data_count": 0, "status": 200, "message": "ok"}
        cap = min(max(int(limit or 25), 1), 10000)
        records: List[Dict[str, Any]] = []
        for pid in pipeline_ids:
            if len(records) >= cap:
                break
            batch, status, msg = _lacrm_paginate(base, headers, "GetPipelineItems", {"PipelineId": pid}, cap - len(records), timeout, verify_ssl)
