...
async def less_annoying_crm_list_deals(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List pipeline items (deals) via GetPipelineItems. Official: https://account.lessannoyingcrm.com/api_docs/v2/Core_Functions/Pipeline_Items"""
    try:
        ...
        (pipeline_ids, status, perr) = await _lacrm_pipeline_ids(base, headers, timeout, verify_ssl)
        if perr:
            return {'records': [], 'data_count': 0, 'status': status, 'message': perr}
        if not pipeline_ids:
            return {'records': [], 'data_count': 0, 'status': 200, 'message': 'ok'}
        cap = min(max(int(limit or 25), 1), 10000)
        records: List[Dict[str, Any]] = []
        for pid in pipeline_ids:
            if len(records) >= cap:
                break
            (batch, status, msg) = await _lacrm_paginate(base, headers, 'GetPipelineItems', {'PipelineId': pid}, cap - len(records), timeout, verify_ssl)
            if msg != 'ok' and (not batch):
...
async def _lacrm_pipeline_ids(base, headers, timeout, verify_ssl):
    resp = await _lacrm_call(base, headers, 'GetPipelines', {}, timeout, verify_ssl)
    data = _lacrm_data(resp)
    err = _lacrm_err(resp, data)
    if err:
