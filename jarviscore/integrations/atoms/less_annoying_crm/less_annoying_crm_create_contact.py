...
def less_annoying_crm_create_contact(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create contact via CreateContact IsCompany=false. Official: https://account.lessannoyingcrm.com/api_docs/v2/Core_Functions/Contacts"""
    try:
        ...
        params = dict(payload)
        params.setdefault("IsCompany", False)
        resp = _lacrm_call(base, headers, "CreateContact", params, timeout, verify_ssl)
        data = _lacrm_data(resp)
        errm = _lacrm_err(resp, data)
        if errm: return {"records": [], "data_count": 0, "status": resp.status_code, "message": errm}
        ids = _lacrm_provision_contact(data)
        records = [{"ContactId": i} for i in ids] if ids else []
