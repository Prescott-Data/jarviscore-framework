...
def less_annoying_crm_get_deal(auth_info: dict, deal_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get pipeline item via GetPipelineItem. Official: https://account.lessannoyingcrm.com/api_docs/v2/Core_Functions/Pipeline_Items"""
    try:
        if not deal_id: return {"records": [], "data_count": 0, "status": 400, "message": "deal_id (PipelineItemId) is required"}
        ...
        resp = _lacrm_call(base, headers, "GetPipelineItem", {"PipelineItemId": str(deal_id)}, timeout, verify_ssl)
        data = _lacrm_data(resp)
        errm = _lacrm_err(resp, data)
        if errm: return {"records": [], "data_count": 0, "status": resp.status_code, "message": errm}
        records = _lacrm_single_pipeline(data)
