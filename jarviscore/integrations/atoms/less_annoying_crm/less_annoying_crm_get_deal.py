...
async def less_annoying_crm_get_deal(deal_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        if not deal_id:
            return {'records': [], 'data_count': 0, 'status': 400, 'message': 'deal_id (PipelineItemId) is required'}
        ...
        resp = await _lacrm_call(base, headers, 'GetPipelineItem', {'PipelineItemId': str(deal_id)}, timeout, verify_ssl)
