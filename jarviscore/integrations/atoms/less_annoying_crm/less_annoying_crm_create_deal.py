...
def less_annoying_crm_create_deal(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        params = dict(payload)
        if not params.get("ContactId") or not params.get("PipelineId") or not params.get("StatusId"):
            return {"records": [], "data_count": 0, "status": 400, "message": "payload.ContactId, payload.PipelineId, and payload.StatusId are required"}
        resp = _lacrm_call(base, headers, "CreatePipelineItem", params, timeout, verify_ssl)
        data = _lacrm_data(resp)
        errm = _lacrm_err(resp, data)
        if errm: return {"records": [], "data_count": 0, "status": resp.status_code, "message": errm}
        ids = _lacrm_provision_pipeline(data)
        records = [{"PipelineItemId": i} for i in ids] if ids else []
        return {"records": records, "data_count": len(records), "status": resp.status_code, "message": "ok", "provision_ids": ids}
