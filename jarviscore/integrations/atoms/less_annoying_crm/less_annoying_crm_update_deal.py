...
def less_annoying_crm_update_deal(auth_info: dict, deal_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        params["PipelineItemId"] = str(deal_id)
        resp = _lacrm_call(base, headers, "EditPipelineItem", params, timeout, verify_ssl)
        data = _lacrm_data(resp)
        errm = _lacrm_err(resp, data)
        if errm: return {"records": [], "data_count": 0, "status": resp.status_code, "message": errm}
        return {"records": [{"PipelineItemId": deal_id}], "data_count": 1, "status": resp.status_code, "message": "ok", "provision_ids": [deal_id]}
