...
def height_get_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        # Height has no "get a single list" endpoint; list all lists and match by id or key.
        resp = _height_get(f"{api}/lists", headers, None, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        pid = str(project_id)
        match = next(
            (item for item in _height_items(resp.json() if resp.text else {})
             if isinstance(item, dict) and pid in (str(item.get("id")), str(item.get("key")))),
            None,
        )
