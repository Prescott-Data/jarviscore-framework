...
def height_update_project(auth_info: dict, project_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a Height list. Official: https://height.app/api-docs"""
    try:
        if not project_id:
        ...
        headers, auth_err = _height_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        resp = _height_put(f"{api}/lists/{project_id}", headers, payload, timeout, verify_ssl)
...
def _height_put(url, headers, body, timeout, verify_ssl):
