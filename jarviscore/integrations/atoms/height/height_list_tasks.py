...
def height_list_tasks(auth_info: dict, project_id: Optional[str] = None, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        extra = {}
        lid = _height_list_id(auth_info, None, project_id)
        if lid:
            extra["listIds"] = lid
        records, status, message = _height_paginate(f"{api}/tasks", headers, limit, timeout, verify_ssl, extra)
