...
def zoho_crm_get_deal(auth_info: dict, deal_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_crm REST: get deal. Official: https://www.zoho.com/crm/developer/docs/api/v2/"""
    try:
        if not deal_id:
        ...
        resp = requests.get(root + "/Deals/" + str(deal_id), headers=headers, timeout=timeout, verify=verify_ssl)
