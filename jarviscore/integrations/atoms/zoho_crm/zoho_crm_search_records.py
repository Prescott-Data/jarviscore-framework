...
def zoho_crm_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_crm REST: search. Official: https://www.zoho.com/crm/developer/docs/api/v2/"""
    try:
        if not query:
        ...
        module = str((auth_info or {}).get("module") or "Deals").strip()
        resp = requests.get(root + "/" + module + "/search", headers=headers, params={"word": str(query), "per_page": _cap(limit)}, timeout=timeout, verify=verify_ssl)
...
# Search: GET /{module}/search supports word/email/phone/criteria query params.
