...
def zoho_crm_get_lead(auth_info: dict, lead_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_crm REST: get lead. Official: https://www.zoho.com/crm/developer/docs/api/v2/"""
    try:
        if not lead_id:
        ...
        resp = requests.get(root + "/Leads/" + str(lead_id), headers=headers, timeout=timeout, verify=verify_ssl)
