...
def zoho_crm_update_lead(auth_info: dict, lead_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_crm REST: update lead. Official: https://www.zoho.com/crm/developer/docs/api/v2/"""
    try:
        if not lead_id:
            return _provision({}, 400, "lead_id is required")
        ...
        resp = requests.put(root + "/Leads/" + str(lead_id), headers=headers, json={"data": [payload]}, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400:
            return _provision({}, resp.status_code, _err(resp))
        return _provision(data if isinstance(data, dict) else {}, resp.status_code, "ok", fallback_id=lead_id)
