...
def zoho_crm_update_deal(auth_info: dict, deal_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_crm REST: update deal. Official: https://www.zoho.com/crm/developer/docs/api/v2/"""
    try:
        if not deal_id:
            return _provision({}, 400, "deal_id is required")
        ...
        resp = requests.put(root + "/Deals/" + str(deal_id), headers=headers, json={"data": [payload]}, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400:
            return _provision({}, resp.status_code, _err(resp))
        return _provision(data if isinstance(data, dict) else {}, resp.status_code, "ok", fallback_id=deal_id)
