...
def freshbooks_get_expense(auth_info: dict, expense_id: str) -> dict:
    import requests
    try:
    ...
    try:
        resp = requests.get(
            f"https://api.freshbooks.com/accounting/account/{account_id}/expenses/expenses/{expense_id}",
            headers={"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"},
            timeout=30
        )
        if resp.status_code != 200:
            return {"success": False, "expense_id": expense_id, "data": None, "error": f"Get expense failed: {resp.status_code} {resp.text}"}
