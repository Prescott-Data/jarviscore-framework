...
def pivotal_tracker_get_task(auth_info: dict, task_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _pt_request("get", root + path, auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
