...
def pivotal_tracker_update_task(auth_info: dict, task_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _pt_request("put", root + path, auth_info, json_body=body_payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
