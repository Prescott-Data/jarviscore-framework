...
async def pivotal_tracker_get_project(project_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        (resp, body, status, msg) = await _pt_request('get', root + '/projects/' + str(project_id).strip(), timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
