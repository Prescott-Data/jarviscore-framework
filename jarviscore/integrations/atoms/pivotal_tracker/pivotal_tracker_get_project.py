...
def pivotal_tracker_get_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get project by ID. Official: https://www.pivotaltracker.com/help/api/rest/v5"""
    try:
        if not project_id:
            return _pt_dataset([], 400, "project_id is required")
        root, err = _pt_root(base_url, auth_info)
        if err:
            return _pt_dataset([], 400, err)
        resp, body, status, msg = _pt_request("get", root + "/projects/" + str(project_id).strip(), auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
