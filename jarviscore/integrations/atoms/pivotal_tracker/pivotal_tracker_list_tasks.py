...
def pivotal_tracker_list_tasks(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List stories (catalog tasks) for a project. Official: https://www.pivotaltracker.com/help/api/rest/v5"""
    try:
        auth_info = auth_info or {}
        project_id = _pt_project_id(auth_info)
        if not project_id:
            return _pt_dataset([], 400, "auth_info.project_id is required for story list")
        extra = auth_info.get("filters") or auth_info.get("params")
        if not isinstance(extra, dict):
            extra = {}
        path = "/projects/" + str(project_id).strip() + "/stories"
        records, status, msg = _pt_paginate(path, base_url, auth_info, limit, timeout, verify_ssl, extra_params=extra)
