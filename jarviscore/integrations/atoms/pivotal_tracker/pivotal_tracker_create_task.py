...
def pivotal_tracker_create_task(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        project_id = _pt_project_id(auth_info, payload)
        if not project_id:
            return _pt_provision({}, 400, "auth_info.project_id or payload.project_id is required")
        ...
        resp, body, status, msg = _pt_request("post", root + path, auth_info, json_body=body_payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
