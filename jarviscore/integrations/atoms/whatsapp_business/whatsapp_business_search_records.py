...
def whatsapp_business_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not query: return _wa_dataset([], 400, "query is required")
        resp = requests.get(f"{root}/{waba}/message_templates", headers=headers, params={"limit": 100}, timeout=timeout, verify=verify_ssl)
