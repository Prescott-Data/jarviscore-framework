...
def shift4shop_update_customer(auth_info: dict, customer_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shift4Shop REST: update customer. Official: https://developers.3dcart.com/rest-api/customers"""
    try:
        if not customer_id: return _s4_provision({}, 400, "customer_id is required")
        ...
        resp = requests.put(root + "/Customers/" + str(customer_id), headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
