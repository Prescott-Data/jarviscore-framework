...
def shift4shop_get_product(auth_info: dict, product_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shift4Shop REST: get product. Official: https://developers.3dcart.com/rest-api/products"""
    try:
        if not product_id: return _s4_dataset([], 400, "product_id is required")
        ...
        resp = requests.get(root + "/Products/" + str(product_id), headers=headers, timeout=timeout, verify=verify_ssl)
