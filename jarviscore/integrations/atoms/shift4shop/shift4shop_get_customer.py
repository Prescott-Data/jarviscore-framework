...
async def shift4shop_get_customer(customer_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Shift4Shop REST: get customer. Official: https://developers.3dcart.com/rest-api/customers"""
    try:
        if not customer_id:
            return _s4_dataset([], 400, 'customer_id is required')
        ...
        resp = await nexus_call('GET', root + '/Customers/' + str(customer_id), headers=headers)
        try:
