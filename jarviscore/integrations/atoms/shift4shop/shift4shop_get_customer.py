...
def shift4shop_get_customer(auth_info: dict, customer_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shift4Shop REST: get customer. Official: https://developers.3dcart.com/rest-api/customers"""
    try:
        if not customer_id: return _s4_dataset([], 400, "customer_id is required")
        ...
        resp = requests.get(root + "/Customers/" + str(customer_id), headers=headers, timeout=timeout, verify=verify_ssl)
        try: data = resp.json() if resp.content else {}
        except Exception: data = {}
        if resp.status_code >= 400: return _s4_dataset([], resp.status_code, _s4_err(resp))
