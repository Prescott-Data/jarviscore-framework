...
def shift4shop_update_product(auth_info: dict, product_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shift4Shop REST: update product. Official: https://developers.3dcart.com/rest-api/products"""
    try:
        if not product_id: return _s4_provision({}, 400, "product_id is required")
        ...
        resp = requests.put(root + "/Products/" + str(product_id), headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
        ...
        return _s4_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok", fallback_id=product_id)
