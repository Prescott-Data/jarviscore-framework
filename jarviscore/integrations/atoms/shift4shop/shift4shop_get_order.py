...
async def shift4shop_get_order(order_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Shift4Shop REST: get order. Official: https://developers.3dcart.com/rest-api/orders"""
    try:
        if not order_id:
            return _s4_dataset([], 400, 'order_id is required')
        ...
        resp = await nexus_call('GET', root + '/Orders/' + str(order_id), headers=headers)
        try:
