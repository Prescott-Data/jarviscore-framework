...
def matomo_update_report(auth_info: dict, id_site: str, report_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a scheduled report via ScheduledReports.updateReport. Official: https://developer.matomo.org/api-reference/reporting-api#module_scheduledreports"""
    try:
        if not report_id:
        ...
        resp, data, err = _mt_api_call(base_url, "ScheduledReports.updateReport", params, auth_info, timeout, verify_ssl)
        if err:
        ...
        return _mt_provision(data, status, fallback_id=report_id)
