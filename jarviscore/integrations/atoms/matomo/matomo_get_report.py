...
def matomo_get_report(auth_info: dict, id_site: str, report_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a scheduled report by idReport via ScheduledReports.getReports. Official: https://developer.matomo.org/api-reference/reporting-api#module_scheduledreports"""
    try:
        if not report_id:
        ...
        resp, data, err = _mt_api_call(
            ...
            {"idSite": site, "idReport": str(report_id)},
            auth_info,
            timeout,
            verify_ssl,
        )
