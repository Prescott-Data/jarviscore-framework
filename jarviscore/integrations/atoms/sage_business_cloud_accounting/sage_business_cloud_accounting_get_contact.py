...
def sage_business_cloud_accounting_get_contact(auth_info: dict, contact_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get contact by id. Official: https://developer.sage.com/accounting/reference/"""
    try:
        if not contact_id:
            return _sage_dataset([], 400, "contact_id is required")
        resp, body, status, msg = _sage_get(f"/contacts/{contact_id}", base_url, auth_info, None, timeout, verify_ssl)
        if status >= 400:
