...
def sage_business_cloud_accounting_create_payment(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create contact payment. Official: https://developer.sage.com/accounting/reference/"""
    try:
        if not isinstance(payload, dict) or not payload:
            return _sage_provision({}, 400, "payload is required", "contact_payment")
        body_payload = _sage_wrap(payload, "contact_payment")
        resp, body, status, msg = _sage_write("POST", "/contact_payments", base_url, auth_info, body_payload, timeout, verify_ssl)
        if status >= 400:
            return _sage_provision(body if isinstance(body, dict) else {}, status, msg, "contact_payment")
        return _sage_provision(body if isinstance(body, dict) else {}, status, "ok", "contact_payment")
    except Exception as e:
        return _sage_provision({}, 500, str(e), "contact_payment")
