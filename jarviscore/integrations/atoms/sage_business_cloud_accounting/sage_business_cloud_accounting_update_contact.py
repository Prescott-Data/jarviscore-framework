...
async def sage_business_cloud_accounting_update_contact(contact_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        (resp, body, status, msg) = await _sage_write('PUT', f'/contacts/{contact_id}', base_url, body_payload, timeout, verify_ssl)
        if status >= 400:
