...
def sage_business_cloud_accounting_create_invoice(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _sage_write("POST", "/sales_invoices", base_url, auth_info, body_payload, timeout, verify_ssl)
        if status >= 400:
