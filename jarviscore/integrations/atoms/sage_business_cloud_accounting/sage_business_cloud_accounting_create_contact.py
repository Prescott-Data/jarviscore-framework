...
async def sage_business_cloud_accounting_create_contact(payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = await _sage_write('POST', '/contacts', base_url, body_payload, timeout, verify_ssl)
        if status >= 400:
