...
def sage_business_cloud_accounting_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search contacts and sales invoices. Official: https://developer.sage.com/accounting/reference/"""
    try:
        if not query:
        ...
        for path, rtype in (("/contacts", "contact"), ("/sales_invoices", "sales_invoice")):
            if len(records) >= cap:
                break
            chunk, status, msg = _sage_list_collection(path, base_url, auth_info, cap - len(records), timeout, verify_ssl, {"search": query})
            ...
            for item in chunk:
                ...
                records.append(rec)
        return _sage_dataset(records[:cap], 200, "ok")
