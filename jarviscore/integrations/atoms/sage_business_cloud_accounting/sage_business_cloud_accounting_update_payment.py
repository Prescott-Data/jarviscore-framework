...
def sage_business_cloud_accounting_update_payment(auth_info: dict, payment_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update contact payment. Official: https://developer.sage.com/accounting/reference/"""
    try:
        if not payment_id:
            return _sage_provision({}, 400, "payment_id is required", "contact_payment", payment_id)
        if not isinstance(payload, dict) or not payload:
            return _sage_provision({}, 400, "payload is required", "contact_payment", payment_id)
        body_payload = _sage_wrap(payload, "contact_payment")
        resp, body, status, msg = _sage_write("PUT", f"/contact_payments/{payment_id}", base_url, auth_info, body_payload, timeout, verify_ssl)
        if status >= 400:
            return _sage_provision(body if isinstance(body, dict) else {}, status, msg, "contact_payment", payment_id)
        return _sage_provision(body if isinstance(body, dict) else {}, status, "ok", "contact_payment", payment_id)
    except Exception as e:
        return _sage_provision({}, 500, str(e), "contact_payment", payment_id)
