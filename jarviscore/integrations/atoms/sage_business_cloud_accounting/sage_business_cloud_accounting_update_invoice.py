...
def sage_business_cloud_accounting_update_invoice(auth_info: dict, invoice_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update sales invoice. Official: https://developer.sage.com/accounting/reference/"""
    try:
        if not invoice_id:
            return _sage_provision({}, 400, "invoice_id is required", "sales_invoice", invoice_id)
        ...
        resp, body, status, msg = _sage_write("PUT", f"/sales_invoices/{invoice_id}", base_url, auth_info, body_payload, timeout, verify_ssl)
        if status >= 400:
        ...
        return _sage_provision({}, 500, str(e), "sales_invoice", invoice_id)
