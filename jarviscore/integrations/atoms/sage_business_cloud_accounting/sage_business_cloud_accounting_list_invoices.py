...
async def sage_business_cloud_accounting_list_invoices(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List sales invoices. Official: https://developer.sage.com/accounting/reference/"""
    try:
        (records, status, msg) = await _sage_list_collection('/sales_invoices', base_url, limit, timeout, verify_ssl)
