...
async def sage_business_cloud_accounting_get_invoice(invoice_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get sales invoice. Official: https://developer.sage.com/accounting/reference/"""
    try:
        if not invoice_id:
            return _sage_dataset([], 400, 'invoice_id is required')
        (resp, body, status, msg) = await _sage_get(f'/sales_invoices/{invoice_id}', base_url, None, timeout, verify_ssl)
        if status >= 400:
