...
def sage_business_cloud_accounting_get_payment(auth_info: dict, payment_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get contact payment. Official: https://developer.sage.com/accounting/reference/"""
    try:
        if not payment_id:
            return _sage_dataset([], 400, "payment_id is required")
        resp, body, status, msg = _sage_get(f"/contact_payments/{payment_id}", base_url, auth_info, None, timeout, verify_ssl)
        if status >= 400:
