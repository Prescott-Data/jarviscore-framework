...
def agilecrm_list_accounts(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List companies via POST /contacts/companies/list. Official: https://github.com/agilecrm/rest-api#23-get-list-of-companies"""
    try:
        ...
        status = 200
        pages = 0
        url = f"{api_root}/contacts/companies/list"
        while len(records) < cap and pages < 50:
            pages += 1
            form = {"page_size": str(cap), "global_sort_key": "-created_time"}
            if cursor:
                form["cursor"] = cursor
            resp = requests.post(
                url,
...
# POST /contacts/companies/list — https://github.com/agilecrm/rest-api#23-get-list-of-companies
