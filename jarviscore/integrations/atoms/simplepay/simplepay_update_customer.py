...
def simplepay_update_customer(auth_info: dict, customer_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """SimplePay: update employee (customer). Official: https://www.simplepay.co.za/api-docs/"""
    try:
        if not customer_id: return _sp_provision({}, 400, "customer_id is required")
        ...
        resp = requests.patch(f"{root}/employees/{customer_id}", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try: data = resp.json() if resp.content else {}
        except Exception: data = {}
        if resp.status_code >= 400: return _sp_provision(data if isinstance(data, dict) else {}, resp.status_code, _sp_err(resp))
