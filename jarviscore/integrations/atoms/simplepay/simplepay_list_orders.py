...
def simplepay_list_orders(auth_info: dict, employee_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not employee_id: return _sp_dataset([], 400, "employee_id is required")
        headers, aerr = _sp_auth(auth_info)
        if aerr: return _sp_dataset([], 401, aerr)
        cap = _sp_cap(limit)
        resp = requests.get(f"{root}/employees/{employee_id}/payslips", headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _sp_dataset([], resp.status_code, _sp_err(resp))
        data = resp.json() if resp.content else []
        records = _sp_unwrap_items(data, "payslip")[:cap]
        return _sp_dataset(records, resp.status_code, "ok")
