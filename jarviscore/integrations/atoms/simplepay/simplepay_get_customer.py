...
def simplepay_get_customer(auth_info: dict, customer_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not customer_id: return _sp_dataset([], 400, "customer_id is required")
        headers, aerr = _sp_auth(auth_info)
        if aerr: return _sp_dataset([], 401, aerr)
        resp = requests.get(f"{root}/employees/{customer_id}", headers=headers, timeout=timeout, verify=verify_ssl)
