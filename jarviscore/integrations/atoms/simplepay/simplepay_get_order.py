...
async def simplepay_get_order(order_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not order_id:
            return _sp_dataset([], 400, 'order_id is required (payslip id)')
        (headers, aerr) = _sp_auth()
        if aerr:
            return _sp_dataset([], 401, aerr)
        resp = await nexus_call('GET', f'{root}/payslips/{order_id}', headers=headers)
