...
def twitter_ads_create_campaign(auth_info: dict, name: str, funding_instrument_id: str, daily_budget_amount_local_micro: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not name or not funding_instrument_id: return _xa_provision({}, 400, "name and funding_instrument_id are required")
        ...
        resp = requests.post(f"{root}/accounts/{aid}/campaigns", headers=headers, params=params, timeout=timeout, verify=verify_ssl)
