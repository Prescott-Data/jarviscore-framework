...
def twitter_ads_get_ad(auth_info: dict, promoted_tweet_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """X Ads API: get ad. Official: https://developer.x.com/en/docs/twitter-ads-api"""
    try:
        root, err = _xa_root(base_url, auth_info)
        if err: return _xa_dataset([], 400, err)
        if not promoted_tweet_id: return _xa_dataset([], 400, "promoted_tweet_id is required")
        headers, aerr = _xa_auth(auth_info)
        if aerr: return _xa_dataset([], 401, aerr)
        aid, err = _xa_account(auth_info)
        if err: return _xa_dataset([], 400, err)
        resp = requests.get(f"{root}/accounts/{aid}/promoted_tweets/{promoted_tweet_id}", headers=headers, timeout=timeout, verify=verify_ssl)
