...
async def twitter_ads_get_ad(promoted_tweet_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not promoted_tweet_id:
            return _xa_dataset([], 400, 'promoted_tweet_id is required')
        ...
        if err:
            return _xa_dataset([], 400, err)
        resp = await nexus_call('GET', f'{root}/accounts/{aid}/promoted_tweets/{promoted_tweet_id}', headers=headers)
