...
def twitter_ads_create_ad(auth_info: dict, line_item_id: str, tweet_ids: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not line_item_id or not tweet_ids: return _xa_provision({}, 400, "line_item_id and tweet_ids are required")
        headers, aerr = _xa_auth(auth_info)
        if aerr: return _xa_provision({}, 401, aerr)
        aid, err = _xa_account(auth_info)
        if err: return _xa_provision({}, 400, err)
        ids = ",".join(x.strip() for x in tweet_ids.split(",") if x.strip())
        resp = requests.post(f"{root}/accounts/{aid}/promoted_tweets", headers=headers, params={"line_item_id": line_item_id, "tweet_ids": ids}, timeout=timeout, verify=verify_ssl)
