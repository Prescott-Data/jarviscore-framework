...
async def twitter_ads_create_ad(line_item_id: str, tweet_ids: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not line_item_id or not tweet_ids:
            return _xa_provision({}, 400, 'line_item_id and tweet_ids are required')
        ...
        resp = await nexus_call('POST', f'{root}/accounts/{aid}/promoted_tweets', headers=headers, params={'line_item_id': line_item_id, 'tweet_ids': ids})
