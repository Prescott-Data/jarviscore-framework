...
async def twitter_ads_update_campaign(campaign_id: str, name: str='', timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not campaign_id:
            return _xa_provision({}, 400, 'campaign_id is required')
        ...
        if err:
            return _xa_provision({}, 400, err)
        resp = await nexus_call('PUT', f'{root}/accounts/{aid}/campaigns/{campaign_id}', headers=headers, params={'name': name} if name else {})
