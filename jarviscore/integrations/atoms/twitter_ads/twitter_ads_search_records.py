...
def twitter_ads_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """X Ads API: search records. Official: https://developer.x.com/en/docs/twitter-ads-api"""
    try:
        root, err = _xa_root(base_url, auth_info)
        if err: return _xa_dataset([], 400, err)
        if not query: return _xa_dataset([], 400, "query is required")
        headers, aerr = _xa_auth(auth_info)
        if aerr: return _xa_dataset([], 401, aerr)
        aid, err = _xa_account(auth_info)
        if err: return _xa_dataset([], 400, err)
        resp = requests.get(f"{root}/accounts/{aid}/campaigns", headers=headers, params={"count": 100}, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _xa_dataset([], resp.status_code, _xa_err(resp))
        rows = _xa_rows(resp.json() if resp.content else {})
        q = query.lower()
        matched = [r for r in rows if q in str(r.get("name","")).lower() or q in str(r.get("id","")).lower()]
        return _xa_dataset(matched[:limit], resp.status_code, "ok")
