...
def twitter_ads_update_ad(auth_info: dict, promoted_tweet_id: str, entity_status: str = "PAUSED", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not promoted_tweet_id: return _xa_provision({}, 400, "promoted_tweet_id is required")
        headers, aerr = _xa_auth(auth_info)
        if aerr: return _xa_provision({}, 401, aerr)
        aid, err = _xa_account(auth_info)
        if err: return _xa_provision({}, 400, err)
        # X Ads promoted_tweets have NO update endpoint; deactivating ("pausing") an ad is done by disassociating (DELETE) the promoted tweet.
        resp = requests.delete(f"{root}/accounts/{aid}/promoted_tweets/{promoted_tweet_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _xa_provision({}, resp.status_code, _xa_err(resp))
        data = resp.json() if resp.content else {}
        return _xa_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok", fallback_id=promoted_tweet_id)
