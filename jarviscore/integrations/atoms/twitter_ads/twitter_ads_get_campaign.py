...
def twitter_ads_get_campaign(auth_info: dict, campaign_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not campaign_id: return _xa_dataset([], 400, "campaign_id is required")
        headers, aerr = _xa_auth(auth_info)
        if aerr: return _xa_dataset([], 401, aerr)
        aid, err = _xa_account(auth_info)
        if err: return _xa_dataset([], 400, err)
        resp = requests.get(f"{root}/accounts/{aid}/campaigns/{campaign_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _xa_dataset([], resp.status_code, _xa_err(resp))
        data = resp.json() if resp.content else {}
        rows = _xa_rows(data)
        return _xa_dataset(rows[:1], resp.status_code, "ok")
