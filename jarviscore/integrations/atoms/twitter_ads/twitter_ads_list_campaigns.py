...
def twitter_ads_list_campaigns(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """X Ads API: list campaigns. Official: https://developer.x.com/en/docs/twitter-ads-api"""
    try:
        ...
        if err: return _xa_dataset([], 400, err)
        resp = requests.get(f"{root}/accounts/{aid}/campaigns", headers=headers, params={"count": limit}, timeout=timeout, verify=verify_ssl)
