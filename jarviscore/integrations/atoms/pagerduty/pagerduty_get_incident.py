...
def pagerduty_get_incident(auth_info: dict, incident_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get incident by id. Official: https://developer.pagerduty.com/api-reference/operations/getIncident"""
    try:
        root, err = _pd_root(base_url, auth_info)
        if err:
            return _pd_dataset([], 400, err)
        if not incident_id:
            return _pd_dataset([], 400, "incident_id is required")
        resp, status, err = _pd_request("get", root + f"/incidents/{incident_id}", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if err:
        ...
        return _pd_dataset(_pd_single(data, "incident"), status, "ok")
