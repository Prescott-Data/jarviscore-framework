...
async def pagerduty_get_incident(incident_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        (resp, status, err) = await _pd_request('get', root + f'/incidents/{incident_id}', timeout=timeout, verify_ssl=verify_ssl)
        if err:
