...
def pagerduty_get_user(auth_info: dict, user_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not user_id:
            return _pd_dataset([], 400, "user_id is required")
        resp, status, err = _pd_request("get", root + f"/users/{user_id}", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if err:
        ...
        return _pd_dataset(_pd_single(data, "user"), status, "ok")
