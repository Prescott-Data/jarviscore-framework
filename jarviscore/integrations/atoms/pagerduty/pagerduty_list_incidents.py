...
def pagerduty_list_incidents(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List incidents with limit/offset pagination (more). Official: https://developer.pagerduty.com/api-reference/operations/listIncidents"""
    try:
        root, err = _pd_root(base_url, auth_info)
        if err:
            return _pd_dataset([], 400, err)
        records, status, msg = _pd_paginate(root + "/incidents", auth_info, {}, limit, timeout, verify_ssl, "incidents")
