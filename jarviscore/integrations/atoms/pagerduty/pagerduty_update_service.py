...
def pagerduty_update_service(auth_info: dict, service_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not service_id:
            return _pd_provision({}, "service", 400, "service_id is required")
        if not isinstance(payload, dict) or not payload:
            return _pd_provision({}, "service", 400, "payload is required")
        body = _pd_wrap("service", payload)
        resp, status, err = _pd_request("put", root + f"/services/{service_id}", auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
        if err:
        ...
        return _pd_provision(data, "service", status, "ok", fallback_id=service_id)
