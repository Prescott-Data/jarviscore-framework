...
def pagerduty_create_incident(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, status, err = _pd_request("post", root + "/incidents", auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl, from_header=True)
        if err:
        ...
        return _pd_provision(data, "incident", status, "ok")
