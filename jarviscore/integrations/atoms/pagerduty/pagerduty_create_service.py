...
def pagerduty_create_service(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create service (wrapped service body). Official: https://developer.pagerduty.com/api-reference/operations/createService"""
    try:
        ...
        resp, status, err = _pd_request("post", root + "/services", auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
        if err:
        ...
        try:
            ...
            data = {}
        if status >= 400:
            return _pd_provision(data, "service", status, _pd_err(resp))
        return _pd_provision(data, "service", status, "ok")

    except Exception as e:
        return _pd_provision({}, "service", 500, str(e))
