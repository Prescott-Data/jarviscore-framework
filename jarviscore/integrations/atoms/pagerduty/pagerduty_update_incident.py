...
async def pagerduty_update_incident(incident_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        (resp, status, err) = await _pd_request('put', root + f'/incidents/{incident_id}', json_body=body, timeout=timeout, verify_ssl=verify_ssl, from_header=True)
        if err:
