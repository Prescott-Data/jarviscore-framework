...
def pagerduty_update_incident(auth_info: dict, incident_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update incident via PUT (acknowledge/resolve/reassign; From header required). Official: https://developer.pagerduty.com/api-reference/operations/updateIncident"""
    try:
        root, err = _pd_root(base_url, auth_info)
        if err:
            return _pd_provision({}, "incident", 400, err)
        if not incident_id:
            return _pd_provision({}, "incident", 400, "incident_id is required")
        if not isinstance(payload, dict) or not payload:
            return _pd_provision({}, "incident", 400, "payload is required")
        body = _pd_wrap("incident", payload)
        resp, status, err = _pd_request("put", root + f"/incidents/{incident_id}", auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl, from_header=True)
        if err:
            return _pd_provision({}, "incident", 401, err)
        try:
            data = resp.json() if resp.content else {}
        except Exception:
            data = {}
        if status >= 400:
            return _pd_provision(data, "incident", status, _pd_err(resp))
        return _pd_provision(data, "incident", status, "ok", fallback_id=incident_id)

    except Exception as e:
        return _pd_provision({}, "incident", 500, str(e))
