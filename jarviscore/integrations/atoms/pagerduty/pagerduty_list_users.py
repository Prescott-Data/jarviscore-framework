...
def pagerduty_list_users(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List users with limit/offset pagination (more). Official: https://developer.pagerduty.com/api-reference/operations/listUsers"""
    try:
        root, err = _pd_root(base_url, auth_info)
        if err:
            return _pd_dataset([], 400, err)
        records, status, msg = _pd_paginate(root + "/users", auth_info, {}, limit, timeout, verify_ssl, "users")
