...
def pagerduty_update_user(auth_info: dict, user_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, status, err = _pd_request("put", root + f"/users/{user_id}", auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
        if err:
