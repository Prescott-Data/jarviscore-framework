...
def klaviyo_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search Klaviyo metrics by name filter. Official: https://developers.klaviyo.com/en/reference/get_metrics"""
    try:
        ...
        params = {"filter": filt, "page[size]": min(max(limit,1),200)} if filt else {"page[size]": min(max(limit,1),200)}
        resp = _kv_get(f"{api}/api/metrics/", headers, params, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        records = _kv_jsonapi_records(resp.json() if resp.text else {})[:limit]
