...
# Klaviyo API — https://developers.klaviyo.com/en/reference/get_profiles
KLAVIYO_API = "https://a.klaviyo.com"
KL_REVISION = "2024-10-15"


def klaviyo_list_profiles(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        records, status, message = _kv_paginate(f"{api}/api/profiles/", headers, limit, timeout, verify_ssl)
...
def _kv_paginate(url, headers, limit, timeout, verify_ssl):
    ...
    while len(records) < cap and next_url and pages < 100:
        pages += 1
        resp = _kv_get(next_url, headers, None if pages > 1 else {"page[size]": min(cap - len(records), 100)}, timeout, verify_ssl)
