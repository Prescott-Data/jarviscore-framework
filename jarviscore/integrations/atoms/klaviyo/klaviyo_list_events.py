...
async def klaviyo_list_events(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List Klaviyo events. Official: https://developers.klaviyo.com/en/reference/get_events"""
    try:
        ...
        (records, status, message) = await _kv_paginate(f'{api}/api/events/', headers, limit, timeout, verify_ssl)
...
async def _kv_paginate(url, headers, limit, timeout, verify_ssl):
    records = []
    status = 0
    ...
    while len(records) < cap and next_url and (pages < 100):
        pages += 1
        resp = await _kv_get(next_url, headers, None if pages > 1 else {'page[size]': min(cap - len(records), 200)}, timeout, verify_ssl)
