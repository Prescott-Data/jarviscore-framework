...
def klaviyo_get_report(auth_info: dict, report_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get Klaviyo metric by ID. Official: https://developers.klaviyo.com/en/reference/get_metrics"""
    try:
        if not report_id: return {"records": [], "data_count": 0, "status": 400, "message": "report_id is required"}
        ...
        resp = _kv_get(f"{api}/api/metrics/{report_id}/", headers, None, timeout, verify_ssl)
