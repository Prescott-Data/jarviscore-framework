...
def klaviyo_get_event(auth_info: dict, event_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get Klaviyo event by ID. Official: https://developers.klaviyo.com/en/reference/get_events"""
    try:
        if not event_id: return {"records": [], "data_count": 0, "status": 400, "message": "event_id is required"}
        ...
        resp = _kv_get(f"{api}/api/events/{event_id}/", headers, None, timeout, verify_ssl)
