...
# Klaviyo API — https://developers.klaviyo.com/en/reference/update_profile
KLAVIYO_API = "https://a.klaviyo.com"
KL_REVISION = "2024-10-15"


def klaviyo_update_profile(auth_info: dict, profile_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if isinstance(payload.get("data"), dict):
            ...
            body = {"data": {"type": "profile", "id": profile_id, "attributes": payload}}
        resp = _kv_patch(f"{api}/api/profiles/{profile_id}/", headers, body, timeout, verify_ssl)
        ...
        return {"records": records, "data_count": len(records), "status": resp.status_code, "message": "ok", "provision_ids": _kv_provision_id(data) or [profile_id]}
...
def _kv_patch(url, headers, body, timeout, verify_ssl):
