...
def travis_ci_update_build(auth_info: dict, build_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not build_id: return _tr_provision({}, 400, "build_id is required")
        headers, aerr = _tr_auth(auth_info)
        if aerr: return _tr_provision({}, 401, aerr)
        resp = requests.post(f"{root}/build/{build_id}/cancel", headers=headers, timeout=timeout, verify=verify_ssl)
