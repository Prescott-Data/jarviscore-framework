...
def travis_ci_create_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Travis CI API v3: create project. Official: https://developer.travis-ci.com/resource"""
    try:
        root, err = _tr_root(base_url, auth_info)
        if err: return _tr_provision({}, 400, err)
        if not project_id: return _tr_provision({}, 400, "project_id slug is required")
        headers, aerr = _tr_auth(auth_info)
        if aerr: return _tr_provision({}, 401, aerr)
        slug = project_id.replace("/", "%2F")
        resp = requests.post(f"{root}/repo/{slug}/activate", headers=headers, timeout=timeout, verify=verify_ssl)
