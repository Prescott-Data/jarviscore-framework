...
async def travis_ci_create_project(project_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not project_id:
            return _tr_provision({}, 400, 'project_id slug is required')
        (headers, aerr) = _tr_auth()
        if aerr:
            return _tr_provision({}, 401, aerr)
        slug = project_id.replace('/', '%2F')
        resp = await nexus_call('POST', f'{root}/repo/{slug}/activate', headers=headers)
