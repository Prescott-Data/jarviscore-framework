...
def travis_ci_get_build(auth_info: dict, build_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not build_id: return _tr_dataset([], 400, "build_id is required")
        headers, aerr = _tr_auth(auth_info)
        if aerr: return _tr_dataset([], 401, aerr)
        resp = requests.get(f"{root}/build/{build_id}", headers=headers, timeout=timeout, verify=verify_ssl)
