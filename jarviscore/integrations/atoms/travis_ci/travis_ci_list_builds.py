...
def travis_ci_list_builds(auth_info: dict, project_id: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Travis CI API v3: list builds. Official: https://developer.travis-ci.com/resource"""
    try:
        ...
        path = f"/repo/{project_id.replace('/', '%2F')}/builds" if project_id else "/builds"
        resp = requests.get(root + path, headers=headers, params={"limit": limit}, timeout=timeout, verify=verify_ssl)
        ...
        return _tr_dataset(builds if isinstance(builds, list) else [], resp.status_code, "ok")
