...
async def travis_ci_get_project(project_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not project_id:
            return _tr_dataset([], 400, 'project_id (slug) is required')
        headers, aerr = _tr_auth()
        if aerr:
            return _tr_dataset([], 401, aerr)
        slug = project_id.replace('/', '%2F')
        resp = await nexus_call('GET', f'{root}/repo/{slug}', headers=headers)
