async def netsuite_get_record(account_id: str, record_type: str, record_id: str) -> dict:
    """Get record via the netsuite API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    return await _get(f'/{record_type}/{record_id}')
