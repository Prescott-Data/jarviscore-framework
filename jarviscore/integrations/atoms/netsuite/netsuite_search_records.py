async def netsuite_search_records(account_id: str, record_type: str, query: str, limit: int=25) -> dict:
    """Search records via the netsuite API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get(f'/{record_type}', params={'q': query, 'limit': limit})
    return {'items': resp.get('items', []), 'total': resp.get('totalResults'), 'count': resp.get('count'), 'has_more': resp.get('hasMore', False)}
