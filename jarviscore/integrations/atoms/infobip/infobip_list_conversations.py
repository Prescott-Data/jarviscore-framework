...
def infobip_list_conversations(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = _ib_get(f"{api}/ccaas/1/conversations", headers, {"limit": min(max(limit, 1), 100)}, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        records = _ib_conversations(resp.json() if resp.text else {})[:limit]
