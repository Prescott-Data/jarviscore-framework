...
def infobip_list_messages(auth_info: dict, conversation_id: Optional[str] = None, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        cid = _ib_conv_id(auth_info, None, conversation_id)
        if not cid:
        ...
        resp = _ib_get(f"{api}/ccaas/1/conversations/{cid}/messages", headers, params, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        records = _ib_messages(resp.json() if resp.text else {})[:limit]
