...
def infobip_get_message(auth_info: dict, message_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        cid = _ib_conv_id(auth_info, None, None)
        if not cid:
        ...
        resp = _ib_get(f"{api}/ccaas/1/conversations/{cid}/messages", headers, {"messageIds": message_id}, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        records = [m for m in _ib_messages(resp.json() if resp.text else {}) if str(m.get("id")) == str(message_id)]
