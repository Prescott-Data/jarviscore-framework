...
def infobip_create_message(auth_info: dict, payload: Dict[str, Any], conversation_id: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        cid = _ib_conv_id(auth_info, payload, conversation_id)
        if not cid:
            return {"records": [], "data_count": 0, "status": 400, "message": "conversation_id is required"}
        api, err = _ib_api_root(base_url)
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        headers, auth_err = _ib_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err, "provision_ids": []}
        agent_id = (auth_info or {}).get("agent_id") or (auth_info or {}).get("agentId")
        if agent_id:
            headers["x-agent-id"] = str(agent_id)
        resp = _ib_post(f"{api}/ccaas/1/conversations/{cid}/messages", headers, payload, timeout, verify_ssl)
