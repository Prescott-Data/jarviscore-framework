...
async def kissmetrics_run_report(report_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp = await _km_post(f'{api}/queries/report', headers, basic, body, timeout, verify_ssl)
        if resp['status_code'] >= 400:
            return {'records': [], 'data_count': 0, 'status': resp['status_code'], 'message': resp['body'][:1000]}
        records = _km_records(resp['json'] if resp['body'] else {})
...
async def _km_post(url, headers, basic, body, timeout, verify_ssl):
