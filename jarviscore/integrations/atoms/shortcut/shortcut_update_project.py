...
async def shortcut_update_project(project_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Shortcut API v3: update project. Official: https://developer.shortcut.com/api/rest/v3#Projects/updateProject"""
    try:
        if not project_id:
            return _sc_provision({}, 400, 'project_id is required')
        ...
        resp = await nexus_call('PUT', root + '/projects/' + str(project_id), headers=headers, json=payload)
        try:
