...
def shortcut_get_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shortcut API v3: get project. Official: https://developer.shortcut.com/api/rest/v3#Projects/getProject"""
    try:
        if not project_id: return _sc_dataset([], 400, "project_id is required")
        root, _ = _sc_root(base_url, auth_info)
        headers, err = _sc_auth(auth_info)
        if err: return _sc_dataset([], 401, err)
        resp = requests.get(root + "/projects/" + str(project_id), headers=headers, timeout=timeout, verify=verify_ssl)
        try: data = resp.json() if resp.content else {}
        except Exception: data = {}
        if resp.status_code >= 400: return _sc_dataset([], resp.status_code, _sc_err(resp))
        return _sc_dataset([data] if isinstance(data, dict) else [], resp.status_code, "ok")
