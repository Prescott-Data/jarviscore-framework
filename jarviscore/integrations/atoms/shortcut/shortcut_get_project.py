...
async def shortcut_get_project(project_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Shortcut API v3: get project. Official: https://developer.shortcut.com/api/rest/v3#Projects/getProject"""
    try:
        if not project_id:
            return _sc_dataset([], 400, 'project_id is required')
        root, _ = _sc_root(base_url)
        headers, err = _sc_auth()
        if err:
            return _sc_dataset([], 401, err)
        resp = await nexus_call('GET', root + '/projects/' + str(project_id), headers=headers)
        try:
