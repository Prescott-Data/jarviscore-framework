...
def shortcut_update_task(auth_info: dict, task_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shortcut API v3: update task. Official: https://developer.shortcut.com/api/rest/v3#Stories/updateStory"""
    try:
        if not task_id: return _sc_provision({}, 400, "task_id is required")
        ...
        resp = requests.put(root + "/stories/" + str(task_id), headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
