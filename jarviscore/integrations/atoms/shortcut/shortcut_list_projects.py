...
def shortcut_list_projects(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shortcut API v3: list projects. Official: https://developer.shortcut.com/api/rest/v3#Projects/listProjects"""
    try:
        root, _ = _sc_root(base_url, auth_info)
        headers, err = _sc_auth(auth_info)
        if err: return _sc_dataset([], 401, err)
        resp = requests.get(root + "/projects", headers=headers, timeout=timeout, verify=verify_ssl)
        try: data = resp.json() if resp.content else []
        except Exception: data = []
        if resp.status_code >= 400: return _sc_dataset([], resp.status_code, _sc_err(resp))
        records = [x for x in (data if isinstance(data, list) else []) if isinstance(x, dict)][:_sc_cap(limit)]
