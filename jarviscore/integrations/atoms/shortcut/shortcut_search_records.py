...
def shortcut_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        cap = _sc_cap(limit)
        # Shortcut v3 free-text search: GET /search/stories?query= (SBQL) -> {data,next,total}.
        resp = requests.get(root + "/search/stories", headers=headers, params={"query": query, "page_size": cap}, timeout=timeout, verify=verify_ssl)
