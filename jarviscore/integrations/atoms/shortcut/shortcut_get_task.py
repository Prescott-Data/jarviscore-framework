...
def shortcut_get_task(auth_info: dict, task_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shortcut API v3: get task. Official: https://developer.shortcut.com/api/rest/v3#Stories/getStory"""
    try:
        if not task_id: return _sc_dataset([], 400, "task_id is required")
        root, _ = _sc_root(base_url, auth_info)
        headers, err = _sc_auth(auth_info)
        if err: return _sc_dataset([], 401, err)
        resp = requests.get(root + "/stories/" + str(task_id), headers=headers, timeout=timeout, verify=verify_ssl)
