...
async def paystack_create_customer(payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        (resp, data, status, err) = await _ps_request('post', root + '/customer', json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if err:
