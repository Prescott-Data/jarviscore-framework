...
def paystack_create_customer(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create customer. Official: https://paystack.com/docs/api/customer/#create-customer"""
    try:
        ...
        resp, data, status, err = _ps_request("post", root + "/customer", auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if err:
            return _ps_provision({}, 401, err)
        if not _ps_ok(resp, data):
            return _ps_provision(data, status, _ps_err(resp))
        return _ps_provision(data, status, "ok")
