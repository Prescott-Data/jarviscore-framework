...
def paystack_get_transaction(auth_info: dict, transaction_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, data, status, err = _ps_request("get", root + f"/transaction/{transaction_id}", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if err:
