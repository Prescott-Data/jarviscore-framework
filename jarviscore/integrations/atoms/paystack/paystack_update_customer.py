...
def paystack_update_customer(auth_info: dict, customer_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update customer by customer code via PUT. Official: https://paystack.com/docs/api/customer/#update-customer"""
    try:
        root, err = _ps_root(base_url, auth_info)
        if err:
            return _ps_provision({}, 400, err)
        if not customer_id:
            return _ps_provision({}, 400, "customer_id is required")
        if not isinstance(payload, dict) or not payload:
            return _ps_provision({}, 400, "payload is required")
        resp, data, status, err = _ps_request("put", root + f"/customer/{customer_id}", auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if err:
            return _ps_provision({}, 401, err)
        if not _ps_ok(resp, data):
            return _ps_provision(data, status, _ps_err(resp))
        return _ps_provision(data, status, "ok", fallback_id=customer_id)
