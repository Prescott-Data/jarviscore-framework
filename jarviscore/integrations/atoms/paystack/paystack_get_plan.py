...
def paystack_get_plan(auth_info: dict, plan_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, data, status, err = _ps_request("get", root + f"/plan/{plan_id}", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if err:
