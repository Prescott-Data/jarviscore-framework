...
def paystack_list_plans(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List plans with page/perPage pagination. Official: https://paystack.com/docs/api/plan/#list-plans"""
    try:
        root, err = _ps_root(base_url, auth_info)
        if err:
            return _ps_dataset([], 400, err)
        records, status, msg = _ps_paginate(root + "/plan", auth_info, {}, limit, timeout, verify_ssl)
