...
def paystack_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resource = str(auth_info.get("resource") or "customer").lower()
        cap = _ps_cap(limit)
        if resource == "transaction":
            ...
            for key, param in (("customer", "customer"), ("status", "status"), ("from", "from"), ("to", "to"), ("amount", "amount")):
                ...
                if val not in (None, ""):
                    params[param] = val
            records, status, msg = _ps_paginate(root + "/transaction", auth_info, params, limit, timeout, verify_ssl)
            if query:
                q = query.lower()
                records = [r for r in records if q in str(r.get("reference", "")).lower() or q == str(r.get("id", ""))]
            return _ps_dataset(records[:cap], status, msg)
        if query:
            resp, data, status, err = _ps_request("get", root + f"/customer/{query}", auth_info, timeout=timeout, verify_ssl=verify_ssl)
            if err:
                return _ps_dataset([], 401, err)
            if _ps_ok(resp, data):
                return _ps_dataset(_ps_items(data), status, "ok")
        records, status, msg = _ps_paginate(root + "/customer", auth_info, {}, limit, timeout, verify_ssl)
        if query:
...
def _ps_match(record, query):
    q = query.lower()
    for key in ("email", "customer_code", "first_name", "last_name", "phone"):
        val = record.get(key)
