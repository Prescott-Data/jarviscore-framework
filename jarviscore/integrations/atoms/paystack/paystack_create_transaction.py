...
def paystack_create_transaction(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, data, status, err = _ps_request("post", root + "/transaction/initialize", auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if err:
