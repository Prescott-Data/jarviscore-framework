...
def paystack_get_customer(auth_info: dict, customer_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Fetch customer by email or customer code. Official: https://paystack.com/docs/api/customer/#fetch-customer"""
    try:
        ...
        resp, data, status, err = _ps_request("get", root + f"/customer/{customer_id}", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if err:
