...
def perforce_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if query and auth_info.get("workflow"):
            params["workflow"] = auth_info.get("workflow")
        resp, data, status, err = _pf_request("get", root + "/projects", auth_info, params=params, timeout=timeout, verify_ssl=verify_ssl)
        if err:
            return _pf_dataset([], 401, err)
        if status >= 400:
            return _pf_dataset([], status, _pf_err(resp))
        records = _pf_projects(data)
        if query:
            records = [r for r in records if _pf_match(r, query)]
        return _pf_dataset(records[: _pf_cap(limit)], status, "ok")
