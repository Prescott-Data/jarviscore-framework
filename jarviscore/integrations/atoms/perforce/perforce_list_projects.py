...
def perforce_list_projects(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        workflow = auth_info.get("workflow")
        if workflow:
        ...
        records = _pf_projects(data)[: _pf_cap(limit)]
        return _pf_dataset(records, status, "ok")
