...
def perforce_create_project(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, data, status, err = _pf_request("post", root + "/projects", auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if err:
