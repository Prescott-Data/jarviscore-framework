...
def perforce_update_project(auth_info: dict, project_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, data, status, err = _pf_request("patch", root + f"/projects/{project_id}", auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if err:
            return _pf_provision({}, 401, err)
        if status >= 400:
            return _pf_provision(data if isinstance(data, dict) else {}, status, _pf_err(resp))
        return _pf_provision(data if isinstance(data, dict) else {}, status, "ok", fallback_id=project_id)
