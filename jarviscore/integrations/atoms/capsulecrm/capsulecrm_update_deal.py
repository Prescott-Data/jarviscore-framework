...
def capsulecrm_update_deal(auth_info: dict, deal_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update Deal via Capsule CRM API. Official: https://developer.capsulecrm.com/v2/operations/Opportunity"""
    try:
        if not deal_id or not payload:
            return {"records": [], "data_count": 0, "status": 400, "message": "deal_id and payload are required"}
        api = _capsule_api_root(base_url)
        headers, auth_err = _capsule_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        body = _capsule_wrap("opportunity", payload)
        resp = _capsule_put_json(f"{api}/opportunities/{deal_id}", headers, body, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        data = resp.json() if resp.text else {}
        records = _capsule_records_from(data, "opportunities", "opportunity")
        return {"records": records, "data_count": len(records), "status": resp.status_code, "message": "ok", "provision_ids": [deal_id]}
