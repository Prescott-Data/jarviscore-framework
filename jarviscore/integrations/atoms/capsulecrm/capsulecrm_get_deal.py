...
def capsulecrm_get_deal(auth_info: dict, deal_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get Deal via Capsule CRM API. Official: https://developer.capsulecrm.com/v2/operations/Opportunity"""
    try:
        if not deal_id:
            return {"records": [], "data_count": 0, "status": 400, "message": "deal_id is required"}
        api = _capsule_api_root(base_url)
        headers, auth_err = _capsule_auth(auth_info)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        resp = _capsule_get(f"{api}/opportunities/{deal_id}", headers, None, timeout, verify_ssl)
