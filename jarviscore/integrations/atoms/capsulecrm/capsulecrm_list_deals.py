...
def capsulecrm_list_deals(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List Deals via Capsule CRM API. Official: https://developer.capsulecrm.com/v2/operations/Opportunity"""
    try:
        api = _capsule_api_root(base_url)
        headers, auth_err = _capsule_auth(auth_info)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        records, status, message = _capsule_paginate(f"{api}/opportunities", headers, limit, timeout, verify_ssl)
