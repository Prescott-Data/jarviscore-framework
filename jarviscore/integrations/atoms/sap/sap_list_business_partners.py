async def sap_list_business_partners(account_id: str, top: int=25, filter: str=None) -> list:
    """List business partners via the sap API."""
    ...
    resp = await _get('/sap/opu/odata/sap/API_BUSINESS_PARTNER/A_BusinessPartner', params=params, headers={'Accept': 'application/json'})
    results = resp.get('d', {}).get('results', [])
    return [{'id': p.get('BusinessPartner'), 'name': p.get('BusinessPartnerFullName'), 'category': p.get('BusinessPartnerCategory'), 'type': p.get('BusinessPartnerType'), 'group': p.get('BusinessPartnerGrouping'), 'created_at': p.get('CreationDate')} for p in results]
