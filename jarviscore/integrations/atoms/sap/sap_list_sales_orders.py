async def sap_list_sales_orders(account_id: str, top: int=25, filter: str=None) -> list:
    """List sales orders via the sap API."""
    ...
    resp = await _get('/sap/opu/odata/sap/API_SALES_ORDER_SRV/A_SalesOrder', params=params, headers={'Accept': 'application/json'})
    results = resp.get('d', {}).get('results', [])
    return [{'id': o.get('SalesOrder'), 'type': o.get('SalesOrderType'), 'customer': o.get('SoldToParty'), 'currency': o.get('TransactionCurrency'), 'net_amount': o.get('TotalNetAmount'), 'date': o.get('SalesOrderDate'), 'delivery_date': o.get('RequestedDeliveryDate'), 'status': o.get('OverallSDProcessStatus')} for o in results]
