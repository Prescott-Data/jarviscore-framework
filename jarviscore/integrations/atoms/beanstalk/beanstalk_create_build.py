...
def beanstalk_create_build(auth_info: dict, repository_id: str, environment_id: str, account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, payload: Optional[Dict[str, Any]] = None, base_url: str = None) -> dict:
    """Create release (trigger deployment). POST /api/{repo_id}/releases.json. Official: https://api.beanstalkapp.com/release"""
    try:
        if not repository_id or not environment_id: return {"records": [], "data_count": 0, "status": 400, "message": "repository_id and environment_id are required"}
        ...
        if "release" not in body: body = {"release": body}
        resp = _beanstalk_post(api_root, f"/{repository_id}/releases.json", headers, basic, body, {"environment_id": environment_id}, timeout, verify_ssl)
