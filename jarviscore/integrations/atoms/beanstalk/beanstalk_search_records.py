...
async def beanstalk_search_records(query: str, account: Optional[str]=None, timeout: int=30, verify_ssl: bool=True, limit: int=25, base_url: str=None) -> dict:
    """Client-side search over repositories list. GET /api/repositories.json. Official: https://api.beanstalkapp.com/repository"""
    try:
        if not query:
        ...
        for item in all_records:
            if not isinstance(item, dict):
                continue
            hay = ' '.join((str(item.get(k, '')) for k in ('title', 'name', 'description'))).lower()
            if q in hay:
                records.append(item)
