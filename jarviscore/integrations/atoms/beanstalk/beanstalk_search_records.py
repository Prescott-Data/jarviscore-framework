...
def beanstalk_search_records(auth_info: dict, query: str, account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, limit: int = 25, base_url: str = None) -> dict:
    ...
    try:
        ...
        all_records, status, message = _beanstalk_paginate(api_root, "/repositories.json", headers, basic, max(limit * 3, 50), timeout, verify_ssl)
        if message != "ok": return {"records": [], "data_count": 0, "status": status, "message": message}
        q = query.lower()
        records = []
