...
def beanstalk_update_build(auth_info: dict, build_id: str, repository_id: str, account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Retry failed release. PUT /api/{repo_id}/releases/{release_id}/retry.json. Official: https://api.beanstalkapp.com/release"""
    try:
        if not build_id or not repository_id: return {"records": [], "data_count": 0, "status": 400, "message": "build_id and repository_id are required"}
        ...
        resp = _beanstalk_put(api_root, f"/{repository_id}/releases/{build_id}/retry.json", headers, basic, {}, None, timeout, verify_ssl)
