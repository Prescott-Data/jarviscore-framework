...
def beanstalk_create_pipeline(auth_info: dict, repository_id: str, payload: Dict[str, Any], account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        if not repository_id or not payload: return {"records": [], "data_count": 0, "status": 400, "message": "repository_id and payload are required"}
        api_root, err = _beanstalk_api_root(base_url, account)
        if err: return {"records": [], "data_count": 0, "status": 400, "message": err}
        headers, basic, auth_err = _beanstalk_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err, "provision_ids": []}
        body = payload if "server_environment" in payload else {"server_environment": payload}
        resp = _beanstalk_post(api_root, f"/{repository_id}/server_environments.json", headers, basic, body, None, timeout, verify_ssl)
        ...
        prov = [env.get("id")] if isinstance(env, dict) and env.get("id") is not None else []
        return {"records": [env] if env else [], "data_count": 1 if env else 0, "status": resp.status_code, "message": "ok", "provision_ids": prov}
