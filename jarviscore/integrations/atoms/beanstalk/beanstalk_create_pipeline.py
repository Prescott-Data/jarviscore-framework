...
async def beanstalk_create_pipeline(repository_id: str, payload: Dict[str, Any], account: Optional[str]=None, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        if not repository_id or not payload:
            return {'records': [], 'data_count': 0, 'status': 400, 'message': 'repository_id and payload are required'}
        ...
        resp = await _beanstalk_post(api_root, f'/{repository_id}/server_environments.json', headers, basic, body, None, timeout, verify_ssl)
