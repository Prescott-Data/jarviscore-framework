...
def beanstalk_update_project(auth_info: dict, project_id: str, payload: Dict[str, Any], account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update repository. PUT /api/repositories/{id}.json. Official: https://api.beanstalkapp.com/repository"""
    try:
        if not project_id or not payload: return {"records": [], "data_count": 0, "status": 400, "message": "project_id and payload are required"}
        ...
        resp = _beanstalk_put(api_root, f"/repositories/{project_id}.json", headers, basic, body, None, timeout, verify_ssl)
        ...
        return {"records": [repo] if repo else [], "data_count": 1 if repo else 0, "status": resp.status_code, "message": "ok", "provision_ids": [project_id]}
