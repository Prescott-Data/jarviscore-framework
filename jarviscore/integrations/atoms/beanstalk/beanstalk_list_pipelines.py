...
def beanstalk_list_pipelines(auth_info: dict, repository_id: str, account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, limit: int = 25, base_url: str = None) -> dict:
    """List server environments (pipelines). GET /api/{repository_id}/server_environments.json. Official: https://api.beanstalkapp.com/server_environment"""
    try:
        if not repository_id: return {"records": [], "data_count": 0, "status": 400, "message": "repository_id is required"}
        ...
        records, status, message = _beanstalk_paginate(api_root, f"/{repository_id}/server_environments.json", headers, basic, limit, timeout, verify_ssl)
