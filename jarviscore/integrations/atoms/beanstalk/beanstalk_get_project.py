...
def beanstalk_get_project(auth_info: dict, project_id: str, account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get repository by ID. GET /api/repositories/{id}.json. Official: https://api.beanstalkapp.com/repository"""
    try:
        if not project_id: return {"records": [], "data_count": 0, "status": 400, "message": "project_id is required"}
        ...
        resp = _beanstalk_get(api_root, f"/repositories/{project_id}.json", headers, basic, None, timeout, verify_ssl)
        if resp.status_code >= 400: return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        data = resp.json() if resp.text else {}
        record = data.get("repository") if isinstance(data, dict) else data
