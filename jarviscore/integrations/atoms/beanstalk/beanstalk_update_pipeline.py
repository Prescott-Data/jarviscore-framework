...
async def beanstalk_update_pipeline(pipeline_id: str, repository_id: str, payload: Dict[str, Any], account: Optional[str]=None, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        if not pipeline_id or not repository_id or (not payload):
            return {'records': [], 'data_count': 0, 'status': 400, 'message': 'pipeline_id, repository_id and payload are required'}
        ...
        resp = await _beanstalk_put(api_root, f'/{repository_id}/server_environments/{pipeline_id}.json', headers, basic, body, None, timeout, verify_ssl)
