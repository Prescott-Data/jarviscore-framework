...
async def beanstalk_list_projects(account: Optional[str]=None, timeout: int=30, verify_ssl: bool=True, limit: int=25, base_url: str=None) -> dict:
    """List repositories (Beanstalk projects). GET /api/repositories.json. Official: https://api.beanstalkapp.com/repository"""
    try:
        ...
        (records, status, message) = await _beanstalk_paginate(api_root, '/repositories.json', headers, basic, limit, timeout, verify_ssl)
...
async def _beanstalk_paginate(api_root, path, headers, basic, limit, timeout, verify_ssl, extra=None):
    records = []
    page = 1
    status = 0
    extra = extra or {}
    while len(records) < limit:
        ...
        params.update(extra)
        resp = await _beanstalk_get(api_root, path, headers, basic, params, timeout, verify_ssl)
        status = resp['status_code']
        if status >= 400:
            return (records, status, resp['body'][:1000])
        batch = _beanstalk_unwrap_list(resp['json'] if resp['body'] else [])
        if not batch:
            break
        for item in batch:
            records.append(item)
