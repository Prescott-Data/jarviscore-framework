...
def beanstalk_get_build(auth_info: dict, build_id: str, account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, repository_id: Optional[str] = None, base_url: str = None) -> dict:
    """Get release by ID. GET /api/{repo_id}/releases/{release_id}.json. Official: https://api.beanstalkapp.com/release"""
    try:
        if not build_id: return {"records": [], "data_count": 0, "status": 400, "message": "build_id is required"}
        if not repository_id: return {"records": [], "data_count": 0, "status": 400, "message": "repository_id is required for Beanstalk releases"}
        ...
        resp = _beanstalk_get(api_root, f"/{repository_id}/releases/{build_id}.json", headers, basic, None, timeout, verify_ssl)
        ...
        return {"records": [rel] if rel else [], "data_count": 1 if rel else 0, "status": resp.status_code, "message": "ok"}
