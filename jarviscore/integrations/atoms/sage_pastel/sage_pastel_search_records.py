...
def sage_pastel_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search customers, invoices, receipts. Official: https://accounting.sageone.co.za/api/2.0.0/Help"""
    try:
        if not query:
            return _pt_dataset([], 400, "query is required")
        cap = _pt_cap(limit)
        records = []
        for service, rtype in (("Customer", "customer"), ("TaxInvoice", "tax_invoice"), ("CustomerReceipt", "customer_receipt")):
            if len(records) >= cap:
                break
            chunk, status, msg = _pt_list(service, base_url, auth_info, min(100, cap * 3), timeout, verify_ssl)
            if status >= 400 and not records:
                return _pt_dataset([], status, msg)
            for item in chunk:
                if _pt_match(item, query):
                    rec = dict(item) if isinstance(item, dict) else {"value": item}
                    rec["record_type"] = rtype
                    records.append(rec)
                    if len(records) >= cap:
                        break
        return _pt_dataset(records[:cap], 200, "ok")
