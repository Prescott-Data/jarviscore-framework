...
async def sage_pastel_create_contact(payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = await _pt_save('Customer', base_url, payload, timeout, verify_ssl)
        if status >= 400:
