...
def sage_pastel_create_contact(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create customer. Official: https://accounting.sageone.co.za/api/2.0.0/Help"""
    try:
        if not isinstance(payload, dict) or not payload:
            return _pt_provision({}, 400, "payload is required")
        resp, body, status, msg = _pt_save("Customer", base_url, auth_info, payload, timeout, verify_ssl)
        if status >= 400:
            return _pt_provision(body if isinstance(body, dict) else {}, status, msg)
        obj = body if isinstance(body, dict) else {}
        return _pt_provision(obj, status, "ok")
