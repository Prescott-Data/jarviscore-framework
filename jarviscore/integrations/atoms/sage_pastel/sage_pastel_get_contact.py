...
def sage_pastel_get_contact(auth_info: dict, contact_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get customer by ID. Official: https://accounting.sageone.co.za/api/2.0.0/Help"""
    try:
        if not contact_id:
            return _pt_dataset([], 400, "contact_id is required")
        resp, body, status, msg = _pt_get("Customer", base_url, auth_info, str(contact_id), None, timeout, verify_ssl)
        if status >= 400:
