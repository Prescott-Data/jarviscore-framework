...
def sage_pastel_update_contact(auth_info: dict, contact_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update customer. Official: https://accounting.sageone.co.za/api/2.0.0/Help"""
    try:
        if not contact_id:
            return _pt_provision({}, 400, "contact_id is required")
        ...
        resp, body, status, msg = _pt_save("Customer", base_url, auth_info, body_payload, timeout, verify_ssl)
        if status >= 400:
        ...
        return _pt_provision({}, 500, str(e), contact_id)
