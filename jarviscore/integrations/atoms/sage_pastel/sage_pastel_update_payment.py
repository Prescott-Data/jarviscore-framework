...
def sage_pastel_update_payment(auth_info: dict, payment_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update customer receipt. Official: https://accounting.sageone.co.za/api/2.0.0/Help"""
    try:
        if not payment_id:
            return _pt_provision({}, 400, "payment_id is required")
        if not isinstance(payload, dict) or not payload:
            return _pt_provision({}, 400, "payload is required", payment_id)
        body_payload = dict(payload)
        body_payload["ID"] = int(payment_id) if str(payment_id).isdigit() else payment_id
        extra = {"useSystemDocumentNumber": str((auth_info or {}).get("use_system_document_number", False)).lower()}
        resp, body, status, msg = _pt_save("CustomerReceipt", base_url, auth_info, body_payload, timeout, verify_ssl, extra)
        if status >= 400:
            return _pt_provision(body if isinstance(body, dict) else {}, status, msg, payment_id)
        return _pt_provision(body if isinstance(body, dict) else {}, status, "ok", payment_id)
    except Exception as e:
        return _pt_provision({}, 500, str(e), payment_id)
