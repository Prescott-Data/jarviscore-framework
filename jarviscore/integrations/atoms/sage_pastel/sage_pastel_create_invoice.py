...
def sage_pastel_create_invoice(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        extra = {"useSystemDocumentNumber": str(auth_info.get("use_system_document_number", True)).lower() if auth_info else "true"}
        resp, body, status, msg = _pt_save("TaxInvoice", base_url, auth_info, payload, timeout, verify_ssl, extra)
        if status >= 400:
