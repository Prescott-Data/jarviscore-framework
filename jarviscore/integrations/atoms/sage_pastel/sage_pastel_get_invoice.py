...
def sage_pastel_get_invoice(auth_info: dict, invoice_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get tax invoice. Official: https://accounting.sageone.co.za/api/2.0.0/Help"""
    try:
        if not invoice_id:
            return _pt_dataset([], 400, "invoice_id is required")
        resp, body, status, msg = _pt_get("TaxInvoice", base_url, auth_info, str(invoice_id), None, timeout, verify_ssl)
        if status >= 400:
