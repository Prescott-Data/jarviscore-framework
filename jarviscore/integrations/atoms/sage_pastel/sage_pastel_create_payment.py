...
def sage_pastel_create_payment(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        extra = {"useSystemDocumentNumber": str((auth_info or {}).get("use_system_document_number", True)).lower()}
        resp, body, status, msg = _pt_save("CustomerReceipt", base_url, auth_info, payload, timeout, verify_ssl, extra)
        if status >= 400:
