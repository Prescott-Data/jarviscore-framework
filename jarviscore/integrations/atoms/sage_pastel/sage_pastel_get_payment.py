...
async def sage_pastel_get_payment(payment_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get customer receipt. Official: https://accounting.sageone.co.za/api/2.0.0/Help"""
    try:
        if not payment_id:
            return _pt_dataset([], 400, 'payment_id is required')
        resp, body, status, msg = await _pt_get('CustomerReceipt', base_url, str(payment_id), None, timeout, verify_ssl)
        if status >= 400:
