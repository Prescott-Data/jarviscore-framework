...
def wordpress_create_user(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wordpress REST: create user. Official: https://developer.wordpress.org/rest-api/"""
    try:
        if not isinstance(payload, dict) or not payload: return _provision({}, 400, "payload is required (username, email and password are mandatory)")
        ...
        resp = requests.post(root + '/users', headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
