...
def wordpress_get_page(auth_info: dict, page_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wordpress REST: get page. Official: https://developer.wordpress.org/rest-api/"""
    try:
        root, err = _root(base_url, auth_info)
        if err: return _dataset([], 400, err)
        if not page_id: return _dataset([], 400, 'page_id is required')
        ...
        resp = requests.get(root + "/pages/" + str(page_id), headers=hdrs, auth=auth, timeout=timeout, verify=verify_ssl)
