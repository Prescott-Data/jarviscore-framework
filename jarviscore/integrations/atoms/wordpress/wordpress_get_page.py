...
async def wordpress_get_page(page_id: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not page_id:
            return _dataset([], 400, 'page_id is required')
        headers, aerr = _auth()
        ...
        resp = await nexus_call('GET', root + '/pages/' + str(page_id), headers=hdrs)
