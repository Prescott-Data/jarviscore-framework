...
def wordpress_create_page(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wordpress REST: create page. Official: https://developer.wordpress.org/rest-api/"""
    try:
        if not isinstance(payload, dict) or not payload: return _provision({}, 400, "payload is required (e.g. title, content, status)")
        ...
        resp = requests.post(root + '/pages', headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
