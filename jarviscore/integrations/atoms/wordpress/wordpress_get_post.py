...
async def wordpress_get_post(post_id: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not post_id:
            return _dataset([], 400, 'post_id is required')
        headers, aerr = _auth()
        ...
        resp = await nexus_call('GET', root + '/posts/' + str(post_id), headers=hdrs)
