...
def wordpress_get_post(auth_info: dict, post_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wordpress REST: get post. Official: https://developer.wordpress.org/rest-api/"""
    try:
        root, err = _root(base_url, auth_info)
        if err: return _dataset([], 400, err)
        if not post_id: return _dataset([], 400, 'post_id is required')
        ...
        resp = requests.get(root + "/posts/" + str(post_id), headers=hdrs, auth=auth, timeout=timeout, verify=verify_ssl)
