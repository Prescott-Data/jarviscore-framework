...
def wordpress_update_post(auth_info: dict, post_id: str, title: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wordpress REST: update post. Official: https://developer.wordpress.org/rest-api/"""
    try:
        root, err = _root(base_url, auth_info)
        if err: return _provision({}, 400, err)
        if not post_id: return _provision({}, 400, 'post_id is required')
        ...
        resp = requests.put(root + "/posts/" + str(post_id), headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _provision({}, resp.status_code, _err(resp))
        data = resp.json() if resp.content else {}
        return _provision(data if isinstance(data, dict) else {}, resp.status_code, 'ok', fallback_id=post_id)
