...
async def wordpress_update_post(post_id: str, title: str='', limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not post_id:
            return _provision({}, 400, 'post_id is required')
        (headers, aerr) = _auth()
        ...
        resp = await nexus_call('PUT', root + '/posts/' + str(post_id), headers=hdrs, json=payload)
