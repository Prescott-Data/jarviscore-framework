...
def wordpress_get_user(auth_info: dict, user_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not user_id: return _dataset([], 400, 'user_id is required')
        ...
        resp = requests.get(root + "/users/" + str(user_id), headers=hdrs, auth=auth, timeout=timeout, verify=verify_ssl)
