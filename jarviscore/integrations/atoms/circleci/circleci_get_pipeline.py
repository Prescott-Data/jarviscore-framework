...
def circleci_get_pipeline(auth_info: dict, pipeline_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get Pipeline via CircleCI API v2. Circle-Token auth. Official: https://circleci.com/docs/api/v2/operations/getPipelineById.md"""
    try:
        if not pipeline_id:
        ...
        resp = _circleci_get(f"{api}/pipeline/{pipeline_id}", headers, None, timeout, verify_ssl)
