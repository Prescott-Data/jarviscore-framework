...
def circleci_list_projects(auth_info: dict, timeout: int = 30, verify_ssl: bool = True, limit: int = 25, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = _circleci_get(f"{api}/me/collaborations", headers, None, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        data = resp.json() if resp.text else {}
        records = _circleci_records(data, ())
