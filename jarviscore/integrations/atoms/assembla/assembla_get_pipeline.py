...
def assembla_get_pipeline(auth_info: dict, pipeline_id: str, space_id: Optional[str] = None, space_tool_id: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not sid or not tool_id:
            return {
                ...
                "data_count": 0,
                "status": 400,
                "message": "space_id and space_tool_id are required",
            }
        ...
        url = _assembla_json_path(
            f"{api_root}/spaces/{sid}/space_tools/{tool_id}/merge_requests/{pipeline_id}"
        )
