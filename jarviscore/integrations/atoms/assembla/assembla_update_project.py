...
# PUT /v1/spaces/{id}.json — https://api-docs.assembla.cc/content/ref/spaces_update.html
_ASSEMBLA_V1_SUFFIX = "/v1"


def assembla_update_project(auth_info: dict, project_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update space (PUT /v1/spaces/{id}.json). Official: https://api-docs.assembla.cc/content/ref/spaces_update.html"""
    try:
        if not base_url:
            return _assembla_provision([], 400, "base_url is required")
        if not project_id:
            return _assembla_provision([], 400, "project_id is required")
        ...
        resp = requests.put(
            url,
            headers=headers,
            json=_assembla_wrap_namespace("space", payload),
            timeout=timeout,
            verify=verify_ssl,
        )
        ...
        rec = body if isinstance(body, dict) and body else {"id": project_id}
        records = [rec] if isinstance(rec, dict) else []
        return _assembla_provision(records, status, "ok", _assembla_provision_ids(body, project_id))
