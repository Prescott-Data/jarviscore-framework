...
def telegram_list_updates(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Telegram Bot API: getUpdates. Official: https://core.telegram.org/bots/api"""
    try:
        root, _, err = _tg_root(base_url, auth_info)
        if err: return _tg_dataset([], 401, err)
        resp = requests.get(f"{root}/getUpdates", params={"limit": min(max(int(limit or 25),1),100)}, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400 or not data.get("ok", True): return _tg_dataset([], resp.status_code, _tg_err(resp, data))
        res = data.get("result") or []
        return _tg_dataset(res if isinstance(res, list) else [], resp.status_code, "ok")
