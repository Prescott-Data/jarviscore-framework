...
def telegram_get_message(auth_info: dict, chat_id: str, message_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not chat_id: return _tg_dataset([], 400, "chat_id is required")
        resp = requests.get(f"{root}/getChat", params={"chat_id": chat_id}, timeout=timeout, verify=verify_ssl)
