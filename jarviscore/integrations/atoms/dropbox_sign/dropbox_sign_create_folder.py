...
# Dropbox Sign API v3 — https://developers.hellosign.com/api/reference/template_create_embedded_draft
SIGN_API = "https://api.hellosign.com/v3"


def dropbox_sign_create_folder(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        if not payload:
            return {
                ...
                "data_count": 0,
                "status": 400,
                "message": "payload is required (HelloSign /template/create_embedded_draft JSON body)",
            }
        ...
        resp = requests.post(
            f"{api}/template/create_embedded_draft",
            headers=headers,
            auth=basic,
            json=payload,
            timeout=timeout,
        ...
        prov: List[str] = []
        if records:
            tid = records[0].get("template_id") or records[0].get("id")
            if tid:
                prov = [str(tid)]
        return {
            "records": records,
