...
# Dropbox Sign API v3 — https://developers.hellosign.com/api/reference/signature_request_get
SIGN_API = "https://api.hellosign.com/v3"


def dropbox_sign_get_file(auth_info: dict, file_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        if not file_id:
            return {
                ...
                "data_count": 0,
                "status": 400,
                "message": "file_id is required (signature_request_id)",
            }
        ...
        resp = requests.get(
            f"{api}/signature_request/{file_id}",
            headers=headers,
            auth=basic,
            timeout=timeout,
            verify=verify_ssl,
