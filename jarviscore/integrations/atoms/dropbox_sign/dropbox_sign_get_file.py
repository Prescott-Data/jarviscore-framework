...
async def dropbox_sign_get_file(file_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get signature request. Basic API key or Bearer OAuth. Official: https://developers.hellosign.com/api/signature-request/get"""
    try:
        if not file_id:
            return {'records': [], 'data_count': 0, 'status': 400, 'message': 'file_id is required (signature_request_id)'}
        ...
        resp = await nexus_call('GET', f'{api}/signature_request/{file_id}', headers=headers)
...
def _sign_auth(json_body: bool=False):
