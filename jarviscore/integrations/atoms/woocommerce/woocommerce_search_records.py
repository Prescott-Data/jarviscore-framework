...
def woocommerce_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """woocommerce REST: search. Official: https://woocommerce.github.io/woocommerce-rest-api-docs/"""
    try:
        ...
        resp = requests.get(root + "/products", headers=hdrs, auth=auth, params={"search": query, "per_page": limit}, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _dataset([], resp.status_code, _err(resp))
        data = resp.json() if resp.content else {}
        return _dataset(data if isinstance(data, list) else [], resp.status_code, "ok")
