...
def woocommerce_get_customer(auth_info: dict, customer_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not customer_id: return _dataset([], 400, 'customer_id is required')
        ...
        resp = requests.get(root + "/customers/" + str(customer_id), headers=hdrs, auth=auth, timeout=timeout, verify=verify_ssl)
