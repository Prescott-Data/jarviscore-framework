...
def woocommerce_create_order(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """woocommerce REST: create order. Official: https://woocommerce.github.io/woocommerce-rest-api-docs/"""
    try:
        if not isinstance(payload, dict) or not payload: return _provision({}, 400, "payload is required (e.g. line_items, billing, payment_method)")
        ...
        resp = requests.post(root + '/orders', headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
