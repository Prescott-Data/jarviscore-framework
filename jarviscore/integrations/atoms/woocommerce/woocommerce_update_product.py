...
def woocommerce_update_product(auth_info: dict, product_id: str, name: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not product_id: return _provision({}, 400, 'product_id is required')
        ...
        resp = requests.put(root + "/products/" + str(product_id), headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
