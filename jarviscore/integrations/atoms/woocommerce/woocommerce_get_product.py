...
def woocommerce_get_product(auth_info: dict, product_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not product_id: return _dataset([], 400, 'product_id is required')
        ...
        resp = requests.get(root + "/products/" + str(product_id), headers=hdrs, auth=auth, timeout=timeout, verify=verify_ssl)
