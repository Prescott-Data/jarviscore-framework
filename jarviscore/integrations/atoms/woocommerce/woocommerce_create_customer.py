...
def woocommerce_create_customer(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """woocommerce REST: create customer. Official: https://woocommerce.github.io/woocommerce-rest-api-docs/"""
    try:
        if not isinstance(payload, dict) or not payload: return _provision({}, 400, "payload is required (email is mandatory for WooCommerce customers)")
        ...
        resp = requests.post(root + '/customers', headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
