...
async def woocommerce_create_customer(payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        if not isinstance(payload, dict) or not payload:
            return _provision({}, 400, 'payload is required (email is mandatory for WooCommerce customers)')
        ...
        resp = await nexus_call('POST', root + '/customers', headers=hdrs, json=payload)
