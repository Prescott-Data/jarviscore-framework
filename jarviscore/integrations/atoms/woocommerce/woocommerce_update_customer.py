...
def woocommerce_update_customer(auth_info: dict, customer_id: str, name: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not customer_id: return _provision({}, 400, 'customer_id is required')
        ...
        resp = requests.put(root + "/customers/" + str(customer_id), headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _provision({}, resp.status_code, _err(resp))
        data = resp.json() if resp.content else {}
        return _provision(data if isinstance(data, dict) else {}, resp.status_code, 'ok', fallback_id=customer_id)
