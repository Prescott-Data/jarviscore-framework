...
def woocommerce_create_product(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """woocommerce REST: create product. Official: https://woocommerce.github.io/woocommerce-rest-api-docs/"""
    try:
        if not isinstance(payload, dict) or not payload: return _provision({}, 400, "payload is required (name is mandatory for WooCommerce products)")
        ...
        resp = requests.post(root + '/products', headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
