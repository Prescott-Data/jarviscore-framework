...
def woocommerce_update_order(auth_info: dict, order_id: str, name: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not order_id: return _provision({}, 400, 'order_id is required')
        ...
        resp = requests.put(root + "/orders/" + str(order_id), headers=hdrs, auth=auth, json=payload, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _provision({}, resp.status_code, _err(resp))
        data = resp.json() if resp.content else {}
        return _provision(data if isinstance(data, dict) else {}, resp.status_code, 'ok', fallback_id=order_id)
