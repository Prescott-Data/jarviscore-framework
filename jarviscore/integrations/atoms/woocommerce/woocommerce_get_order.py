...
async def woocommerce_get_order(order_id: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not order_id:
            return _dataset([], 400, 'order_id is required')
        headers, aerr = _auth()
        ...
        resp = await nexus_call('GET', root + '/orders/' + str(order_id), headers=hdrs)
