...
def woocommerce_get_order(auth_info: dict, order_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """woocommerce REST: get order. Official: https://woocommerce.github.io/woocommerce-rest-api-docs/"""
    try:
        root, err = _root(base_url, auth_info)
        if err: return _dataset([], 400, err)
        if not order_id: return _dataset([], 400, 'order_id is required')
        ...
        resp = requests.get(root + "/orders/" + str(order_id), headers=hdrs, auth=auth, timeout=timeout, verify=verify_ssl)
