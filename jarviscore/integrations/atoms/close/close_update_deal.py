...
def close_update_deal(auth_info: dict, deal_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update an opportunity (deal) in Close CRM via PUT. HTTP Basic API key (username, empty password). Official: https://developer.close.com/"""
    try:
        ...
        headers, basic, auth_err = _close_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        resp = _close_put_json(f"{api}/opportunity/{deal_id}/", headers, basic, payload, timeout, verify_ssl)
