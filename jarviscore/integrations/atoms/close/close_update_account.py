...
def close_update_account(auth_info: dict, account_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a lead (account) in Close CRM via PUT. HTTP Basic API key (username, empty password). Official: https://developer.close.com/"""
    try:
        ...
        resp = _close_put_json(f"{api}/lead/{account_id}/", headers, basic, payload, timeout, verify_ssl)
        ...
        return {"records": records, "data_count": len(records), "status": resp.status_code, "message": "ok", "provision_ids": [account_id]}
