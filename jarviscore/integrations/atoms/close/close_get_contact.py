...
def close_get_contact(auth_info: dict, contact_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a contact by ID from Close CRM. HTTP Basic API key (username, empty password). Official: https://developer.close.com/"""
    try:
        if not contact_id:
        ...
        resp = _close_get(f"{api}/contact/{contact_id}/", headers, basic, None, timeout, verify_ssl)
