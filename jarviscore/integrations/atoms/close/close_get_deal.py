...
def close_get_deal(auth_info: dict, deal_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get an opportunity (deal) by ID from Close CRM. HTTP Basic API key (username, empty password). Official: https://developer.close.com/"""
    try:
        if not deal_id:
        ...
        resp = _close_get(f"{api}/opportunity/{deal_id}/", headers, basic, None, timeout, verify_ssl)
