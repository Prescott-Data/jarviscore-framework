...
def close_create_deal(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create an opportunity (deal) in Close CRM. HTTP Basic API key (username, empty password). Official: https://developer.close.com/"""
    try:
        if not payload:
        ...
        resp = _close_post_json(f"{api}/opportunity/", headers, basic, payload, timeout, verify_ssl)
