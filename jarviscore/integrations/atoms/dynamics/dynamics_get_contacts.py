def dynamics_get_contacts(auth_info: dict, org_url: str, max_results: int = 50) -> dict:
    import requests
    ...
    try:
        resp = requests.get(
            f"{org_url.rstrip('/')}/api/data/v9.2/contacts",
            headers={"Authorization": f"Bearer {access_token}", "OData-MaxVersion": "4.0", "OData-Version": "4.0", "Accept": "application/json"},
            params={"$top": max_results, "$select": "contactid,fullname,emailaddress1,telephone1,jobtitle"},
            timeout=30
        )
