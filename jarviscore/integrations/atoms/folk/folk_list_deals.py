...
def folk_list_deals(auth_info: dict, group_id: str, object_type: str = "Deals", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        gid, otype, err = _folk_deal_scope(group_id, object_type, auth_info)
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        ...
        records, status, message = _folk_paginate(
            f"{api}/groups/{gid}/{otype}", headers, {}, limit, timeout, verify_ssl
        )
        if message != "ok":
