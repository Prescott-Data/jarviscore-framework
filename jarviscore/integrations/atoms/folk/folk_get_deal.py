...
def folk_get_deal(auth_info: dict, group_id: str, deal_id: str, object_type: str = "Deals", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.get(
            f"{api}/groups/{gid}/{otype}/{object_id}",
            headers=headers,
            timeout=timeout,
            verify=verify_ssl,
        )
