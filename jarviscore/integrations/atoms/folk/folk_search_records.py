...
def folk_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search people by name (GET /v1/people?filter[fullName][like]=). Bearer API key in Authorization header per Folk External API. Official: https://developer.folk.app/"""
    try:
        if not query:
        ...
        headers, auth_err = _folk_auth(auth_info)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        params = {"filter[fullName][like]": str(query).strip()}
        records, status, message = _folk_paginate(
            f"{api}/people", headers, params, limit, timeout, verify_ssl
        )
        if message != "ok":
