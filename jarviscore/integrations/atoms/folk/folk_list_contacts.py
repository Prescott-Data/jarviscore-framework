...
async def folk_list_contacts(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        records, status, message = await _folk_paginate(f'{api}/people', headers, {}, limit, timeout, verify_ssl)
        if message != 'ok':
