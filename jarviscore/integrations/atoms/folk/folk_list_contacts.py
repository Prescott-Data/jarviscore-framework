...
def folk_list_contacts(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List people (GET /v1/people). Catalog contact → Folk person. Bearer API key in Authorization header per Folk External API. Official: https://developer.folk.app/"""
    try:
        ...
        records, status, message = _folk_paginate(
            f"{api}/people", headers, {}, limit, timeout, verify_ssl
        )
        if message != "ok":
