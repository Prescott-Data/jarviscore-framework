...
async def folk_get_account(account_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get company (GET /v1/companies/{companyId}). account_id is companyId. Bearer API key in Authorization header per Folk External API. Official: https://developer.folk.app/"""
    try:
        if not account_id:
        ...
        resp = await nexus_call('GET', f'{api}/companies/{company_id}', headers=headers)
