...
async def asana_list_projects(workspace: Optional[str]=None, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        records, status, message = await _asana_paginate(f'{api_root}/projects', headers, limit, timeout, verify_ssl, {'workspace': str(workspace_gid)})
        if message != 'ok':
