...
def asana_update_task(auth_info: dict, task_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update task via PUT /tasks/{task_gid}. Official: https://developers.asana.com/reference/updatetask"""
    try:
        if not task_id:
        ...
        return _asana_provision(records, status, "ok", _asana_provision_ids(body, task_id))
    except Exception as e:
        return _asana_provision([], 500, str(e))


# PUT /tasks/{task_gid} — https://developers.asana.com/reference/updatetask
