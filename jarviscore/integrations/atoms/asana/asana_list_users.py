...
async def asana_list_users(workspace: Optional[str]=None, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List users in a workspace (GET /users?workspace=). Asana REST API v1.0. Official: https://developers.asana.com/reference/rest-api-reference"""
    try:
        if not base_url:
        ...
        if not workspace_gid:
            return {'records': [], 'data_count': 0, 'status': 400, 'message': 'workspace (or workspace_gid in auth_info) is required'}
        (headers, auth_err) = _asana_headers()
        if auth_err:
            return {'records': [], 'data_count': 0, 'status': 401, 'message': auth_err}
        (records, status, message) = await _asana_paginate(f'{api_root}/users', headers, limit, timeout, verify_ssl, {'workspace': str(workspace_gid)})
        if message != 'ok':
...
def _asana_workspace(workspace=None):
    return workspace or None or None or None

async def _asana_paginate(url, headers, limit, timeout, verify_ssl, extra_params=None):
