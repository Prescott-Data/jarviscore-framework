...
def streak_update_account(auth_info: dict, pipeline_key: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Streak API: update account. Official: https://streak.com/api#update_a_pipeline"""
    try:
        if not pipeline_key: return _st_provision({}, 400, "pipeline_key is required")
        ...
        resp = requests.post(root + "/pipelines/" + str(pipeline_key), headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
