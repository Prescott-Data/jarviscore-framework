...
def streak_update_contact(auth_info: dict, contact_key: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Streak API: update contact. Official: https://streak.com/api#update_a_contact"""
    try:
        if not contact_key: return _st_provision({}, 400, "contact_key is required")
        ...
        # Streak contacts are v2: POST /api/v2/contacts/{contactKey}.
        resp = requests.post(root_v2 + "/contacts/" + str(contact_key), headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
