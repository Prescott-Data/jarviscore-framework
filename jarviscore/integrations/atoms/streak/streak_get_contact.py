...
def streak_get_contact(auth_info: dict, contact_key: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Streak API: get contact. Official: https://streak.com/api#get_a_contact"""
    try:
        if not contact_key: return _st_dataset([], 400, "contact_key is required")
        ...
        # Streak contacts are v2: GET /api/v2/contacts/{contactKey}.
        resp = requests.get(root_v2 + "/contacts/" + str(contact_key), headers=headers, timeout=timeout, verify=verify_ssl)
