...
def streak_get_deal(auth_info: dict, box_key: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Streak API: get deal. Official: https://streak.com/api#get_a_box"""
    try:
        if not box_key: return _st_dataset([], 400, "box_key is required")
        root, _ = _st_root(base_url, auth_info)
        headers, err = _st_auth(auth_info)
        if err: return _st_dataset([], 401, err)
        resp = requests.get(root + "/boxes/" + str(box_key), headers=headers, timeout=timeout, verify=verify_ssl)
