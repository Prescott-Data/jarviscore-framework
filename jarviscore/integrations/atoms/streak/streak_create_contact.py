...
def streak_create_contact(auth_info: dict, team_key: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Streak API: create contact. Official: https://streak.com/api#create_a_contact"""
    try:
        if not team_key: return _st_provision({}, 400, "team_key is required")
        ...
        # Streak contacts are team-scoped v2: POST /api/v2/teams/{teamKey}/contacts/.
        resp = requests.post(root_v2 + f"/teams/{team_key}/contacts/", headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
        ...
        return _st_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok")
