...
async def streak_create_contact(team_key: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Streak API: create contact. Official: https://streak.com/api#create_a_contact"""
    try:
        if not team_key:
            return _st_provision({}, 400, 'team_key is required')
        ...
        resp = await nexus_call('POST', root_v2 + f'/teams/{team_key}/contacts/', headers=headers, json=payload)
        try:
