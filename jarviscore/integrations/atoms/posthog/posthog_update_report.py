...
def posthog_update_report(auth_info: dict, report_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update saved insight. Official: https://posthog.com/docs/api/product-analytics-2"""
    try:
        if not report_id:
            return _pg_provision({}, 400, "report_id is required")
        ...
        resp, body, status, msg = _pg_request(
            "patch",
            host + f"/api/projects/{project_id}/insights/{str(report_id).strip()}/",
            auth_info,
            json_body=payload,
            timeout=timeout,
        ...
        return _pg_provision(data, status, "ok", fallback_id=report_id)
