...
def posthog_get_report(auth_info: dict, report_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get insight by ID. Official: https://posthog.com/docs/api/product-analytics-2"""
    try:
        if not report_id:
            return _pg_dataset([], 400, "report_id is required")
