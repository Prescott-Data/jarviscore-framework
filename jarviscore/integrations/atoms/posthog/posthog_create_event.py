...
def posthog_create_event(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if "api_key" not in body:
            api_key = auth_info.get("project_api_key") or auth_info.get("project_token") or auth_info.get("api_key")
            if api_key:
        ...
        ingest = _pg_ingest_host(base_url, auth_info)
        resp = requests.post(ingest + "/i/v0/e/", json=body, timeout=timeout, verify=verify_ssl)
