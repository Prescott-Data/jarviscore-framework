...
def posthog_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        cap = _pg_cap(limit)
        q = str(query).replace("'", "''")
        hogql = auth_info.get("hogql") or (
            "SELECT uuid, event, timestamp, distinct_id, properties FROM events "
            f"WHERE event ILIKE '%{q}%' OR toString(properties) ILIKE '%{q}%' "
            f"ORDER BY timestamp DESC LIMIT {cap}"
        )
        resp, data, status, msg = _pg_query(base_url, auth_info, hogql, timeout, verify_ssl)
        if status >= 400:
