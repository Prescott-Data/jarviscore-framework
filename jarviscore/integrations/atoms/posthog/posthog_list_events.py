...
def posthog_list_events(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List events via HogQL query endpoint. Official: https://posthog.com/docs/api"""
    try:
        auth_info = auth_info or {}
        cap = _pg_cap(limit)
        hogql = auth_info.get("hogql") or f"SELECT uuid, event, timestamp, distinct_id, properties FROM events ORDER BY timestamp DESC LIMIT {cap}"
