...
def wave_accounting_get_contact(auth_info: dict, business_id: str = "", customer_id: str = "", invoice_id: str = "", payment_id: str = "", name: str = "", query: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not customer_id: return _wv_dataset([], 400, "customer_id is required")
        gql = "query GetCustomer($id: ID!, $customerId: ID!) { business(id: $id) { customer(id: $customerId) { id name email } } }"
        data, status, err = _wv_post(gql, auth_info, base_url, {"id": str(bid), "customerId": str(customer_id)}, timeout, verify_ssl)
