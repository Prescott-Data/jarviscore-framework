...
def wave_accounting_list_payments(auth_info: dict, business_id: str = "", customer_id: str = "", invoice_id: str = "", payment_id: str = "", name: str = "", query: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Wave GraphQL: list payments. Official: https://developer.waveapps.com/hc/en-us/articles/360019588314-API-Reference"""
    try:
        bid = business_id or (auth_info or {}).get("business_id")
        if not bid: return _wv_dataset([], 400, "business_id is required")
        if not invoice_id: return _wv_dataset([], 400, "invoice_id is required")
        ...
        data, status, err = _wv_post(gql, auth_info, base_url, {"id": str(bid), "invoiceId": str(invoice_id)}, timeout, verify_ssl)
        ...
        payments = (inv or {}).get("payments") or []
        rows = [p for p in payments if isinstance(p, dict)]
        return _wv_dataset(rows[:limit], status, "ok")
