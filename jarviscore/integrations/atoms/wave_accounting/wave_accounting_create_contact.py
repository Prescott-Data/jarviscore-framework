...
async def wave_accounting_create_contact(business_id: str='', customer_id: str='', invoice_id: str='', payment_id: str='', name: str='', query: str='', limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Wave GraphQL: create contact. Official: https://developer.waveapps.com/hc/en-us/articles/360019588314-API-Reference"""
    try:
        bid = business_id or (None or {}).get('business_id')
        if not bid:
            return _wv_provision({}, 400, 'business_id is required')
        if not name:
            return _wv_provision({}, 400, 'name is required')
        gql = 'mutation CreateCustomer($input: CustomerCreateInput!) { customerCreate(input: $input) { didSucceed inputErrors { message code path } customer { id name email } } }'
        inp = {'businessId': str(bid), 'name': name}
        ...
        payload = (data.get('data') or {}).get('customerCreate') or {} if isinstance(data, dict) else {}
        if not payload.get('didSucceed'):
            return _wv_provision({}, 400, str(payload.get('inputErrors') or 'customerCreate failed')[:1000])
