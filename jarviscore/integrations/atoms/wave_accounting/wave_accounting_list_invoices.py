...
def wave_accounting_list_invoices(auth_info: dict, business_id: str = "", customer_id: str = "", invoice_id: str = "", payment_id: str = "", name: str = "", query: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Wave GraphQL: list invoices. Official: https://developer.waveapps.com/hc/en-us/articles/360019588314-API-Reference"""
    try:
        bid = business_id or (auth_info or {}).get("business_id")
        if not bid: return _wv_dataset([], 400, "business_id is required")
        gql = "query ListInvoices($id: ID!, $page: Int!, $pageSize: Int!) { business(id: $id) { invoices(page: $page, pageSize: $pageSize) { edges { node { id invoiceNumber status total { value } } } } } }"
        data, status, err = _wv_post(gql, auth_info, base_url, {"id": str(bid), "page": 1, "pageSize": int(limit)}, timeout, verify_ssl)
        ...
        edges = (((data.get("data") or {}).get("business") or {}).get("invoices") or {}).get("edges") or [] if isinstance(data, dict) else []
