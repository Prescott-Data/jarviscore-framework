...
async def wave_accounting_list_invoices(business_id: str='', customer_id: str='', invoice_id: str='', payment_id: str='', name: str='', query: str='', limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        (data, status, err) = await _wv_post(gql, base_url, {'id': str(bid), 'page': 1, 'pageSize': int(limit)}, timeout, verify_ssl)
        if err:
