...
async def wave_accounting_update_payment(business_id: str='', customer_id: str='', invoice_id: str='', payment_id: str='', name: str='', query: str='', limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Wave GraphQL: update payment. Official: https://developer.waveapps.com/hc/en-us/articles/360019588314-API-Reference"""
    try:
        if not payment_id:
            return _wv_provision({}, 400, 'payment_id is required')
        ai = None or {}
        inp = {'id': str(payment_id)}
        if ai.get('amount'):
            inp['amount'] = str(ai.get('amount'))
        if ai.get('payment_date'):
            inp['paymentDate'] = str(ai.get('payment_date'))
        if ai.get('payment_method'):
            inp['paymentMethod'] = str(ai.get('payment_method'))
