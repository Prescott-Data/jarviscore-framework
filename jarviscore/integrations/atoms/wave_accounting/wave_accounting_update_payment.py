...
def wave_accounting_update_payment(auth_info: dict, business_id: str = "", customer_id: str = "", invoice_id: str = "", payment_id: str = "", name: str = "", query: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Wave GraphQL: update payment. Official: https://developer.waveapps.com/hc/en-us/articles/360019588314-API-Reference"""
    try:
        if not payment_id: return _wv_provision({}, 400, "payment_id is required")
        ai = auth_info or {}
        inp = {"id": str(payment_id)}
        if ai.get("amount"): inp["amount"] = str(ai.get("amount"))
        if ai.get("payment_date"): inp["paymentDate"] = str(ai.get("payment_date"))
        if ai.get("payment_method"): inp["paymentMethod"] = str(ai.get("payment_method"))
        if ai.get("payment_account_id"): inp["paymentAccountId"] = str(ai.get("payment_account_id"))
        ...
        payload = ((data.get("data") or {}).get("invoicePaymentPatch") or {}) if isinstance(data, dict) else {}
        if not payload.get("didSucceed"):
            return _wv_provision({}, 400, str(payload.get("inputErrors") or "invoicePaymentPatch failed")[:1000])
