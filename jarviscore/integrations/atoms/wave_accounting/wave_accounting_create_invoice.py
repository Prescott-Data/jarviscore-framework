...
async def wave_accounting_create_invoice(business_id: str='', customer_id: str='', invoice_id: str='', payment_id: str='', name: str='', query: str='', limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if name:
            inp['title'] = name
        product_id = (None or {}).get('product_id')
        if product_id:
            inp['items'] = [{'productId': str(product_id)}]
