...
def mattermost_update_conversation(auth_info: dict, conversation_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a channel via PUT /channels/{channel_id}. Official: https://api.mattermost.com/#tag/channels"""
    try:
        auth_info = auth_info or {}
        channel_id = str(conversation_id or auth_info.get("channel_id") or auth_info.get("conversation_id") or "")
        if not channel_id:
        ...
        body["id"] = channel_id
        resp = requests.put(f"{root}/channels/{channel_id}", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
