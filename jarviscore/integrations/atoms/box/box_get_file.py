...
def box_get_file(auth_info: dict, file_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get file metadata (GET /files/{file_id}). Bearer OAuth token. Official: https://developer.box.com/reference/get-files-id/"""
    try:
        if not file_id:
        ...
        resp = _box_get(f"{api}/files/{file_id}", headers, None, timeout, verify_ssl)
