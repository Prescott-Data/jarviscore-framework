...
# Create company account — POST /api/v1/contact with record_type company. Official: https://www.nimble.com/developers/docs/


def nimble_create_account(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create company contact (record_type=company). Official: https://www.nimble.com/developers/docs/"""
    try:
        ...
        if not body.get("record_type"):
            body["record_type"] = "company"
        resp = requests.post(f"{root}/contact", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        ...
        return _nb_provision(data if isinstance(data, dict) else {}, status, "ok")
