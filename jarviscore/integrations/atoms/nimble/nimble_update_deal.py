...
# Update deal — PUT /api/v2/deals/{deal_id}. Official: https://www.nimble.com/developers/docs/


def nimble_update_deal(auth_info: dict, deal_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update deal by id. Official: https://www.nimble.com/developers/docs/"""
    try:
        if not deal_id:
        ...
        resp = requests.put(f"{root}/deals/{deal_id}", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        ...
        return _nb_provision(data if isinstance(data, dict) else {}, status, "ok", fallback_id=deal_id)
