...
# Search contacts — GET /api/v1/contacts with keyword or advanced query param. Official: https://www.nimble.com/developers/docs/


def nimble_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search contacts via keyword or advanced query param. Official: https://www.nimble.com/developers/docs/"""
    try:
        if not query:
        ...
        params = {}
        if auth_info.get("advanced_query") or auth_info.get("use_advanced"):
            import json
            q = auth_info.get("advanced_query")
            if isinstance(q, dict):
                params["query"] = json.dumps(q)
            elif auth_info.get("query_json"):
                params["query"] = str(auth_info.get("query_json"))
            else:
                params["query"] = str(query)
        else:
            params["keyword"] = str(query)
        if auth_info.get("record_type"):
            params["record_type"] = str(auth_info.get("record_type"))
