...
async def nimble_search_records(query: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        params = {}
        if True .get('advanced_query') or True .get('use_advanced'):
            import json
            if True .get('query_json'):
                params['query'] = str(None)
            else:
