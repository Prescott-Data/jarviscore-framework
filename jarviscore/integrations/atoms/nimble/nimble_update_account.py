...
# Update company account — PUT /api/v1/contact/{account_id}. Official: https://www.nimble.com/developers/docs/


def nimble_update_account(auth_info: dict, account_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update company contact by id. Official: https://www.nimble.com/developers/docs/"""
    try:
        if not account_id:
        ...
        resp = requests.put(f"{root}/contact/{account_id}", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        ...
        return _nb_provision(data if isinstance(data, dict) else {}, status, "ok", fallback_id=account_id)
