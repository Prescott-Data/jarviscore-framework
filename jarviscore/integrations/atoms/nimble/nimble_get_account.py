...
# Get company account — GET /api/v1/contact/{account_id}. Official: https://www.nimble.com/developers/docs/


def nimble_get_account(auth_info: dict, account_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get company contact by id. Official: https://www.nimble.com/developers/docs/"""
    try:
        if not account_id:
            return _nb_dataset([], 400, "account_id is required")
        ...
        records, status, msg = _nb_get_one(f"{root}/contact/{account_id}", headers, timeout, verify_ssl)
