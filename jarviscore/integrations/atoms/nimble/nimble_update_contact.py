...
# Update contact — PUT /api/v1/contact/{contact_id}. Official: https://www.nimble.com/developers/docs/


def nimble_update_contact(auth_info: dict, contact_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update contact by id. Official: https://www.nimble.com/developers/docs/"""
    try:
        if not contact_id:
            return _nb_provision({}, 400, "contact_id is required")
        root, err = _nb_v1_root(base_url)
        if err:
            return _nb_provision({}, 400, err)
        headers, aerr = _nb_auth(auth_info, json_body=True)
        if aerr:
            return _nb_provision({}, 400, aerr)
        body = payload if isinstance(payload, dict) else {}
        auth_info = auth_info or {}
        params = {}
        if auth_info.get("type") is not None:
            params["type"] = str(auth_info.get("type"))
        resp = requests.put(f"{root}/contact/{contact_id}", headers=headers, params=params or None, json=body, timeout=timeout, verify=verify_ssl)
        ...
        try:
            ...
            data = {}
        if status >= 400:
            return _nb_provision({}, status, _nb_error(resp))
        return _nb_provision(data if isinstance(data, dict) else {}, status, "ok", fallback_id=contact_id)
    except Exception as e:
        return _nb_provision({}, 500, str(e))
