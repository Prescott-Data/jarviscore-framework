...
# List deals — GET /api/v2/deals (limit, sort, meta.resources). Official: https://www.nimble.com/developers/docs/


def nimble_list_deals(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        params = {}
        if auth_info.get("sort"):
            params["sort"] = str(auth_info.get("sort"))
        if auth_info.get("query"):
            params["query"] = auth_info.get("query")
        records, status, msg = _nb_paginate_deals(f"{root}/deals", headers, params, limit, timeout, verify_ssl)
