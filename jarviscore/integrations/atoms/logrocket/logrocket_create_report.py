...
def logrocket_create_report(auth_info: dict, org_id: str, app_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.post(f"{app_base}/highlights/", headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
        try:
