...
def segment_update_destination(auth_info: dict, destination_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.patch(f"{root}/destinations/{destination_id}", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
            data = resp.json() if resp.content else {}
        except Exception:
            data = {}
        if resp.status_code >= 400:
            return _sg_provision(data, resp.status_code, _sg_err(resp), "destination", fallback_id=destination_id)
        return _sg_provision(data, resp.status_code, "ok", "destination", fallback_id=destination_id)
