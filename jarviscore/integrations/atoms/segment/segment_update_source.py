...
def segment_update_source(auth_info: dict, source_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Segment Public API: update source. Official: https://segment.com/docs/api/public-api/#tag/Sources/operation/updateSource"""
    try:
        if not source_id:
            return _sg_provision({}, 400, "source_id is required", "source")
        ...
        resp = requests.patch(f"{root}/sources/{source_id}", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
            data = resp.json() if resp.content else {}
        except Exception:
            data = {}
        if resp.status_code >= 400:
            return _sg_provision(data, resp.status_code, _sg_err(resp), "source", fallback_id=source_id)
        return _sg_provision(data, resp.status_code, "ok", "source", fallback_id=source_id)
    except Exception as e:
        return _sg_provision({}, 500, str(e), "source")
