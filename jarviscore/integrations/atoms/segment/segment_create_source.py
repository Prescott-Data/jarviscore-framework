...
def segment_create_source(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Segment Public API: create source. Official: https://segment.com/docs/api/public-api/#tag/Sources/operation/createSource"""
    try:
        ...
        resp = requests.post(f"{root}/sources", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
            data = resp.json() if resp.content else {}
        except Exception:
            data = {}
        if resp.status_code >= 400:
            return _sg_provision(data, resp.status_code, _sg_err(resp), "source")
        return _sg_provision(data, resp.status_code, "ok", "source")
