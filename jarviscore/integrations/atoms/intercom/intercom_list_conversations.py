...
async def intercom_list_conversations(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List Intercom conversations. Official: https://developers.intercom.com/docs/references/rest-api/api.intercom.io/conversations"""
    try:
        ...
        (records, status, message) = await _ic_paginate(f'{api}/conversations', headers, limit, timeout, verify_ssl)
...
async def _ic_paginate(url, headers, limit, timeout, verify_ssl):
    records = []
    starting_after = None
    status = 0
    ...
    while len(records) < cap and pages < 100:
        ...
        resp = await _ic_get(url, headers, params, timeout, verify_ssl)
