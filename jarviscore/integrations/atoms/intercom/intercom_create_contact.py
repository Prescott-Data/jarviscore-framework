...
def intercom_create_contact(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create Intercom contact. Official: https://developers.intercom.com/docs/references/rest-api/api.intercom.io/contacts/listcontacts"""
    try:
        if not payload:
        ...
        resp = _ic_post(f"{api}/contacts", headers, payload, timeout, verify_ssl)
