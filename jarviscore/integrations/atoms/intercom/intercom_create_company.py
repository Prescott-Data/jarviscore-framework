...
def intercom_create_company(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create Intercom company. Official: https://developers.intercom.com/docs/references/rest-api/api.intercom.io/companies"""
    try:
        if not payload:
        ...
        resp = _ic_post(f"{api}/companies", headers, payload, timeout, verify_ssl)
