...
def intercom_get_company(auth_info: dict, record_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get Intercom company by ID. Official: https://developers.intercom.com/docs/references/rest-api/api.intercom.io/companies"""
    try:
        if not record_id:
        ...
        resp = _ic_get(f"{api}/companies/{record_id}", headers, None, timeout, verify_ssl)
