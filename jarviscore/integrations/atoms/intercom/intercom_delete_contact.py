...
ATOM_POLICY = {"effect": "destructive", "approval": "required", "idempotency_fields": ["contact_id"], "consequence": "Deletes the Intercom contact."}
INTERCOM_API = 'https://api.intercom.io'

async def intercom_delete_contact(contact_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp = await _ic_delete(f'{api}/contacts/{contact_id}', headers, timeout, verify_ssl)
...
async def _ic_delete(url, headers, timeout, verify_ssl):
