...
def nutshell_update_account(auth_info: dict, account_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update Account via Nutshell JSON-RPC. Official: https://developers-rpc.nutshell.com/detail/class_core.html#editAccount"""
    try:
        if not account_id:
            return _ns_provision({}, 400, "account_id is required")
        ...
        account.pop("rev", None)
        if rev is None:
            existing, est, _ = _ns_rpc(base, headers, "getAccount", {"accountId": aid}, timeout, verify_ssl)
            if est >= 400 or not isinstance(existing, dict):
                return _ns_provision({}, est if est >= 400 else 400, "account not found or rev missing")
            rev = existing.get("rev")
        result, status, msg = _ns_rpc(base, headers, "editAccount", {"accountId": aid, "rev": rev, "account": account}, timeout, verify_ssl)
        if msg != "ok":
            return _ns_provision({}, status, msg, fallback_id=account_id)
        return _ns_provision(result, status, msg, fallback_id=account_id)
