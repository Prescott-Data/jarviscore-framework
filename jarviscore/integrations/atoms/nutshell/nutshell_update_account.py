...
async def nutshell_update_account(account_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if rev is None:
            (existing, est, _) = await _ns_rpc(base, headers, 'getAccount', {'accountId': aid}, timeout, verify_ssl)
        ...
        (result, status, msg) = await _ns_rpc(base, headers, 'editAccount', {'accountId': aid, 'rev': rev, 'account': account}, timeout, verify_ssl)
        if msg != 'ok':
