...
def nutshell_update_deal(auth_info: dict, deal_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update Deal via Nutshell JSON-RPC. Official: https://developers-rpc.nutshell.com/detail/class_core.html#editLead"""
    try:
        if not deal_id:
        ...
        lead.pop("rev", None)
        if rev is None:
            existing, est, _ = _ns_rpc(base, headers, "getLead", {"leadId": lid}, timeout, verify_ssl)
        ...
        result, status, msg = _ns_rpc(base, headers, "editLead", {"leadId": lid, "rev": rev, "lead": lead}, timeout, verify_ssl)
        if msg != "ok":
