...
def nutshell_update_contact(auth_info: dict, contact_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if rev is None:
            existing, est, _ = _ns_rpc(base, headers, "getContact", {"contactId": cid}, timeout, verify_ssl)
        ...
        result, status, msg = _ns_rpc(base, headers, "editContact", params, timeout, verify_ssl)
        if msg != "ok":
