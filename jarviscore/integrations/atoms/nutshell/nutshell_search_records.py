...
def nutshell_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        cap = _ns_cap(limit)
        result, status, msg = _ns_rpc(base, headers, "searchUniversal", {"string": str(query), "limit": cap}, timeout, verify_ssl)
        if msg != "ok":
