...
async def nutshell_create_account(payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        result, status, msg = await _ns_rpc(base, headers, 'newAccount', {'account': payload}, timeout, verify_ssl)
        if msg != 'ok':
