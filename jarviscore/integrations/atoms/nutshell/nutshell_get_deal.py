...
def nutshell_get_deal(auth_info: dict, deal_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get Deal via Nutshell JSON-RPC. Official: https://developers-rpc.nutshell.com/detail/class_core.html#getLead"""
    try:
        if not deal_id:
        ...
        result, status, msg = _ns_rpc(base, headers, "getLead", {"leadId": lid}, timeout, verify_ssl)
        if msg != "ok":
