...
def nutshell_get_contact(auth_info: dict, contact_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        result, status, msg = _ns_rpc(base, headers, "getContact", {"contactId": int(contact_id) if str(contact_id).isdigit() else contact_id}, timeout, verify_ssl)
        if msg != "ok":
