...
def nutshell_list_deals(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List Deals via Nutshell JSON-RPC. Official: https://developers-rpc.nutshell.com/detail/class_core.html#findLeads"""
    try:
        ...
        query = auth_info.get("query") if isinstance(auth_info.get("query"), dict) else {}
        params = {"query": query, "orderBy": "id", "orderDirection": "ASC", "stubResponses": False}
        records, status, msg = _ns_paginate(base, headers, "findLeads", params, limit, timeout, verify_ssl)
