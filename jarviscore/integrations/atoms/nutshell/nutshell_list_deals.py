...
async def nutshell_list_deals(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        query = None if isinstance(None, dict) else {}
        params = {'query': query, 'orderBy': 'id', 'orderDirection': 'ASC', 'stubResponses': False}
        records, status, msg = await _ns_paginate(base, headers, 'findLeads', params, limit, timeout, verify_ssl)
