...
def nutshell_get_account(auth_info: dict, account_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get Account via Nutshell JSON-RPC. Official: https://developers-rpc.nutshell.com/detail/class_core.html#getAccount"""
    try:
        if not account_id:
        ...
        result, status, msg = _ns_rpc(base, headers, "getAccount", {"accountId": aid}, timeout, verify_ssl)
        if msg != "ok":
