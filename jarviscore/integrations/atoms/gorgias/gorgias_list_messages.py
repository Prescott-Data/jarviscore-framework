...
def gorgias_list_messages(auth_info: dict, conversation_id: Optional[str] = None, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        extra = {}
        tid = _gorgias_ticket_id(auth_info, None, conversation_id)
        if tid:
            extra["ticket_id"] = tid
        records, status, message = _gorgias_cursor_paginate(f"{api}/messages", headers, basic, limit, timeout, verify_ssl, extra)
