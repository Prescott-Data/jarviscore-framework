...
def gorgias_create_message(auth_info: dict, payload: Dict[str, Any], conversation_id: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        tid = _gorgias_ticket_id(auth_info, payload, conversation_id)
        if not tid:
        ...
        resp = _gorgias_post(f"{api}/tickets/{tid}/messages", headers, basic, payload, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        data = resp.json() if resp.text else {}
        records = _gorgias_single_record(data)
        return {"records": records, "data_count": len(records), "status": resp.status_code, "message": "ok", "provision_ids": _gorgias_provision_id(data)}
