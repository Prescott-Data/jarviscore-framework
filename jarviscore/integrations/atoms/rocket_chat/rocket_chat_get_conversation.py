...
def rocket_chat_get_conversation(auth_info: dict, conversation_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _rc_request("get", root + "/channels.info", auth_info, params={"roomId": conversation_id}, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
