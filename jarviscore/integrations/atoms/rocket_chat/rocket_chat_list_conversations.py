...
def rocket_chat_list_conversations(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _rc_request("get", root + "/rooms.get", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _rc_dataset([], status, msg)
        rows = body.get("update") if isinstance(body.get("update"), list) else []
        return _rc_dataset(rows[: _rc_cap(limit)], status, msg)
