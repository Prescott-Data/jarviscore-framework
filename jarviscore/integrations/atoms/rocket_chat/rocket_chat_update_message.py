...
def rocket_chat_update_message(auth_info: dict, message_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a message. Official: https://developer.rocket.chat/apidocs/authentication-api"""
    try:
        if not message_id:
            return _rc_provision({}, 400, "message_id is required")
        if not isinstance(payload, dict) or not payload:
            return _rc_provision({}, 400, "payload is required")
        room_id = _rc_room_id(auth_info, payload)
        text = payload.get("text") or payload.get("msg")
        if not room_id or not text:
            return _rc_provision({}, 400, "auth_info.room_id and payload.text are required")
        root, err = _rc_root(base_url, auth_info)
        if err:
            return _rc_provision({}, 400, err)
        body_payload = {"roomId": room_id, "msgId": message_id, "text": text}
        resp, body, status, msg = _rc_request("post", root + "/chat.update", auth_info, json_body=body_payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _rc_provision(body if isinstance(body, dict) else {}, status, msg, fallback_id=message_id)
        message = body.get("message") if isinstance(body.get("message"), dict) else body
        return _rc_provision(message if isinstance(message, dict) else {}, status, "ok", fallback_id=message_id)
    except Exception as e:
        return _rc_provision({}, 500, str(e), fallback_id=message_id)
