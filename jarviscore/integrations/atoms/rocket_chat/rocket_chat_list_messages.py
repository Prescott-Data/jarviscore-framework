...
def rocket_chat_list_messages(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List messages in a room. Official: https://developer.rocket.chat/apidocs/authentication-api"""
    try:
        room_id = _rc_room_id(auth_info)
        if not room_id:
        ...
        resp, body, status, msg = _rc_request("get", root + "/channels.messages", auth_info, params=params, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
