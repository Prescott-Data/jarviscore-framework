...
def rocket_chat_create_conversation(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _rc_request("post", root + "/channels.create", auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
