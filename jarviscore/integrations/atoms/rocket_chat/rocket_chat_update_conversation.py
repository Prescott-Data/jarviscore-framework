...
def rocket_chat_update_conversation(auth_info: dict, conversation_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _rc_request("post", root + "/channels.rename", auth_info, json_body=body_payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
        ...
        return _rc_provision(channel if isinstance(channel, dict) else {}, status, "ok", fallback_id=conversation_id)
    except Exception as e:
        return _rc_provision({}, 500, str(e), fallback_id=conversation_id)
