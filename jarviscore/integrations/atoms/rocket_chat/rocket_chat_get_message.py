...
def rocket_chat_get_message(auth_info: dict, message_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a message by ID. Official: https://developer.rocket.chat/apidocs/authentication-api"""
    try:
        if not message_id:
        ...
        resp, body, status, msg = _rc_request("get", root + "/chat.getMessage", auth_info, params={"msgId": message_id}, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _rc_dataset([], status, msg)
        message = body.get("message") if isinstance(body.get("message"), dict) else None
        return _rc_dataset([message] if message else [], status, msg)
