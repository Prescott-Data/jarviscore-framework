...
def rocket_chat_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        cap = _rc_cap(limit)
        out = []
        resp, body, status, msg = _rc_request("get", root + "/spotlight", auth_info, params={"query": query}, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
        ...
        for key, rtype in (("rooms", "room"), ("users", "user")):
            ...
            if len(out) >= cap:
                break
        if len(out) < cap:
            resp2, body2, status2, msg2 = _rc_request("get", root + "/rooms.get", auth_info, timeout=timeout, verify_ssl=verify_ssl)
            if status2 < 400:
