...
def freshchat_create_conversation(auth_info: dict, channel_id: str, messages: List[Any], users: Optional[List[Any]] = None, status: str = "new", assigned_agent_id: Optional[str] = None, assigned_group_id: Optional[str] = None, properties: Optional[Dict[str, Any]] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        headers, auth_err = _fc_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        resp = requests.post(f"{api}/conversations", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        status_code = resp.status_code
        data = resp.json() if resp.text else {}
        if status_code >= 400:
            return {"records": [], "data_count": 0, "status": status_code, "message": resp.text[:1000]}
        ...
        return {
            ...
            "status": status_code,
            "message": "ok",
            "provision_ids": _fc_provision_id(data),
        }
