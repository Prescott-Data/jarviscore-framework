...
def podio_get_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get app by ID. Official: https://developers.podio.com/doc/applications/get-app-22349"""
    try:
        if not project_id:
            return _po_dataset([], 400, "project_id is required")
        root, err = _po_root(base_url, auth_info)
        if err:
            return _po_dataset([], 400, err)
        resp, body, status, msg = _po_request("get", root + "/app/" + str(project_id).strip(), auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
