...
async def podio_get_project(project_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = await _po_request('get', root + '/app/' + str(project_id).strip(), timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
