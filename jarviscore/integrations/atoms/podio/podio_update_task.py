...
def podio_update_task(auth_info: dict, task_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update task. Official: https://developers.podio.com/doc/tasks/update-task-93188"""
    try:
        if not task_id:
            return _po_provision({}, 400, "task_id is required")
        if not isinstance(payload, dict) or not payload:
            return _po_provision({}, 400, "payload is required")
        root, err = _po_root(base_url, auth_info)
        if err:
            return _po_provision({}, 400, err)
        resp, body, status, msg = _po_request("put", root + "/task/" + str(task_id).strip(), auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _po_provision(body if isinstance(body, dict) else {}, status, msg)
        data = body if isinstance(body, dict) else {}
        return _po_provision(data, status, "ok", fallback_id=task_id)
