...
def podio_list_projects(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List apps (catalog projects) by space or all accessible apps. Official: https://developers.podio.com/doc/applications"""
    try:
        auth_info = auth_info or {}
        space_id = auth_info.get("space_id") or auth_info.get("space")
        if space_id:
            path = "/app/space/" + str(space_id).strip() + "/"
            records, status, msg = _po_paginate_get(path, base_url, auth_info, limit, timeout, verify_ssl)
        else:
            root, err = _po_root(base_url, auth_info)
            if err:
                return _po_dataset([], 400, err)
            resp, body, status, msg = _po_request("get", root + "/app/v2/", auth_info, timeout=timeout, verify_ssl=verify_ssl)
            if status >= 400:
