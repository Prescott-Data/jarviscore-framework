...
def podio_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        app_id = auth_info.get("app_id") or auth_info.get("app")
        ...
        while len(records) < cap:
            ...
            ref_type = auth_info.get("ref_type")
            if ref_type:
                body["ref_type"] = ref_type
            resp, data, status, msg = _po_request("post", root + path, auth_info, json_body=body, timeout=timeout, verify_ssl=verify_ssl)
            if status >= 400:
