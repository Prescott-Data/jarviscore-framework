...
def podio_create_project(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create app on a space. Official: https://developers.podio.com/doc/applications/add-new-app-22351"""
    try:
        ...
        resp, body, status, msg = _po_request("post", root + "/app/", auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _po_provision(body if isinstance(body, dict) else {}, status, msg)
        data = body if isinstance(body, dict) else {}
        return _po_provision(data, status, "ok")
