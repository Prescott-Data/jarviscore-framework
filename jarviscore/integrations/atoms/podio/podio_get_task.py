...
def podio_get_task(auth_info: dict, task_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _po_request("get", root + "/task/" + str(task_id).strip(), auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
