...
def targetprocess_create_task(auth_info: dict, name: str, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not name or not project_id:
            return _tp_provision({}, 400, "name and project_id are required")
        headers, aerr = _tp_auth(auth_info, json_body=True)
        if aerr:
            return _tp_provision({}, 401, aerr)
        body = {"Name": name, "Project": {"Id": int(project_id) if str(project_id).isdigit() else project_id}}
        resp = requests.post(f"{root}/UserStories", headers=headers, params={"access_token": str((auth_info or {}).get("access_token") or "").strip(), "format": "json"}, json=body, timeout=timeout, verify=verify_ssl)
