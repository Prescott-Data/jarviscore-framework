...
def targetprocess_get_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Targetprocess REST: get project by Id. Official: https://dev.targetprocess.com/docs/REST%20API"""
    try:
        root, err = _tp_root(base_url, auth_info)
        if err:
            return _tp_dataset([], 400, err)
        if not project_id:
            return _tp_dataset([], 400, "project_id is required")
        headers, aerr = _tp_auth(auth_info)
        if aerr:
            return _tp_dataset([], 401, aerr)
        resp = requests.get(f"{root}/Projects/{project_id}", headers=headers, params={"access_token": str((auth_info or {}).get("access_token") or "").strip(), "format": "json"}, timeout=timeout, verify=verify_ssl)
