...
def targetprocess_update_task(auth_info: dict, task_id: str, name: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.post(f"{root}/UserStories/{task_id}", headers=headers, params={"access_token": str((auth_info or {}).get("access_token") or "").strip(), "format": "json"}, json=body, timeout=timeout, verify=verify_ssl)
