...
def targetprocess_update_project(auth_info: dict, project_id: str, name: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Targetprocess REST: update project. Official: https://dev.targetprocess.com/docs/REST%20API"""
    try:
        root, err = _tp_root(base_url, auth_info)
        if err:
            return _tp_provision({}, 400, err)
        if not project_id:
            return _tp_provision({}, 400, "project_id is required")
        headers, aerr = _tp_auth(auth_info, json_body=True)
        if aerr:
            return _tp_provision({}, 401, aerr)
        body = {"Name": name} if name else {}
        resp = requests.post(f"{root}/Projects/{project_id}", headers=headers, params={"access_token": str((auth_info or {}).get("access_token") or "").strip(), "format": "json"}, json=body, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _tp_provision({}, resp.status_code, _tp_err(resp))
        data = resp.json() if resp.content else {}
        return _tp_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok", fallback_id=project_id)
