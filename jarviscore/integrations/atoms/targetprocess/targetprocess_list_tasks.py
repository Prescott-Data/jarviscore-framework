...
def targetprocess_list_tasks(auth_info: dict, project_id: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Targetprocess REST: list user stories (tasks). Official: https://dev.targetprocess.com/docs/REST%20API"""
    try:
        ...
        params = {}
        if project_id:
            params["where"] = f"Project.Id eq {project_id}"
        records, status, msg = _tp_paginate(f"{root}/UserStories", auth_info, params, limit, timeout, verify_ssl)
