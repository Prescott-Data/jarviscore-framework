...
# Jumia Seller Center API — http://sellerapi.sellercenter.jumia.com/v2.7.11/sales-order-endpoints/getorders/
JUMIA_SC_API = "http://sellerapi.sellercenter.jumia.com"
SC_VERSION = "1.0"


def jumia_seller_center_list_orders(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List orders via GetOrders. Official: http://sellerapi.sellercenter.jumia.com/v2.7.11/sales-order-endpoints/getorders/"""
    try:
        ...
        extra = {"Limit": str(cap), "Offset": "0"}
        resp = _jsc_call(base, "GetOrders", user_id, api_key, extra, timeout=timeout, verify_ssl=verify_ssl)
        data = _jsc_json(resp)
...
def _jsc_orders(data):
    ...
    if isinstance(orders, dict):
        ...
        if isinstance(order, list):
            records.extend([o for o in order if isinstance(o, dict)])
