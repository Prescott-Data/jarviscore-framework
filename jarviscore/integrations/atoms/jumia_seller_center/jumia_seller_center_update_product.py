...
# Jumia Seller Center API — http://sellerapi.sellercenter.jumia.com/v2.7.11/product-endpoints/productupdate/
JUMIA_SC_API = "http://sellerapi.sellercenter.jumia.com"
SC_VERSION = "1.0"


def jumia_seller_center_update_product(auth_info: dict, product_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        payload.setdefault("SellerSku", product_id)
        ...
        resp = _jsc_call(base, "ProductUpdate", user_id, api_key, body=body, timeout=timeout, verify_ssl=verify_ssl)
        data = _jsc_json(resp)
        ...
        records = [{"feed_id": fid, "seller_sku": product_id} for fid in feed_ids] if feed_ids else [{"seller_sku": product_id}]
        return {"records": records, "data_count": len(records), "status": status, "message": "ok", "provision_ids": feed_ids or [product_id]}
...
def _jsc_product_xml(payload):
    ...
    if products is None and any(k in payload for k in ("SellerSku", "seller_sku", "Name", "name", "Price", "price")):
        products = [payload]
