...
async def jumia_seller_center_search_records(query: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Search products via GetProducts Search filter. Official: http://sellerapi.sellercenter.jumia.com/v2.7.11/product-endpoints/getproducts/"""
    try:
        if not query:
        ...
        extra = {'Search': str(query), 'Limit': str(cap)}
        resp = await _jsc_call(base, 'GetProducts', user_id, api_key, extra, timeout=timeout, verify_ssl=verify_ssl)
        data = _jsc_json(resp)
        ...
        records = _jsc_products(data)[:cap]
