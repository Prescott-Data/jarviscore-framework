...
def freshsales_list_accounts(auth_info: dict, view_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        records, status, message = _fs_paginate_view(url, headers, "sales_accounts", limit, timeout, verify_ssl)
        if message != "ok":
