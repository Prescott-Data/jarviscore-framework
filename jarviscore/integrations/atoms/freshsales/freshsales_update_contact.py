...
def freshsales_update_contact(auth_info: dict, contact_id: str, fields: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.put(url, headers=headers, json={"contact": fields}, timeout=timeout, verify=verify_ssl)
        ...
        return {
            ...
            "status": status,
            "message": "ok",
            "provision_ids": _fs_provision_id(data, "contact") or [str(contact_id).strip()],
        }
