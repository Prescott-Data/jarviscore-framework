...
async def freshsales_create_contact(fields: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        url = f'{api}/contacts'
        resp = await nexus_call('POST', url, headers=headers, json={'contact': fields})
        status = resp['status_code']
        data = resp['json'] if resp['body'] else {}
        if status >= 400:
            return {'records': [], 'data_count': 0, 'status': status, 'message': resp['body'][:1000]}
        records = _fs_entity_records(data, 'contact')
        return {'records': records, 'data_count': len(records), 'status': status, 'message': 'ok', 'provision_ids': _fs_provision_id(data, 'contact')}
