...
def freshsales_create_contact(auth_info: dict, fields: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create a contact (POST /api/contacts, body.contact). Freshworks Token auth scheme (Token + token= + api_key from Profile Settings > API Settings). Official: https://developers.freshworks.com/docs/api/crm/sales/"""
    try:
        ...
        url = f"{api}/contacts"
        resp = requests.post(url, headers=headers, json={"contact": fields}, timeout=timeout, verify=verify_ssl)
        ...
        return {
            ...
            "status": status,
            "message": "ok",
            "provision_ids": _fs_provision_id(data, "contact"),
        }
