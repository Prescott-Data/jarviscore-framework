...
def freshsales_get_account(auth_info: dict, account_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """View an account (GET .../sales_accounts/{id}). response.sales_account. Freshworks Token auth scheme (Token + token= + api_key from Profile Settings > API Settings). Official: https://developers.freshworks.com/docs/api/crm/sales/"""
    try:
        if not account_id:
