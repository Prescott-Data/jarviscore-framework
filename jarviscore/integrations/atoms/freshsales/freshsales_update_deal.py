...
def freshsales_update_deal(auth_info: dict, deal_id: str, fields: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a deal (PUT .../deals/{id}, body.deal). Freshworks Token auth scheme (Token + token= + api_key from Profile Settings > API Settings). Official: https://developers.freshworks.com/docs/api/crm/sales/"""
    try:
        if not deal_id:
        ...
        headers, auth_err = _fs_sales_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        url = f"{api}/deals/{str(deal_id).strip()}"
        resp = requests.put(url, headers=headers, json={"deal": fields}, timeout=timeout, verify=verify_ssl)
        status = resp.status_code
        data = resp.json() if resp.text else {}
        if status >= 400:
            return {"records": [], "data_count": 0, "status": status, "message": resp.text[:1000]}
        records = _fs_entity_records(data, "deal")
        return {
            "records": records,
            "data_count": len(records),
            "status": status,
            "message": "ok",
            "provision_ids": _fs_provision_id(data, "deal") or [str(deal_id).strip()],
        }
