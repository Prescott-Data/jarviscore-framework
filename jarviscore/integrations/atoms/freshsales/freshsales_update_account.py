...
async def freshsales_update_account(account_id: str, fields: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp = await nexus_call('PUT', url, headers=headers, json={'sales_account': fields})
        status = resp['status_code']
        data = resp['json'] if resp['body'] else {}
        if status >= 400:
            return {'records': [], 'data_count': 0, 'status': status, 'message': resp['body'][:1000]}
        records = _fs_entity_records(data, 'sales_account')
        return {'records': records, 'data_count': len(records), 'status': status, 'message': 'ok', 'provision_ids': _fs_provision_id(data, 'sales_account') or [str(account_id).strip()]}
