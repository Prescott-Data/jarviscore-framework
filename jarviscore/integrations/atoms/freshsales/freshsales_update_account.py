...
def freshsales_update_account(auth_info: dict, account_id: str, fields: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        url = f"{api}/sales_accounts/{str(account_id).strip()}"
        resp = requests.put(url, headers=headers, json={"sales_account": fields}, timeout=timeout, verify=verify_ssl)
        ...
        return {
            ...
            "status": status,
            "message": "ok",
            "provision_ids": _fs_provision_id(data, "sales_account") or [str(account_id).strip()],
        }
