...
def freshsales_list_deals(auth_info: dict, view_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List deals from a view (GET .../deals/view/{viewId}). response.deals[]. Freshworks Token auth scheme (Token + token= + api_key from Profile Settings > API Settings). Official: https://developers.freshworks.com/docs/api/crm/sales/"""
    try:
        if view_id in (None, ""):
            return {"records": [], "data_count": 0, "status": 400, "message": "view_id is required (from GET /deals/filters)"}
        ...
        url = f"{api}/deals/view/{str(view_id).strip()}"
        records, status, message = _fs_paginate_view(url, headers, "deals", limit, timeout, verify_ssl)
        if message != "ok":
