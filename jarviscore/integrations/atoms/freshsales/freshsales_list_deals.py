...
async def freshsales_list_deals(view_id: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        records, status, message = await _fs_paginate_view(url, headers, 'deals', limit, timeout, verify_ssl)
        if message != 'ok':
