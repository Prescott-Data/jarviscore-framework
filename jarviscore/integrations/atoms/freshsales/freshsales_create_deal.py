...
async def freshsales_create_deal(fields: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        url = f'{api}/deals'
        resp = await nexus_call('POST', url, headers=headers, json={'deal': fields})
        status = resp['status_code']
        data = resp['json'] if resp['body'] else {}
        if status >= 400:
            return {'records': [], 'data_count': 0, 'status': status, 'message': resp['body'][:1000]}
        records = _fs_entity_records(data, 'deal')
        return {'records': records, 'data_count': len(records), 'status': status, 'message': 'ok', 'provision_ids': _fs_provision_id(data, 'deal')}
