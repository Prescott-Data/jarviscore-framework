...
# Zoho Desk API v1 — Official: https://desk.zoho.com/DeskAPIDocument#create-department
...
def zoho_desk_create_department(auth_info: dict, name: str, description: str = "", org_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_desk API: create department. Official: https://desk.zoho.com/DeskAPIDocument"""
    try:
        if not name:
            return _zd_provision({}, 400, "name is required")
        ...
        resp = requests.post(f"{root}/departments", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _zd_provision({}, resp.status_code, _zd_err(resp))
        data = resp.json() if resp.content else {}
        return _zd_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok")
