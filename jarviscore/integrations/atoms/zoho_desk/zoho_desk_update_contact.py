...
# Zoho Desk API v1 — Official: https://desk.zoho.com/DeskAPIDocument#update-contact
...
def zoho_desk_update_contact(auth_info: dict, contact_id: str, first_name: str = "", last_name: str = "", email: str = "", org_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if email:
            body["email"] = email
        resp = requests.patch(f"{root}/contacts/{contact_id}", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
