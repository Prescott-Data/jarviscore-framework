...
# Zoho Desk API v1 — Official: https://desk.zoho.com/DeskAPIDocument#create-ticket
...
def zoho_desk_create_ticket(auth_info: dict, subject: str, department_id: str, contact_id: str = "", description: str = "", org_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_desk API: create ticket. Official: https://desk.zoho.com/DeskAPIDocument"""
    try:
        if not subject:
        ...
        resp = requests.post(f"{root}/tickets", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
