...
# Zoho Desk API v1 — Official: https://desk.zoho.com/DeskAPIDocument#update-ticket
...
def zoho_desk_update_ticket(auth_info: dict, ticket_id: str, subject: str = "", status: str = "", priority: str = "", org_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_desk API: update ticket. Official: https://desk.zoho.com/DeskAPIDocument"""
    try:
        if not ticket_id:
            return _zd_provision({}, 400, "ticket_id is required")
        ...
        resp = requests.patch(f"{root}/tickets/{ticket_id}", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _zd_provision({}, resp.status_code, _zd_err(resp))
        data = resp.json() if resp.content else {}
        return _zd_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok", fallback_id=ticket_id)
