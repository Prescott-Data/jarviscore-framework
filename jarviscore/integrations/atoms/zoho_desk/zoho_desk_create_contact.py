...
# Zoho Desk API v1 — Official: https://desk.zoho.com/DeskAPIDocument#create-contact
...
def zoho_desk_create_contact(auth_info: dict, last_name: str, first_name: str = "", email: str = "", org_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_desk API: create contact. Official: https://desk.zoho.com/DeskAPIDocument"""
    try:
        if not last_name:
            return _zd_provision({}, 400, "last_name is required")
        ...
        if email:
            body["email"] = email
        resp = requests.post(f"{root}/contacts", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
