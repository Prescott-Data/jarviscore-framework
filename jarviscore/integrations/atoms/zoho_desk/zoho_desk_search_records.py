...
# Zoho Desk API v1 — Official: https://desk.zoho.com/DeskAPIDocument#search-tickets
...
def zoho_desk_search_records(auth_info: dict, query: str, org_id: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_desk API: search records. Official: https://desk.zoho.com/DeskAPIDocument"""
    try:
        if not query:
        ...
        records, status, msg = _zd_paginate(f"{root}/tickets/search", headers, limit, timeout, verify_ssl, {"_all": query})
