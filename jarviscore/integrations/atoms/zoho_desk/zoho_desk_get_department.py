...
# Zoho Desk API v1 — Official: https://desk.zoho.com/DeskAPIDocument#get-department
...
def zoho_desk_get_department(auth_info: dict, department_id: str, org_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zoho_desk API: get department. Official: https://desk.zoho.com/DeskAPIDocument"""
    try:
        if not department_id:
            return _zd_dataset([], 400, "department_id is required")
        ...
        resp = requests.get(f"{root}/departments/{department_id}", headers=headers, timeout=timeout, verify=verify_ssl)
