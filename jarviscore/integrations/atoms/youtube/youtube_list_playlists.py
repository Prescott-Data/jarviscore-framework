async def youtube_list_playlists(channel_id: str, max_results: int=25) -> list:
    """List playlists via the youtube API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/playlists', params={'part': 'snippet,contentDetails', 'channelId': channel_id, 'maxResults': max_results})
    return [{'id': item['id'], 'title': item['snippet']['title'], 'description': item['snippet']['description'], 'published_at': item['snippet']['publishedAt'], 'item_count': item['contentDetails']['itemCount'], 'thumbnail': item['snippet']['thumbnails'].get('default', {}).get('url')} for item in resp.get('items', [])]
