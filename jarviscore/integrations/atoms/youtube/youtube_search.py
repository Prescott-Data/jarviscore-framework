async def youtube_search(query: str, max_results: int=10, video_type: str='video') -> list:
    """Search via the youtube API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/search', params={'part': 'snippet', 'q': query, 'type': video_type, 'maxResults': max_results})
    return [{'id': item['id'].get('videoId') or item['id'].get('channelId') or item['id'].get('playlistId'), 'title': item['snippet']['title'], 'description': item['snippet']['description'], 'channel': item['snippet']['channelTitle'], 'published_at': item['snippet']['publishedAt'], 'thumbnail': item['snippet']['thumbnails'].get('default', {}).get('url')} for item in resp.get('items', [])]
