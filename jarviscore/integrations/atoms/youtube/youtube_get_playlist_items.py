async def youtube_get_playlist_items(playlist_id: str, max_results: int=25) -> list:
    """Get playlist items via the youtube API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/playlistItems', params={'part': 'snippet,contentDetails', 'playlistId': playlist_id, 'maxResults': max_results})
    return [{'video_id': item['contentDetails']['videoId'], 'title': item['snippet']['title'], 'description': item['snippet']['description'], 'published_at': item['snippet']['publishedAt'], 'position': item['snippet']['position'], 'thumbnail': item['snippet']['thumbnails'].get('default', {}).get('url')} for item in resp.get('items', [])]
