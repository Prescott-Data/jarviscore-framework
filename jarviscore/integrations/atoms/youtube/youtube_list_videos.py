async def youtube_list_videos(channel_id: str, max_results: int=25) -> list:
    """List videos via the youtube API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/search', params={'part': 'snippet', 'channelId': channel_id, 'type': 'video', 'order': 'date', 'maxResults': max_results})
    return [{'id': item['id']['videoId'], 'title': item['snippet']['title'], 'description': item['snippet']['description'], 'published_at': item['snippet']['publishedAt'], 'thumbnail': item['snippet']['thumbnails'].get('default', {}).get('url')} for item in resp.get('items', [])]
