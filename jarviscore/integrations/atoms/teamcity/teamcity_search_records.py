...
def teamcity_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not query: return _tc_dataset([], 400, "query is required")
        headers, aerr = _tc_auth(auth_info)
        if aerr: return _tc_dataset([], 401, aerr)
        resp = requests.get(f"{root}/projects", headers=headers, params={"locator": "count:100"}, timeout=timeout, verify=verify_ssl)
