...
def teamcity_get_build(auth_info: dict, build_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not build_id: return _tc_dataset([], 400, "build_id is required")
        headers, aerr = _tc_auth(auth_info)
        if aerr: return _tc_dataset([], 401, aerr)
        resp = requests.get(f"{root}/builds/id:{build_id}", headers=headers, timeout=timeout, verify=verify_ssl)
