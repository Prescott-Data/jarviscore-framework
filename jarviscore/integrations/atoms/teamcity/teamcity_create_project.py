...
def teamcity_create_project(auth_info: dict, name: str, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if err: return _tc_provision({}, 400, err)
        if not name or not project_id: return _tc_provision({}, 400, "name and project_id are required")
        headers, aerr = _tc_auth(auth_info)
        if aerr: return _tc_provision({}, 401, aerr)
        headers["Content-Type"] = "application/json"
        body = {"name": name, "id": project_id}
        resp = requests.post(f"{root}/projects", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _tc_provision({}, resp.status_code, _tc_err(resp))
        data = resp.json() if resp.content else {}
        return _tc_provision(data if isinstance(data, dict) else {}, resp.status_code, "ok")
    except Exception as e: return _tc_provision({}, 500, str(e))
