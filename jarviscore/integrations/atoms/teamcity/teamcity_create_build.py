...
def teamcity_create_build(auth_info: dict, build_type_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not build_type_id: return _tc_provision({}, 400, "build_type_id is required")
        headers, aerr = _tc_auth(auth_info)
        if aerr: return _tc_provision({}, 401, aerr)
        headers["Content-Type"] = "application/json"
        body = {"buildType": {"id": build_type_id}}
        resp = requests.post(f"{root}/buildQueue", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
