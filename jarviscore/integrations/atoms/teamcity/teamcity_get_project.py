...
def teamcity_get_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not project_id: return _tc_dataset([], 400, "project_id is required")
        headers, aerr = _tc_auth(auth_info)
        if aerr: return _tc_dataset([], 401, aerr)
        resp = requests.get(f"{root}/projects/id:{project_id}", headers=headers, timeout=timeout, verify=verify_ssl)
