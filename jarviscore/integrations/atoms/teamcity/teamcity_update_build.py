...
def teamcity_update_build(auth_info: dict, build_id: str, comment: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not build_id: return _tc_provision({}, 400, "build_id is required")
        headers, aerr = _tc_auth(auth_info)
        if aerr: return _tc_provision({}, 401, aerr)
        headers["Content-Type"] = "text/plain"
        resp = requests.put(f"{root}/builds/id:{build_id}/comment", headers=headers, data=comment, timeout=timeout, verify=verify_ssl)
