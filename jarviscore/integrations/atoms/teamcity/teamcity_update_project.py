...
def teamcity_update_project(auth_info: dict, project_id: str, name: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not project_id: return _tc_provision({}, 400, "project_id is required")
        headers, aerr = _tc_auth(auth_info)
        if aerr: return _tc_provision({}, 401, aerr)
        headers["Content-Type"] = "text/plain"
        resp = requests.put(f"{root}/projects/id:{project_id}/name", headers=headers, data=name or project_id, timeout=timeout, verify=verify_ssl)
