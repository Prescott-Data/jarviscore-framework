...
def egnyte_update_file(auth_info: dict, file_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        if not file_id or not payload or not isinstance(payload, dict):
        ...
        data = resp.json() if resp.text else {"path": file_id}
        records = [data] if isinstance(data, dict) else [{"path": file_id}]
        dest = body.get("destination") or body.get("new_name") or file_id
        return {
            "records": records,
