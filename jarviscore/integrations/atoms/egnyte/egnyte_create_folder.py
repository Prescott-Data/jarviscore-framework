...
async def egnyte_create_folder(payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Create folder (POST /pubapi/v1/fs/{path} action=add_folder). Bearer OAuth per Egnyte Public API. Official: https://developers.egnyte.com/docs/read/File_System_Management_API_Documentation"""
    try:
        ...
        resp = await nexus_call('POST', _egnyte_fs_url(api, str(folder_path)), headers=headers, json=body)
        if resp['status_code'] >= 400:
            return {'records': [], 'data_count': 0, 'status': resp['status_code'], 'message': resp['body'][:1000]}
        data = resp['json'] if resp['body'] else {'path': folder_path}
        records = [data] if isinstance(data, dict) else [{'path': folder_path}]
        return {'records': records, 'data_count': len(records), 'status': resp['status_code'], 'message': 'ok', 'provision_ids': _egnyte_provision_id(data, str(folder_path))}
...
def _egnyte_provision_id(data: Any, fallback: str) -> List[str]:
    if isinstance(data, dict):
        for key in ('entry_id', 'folder_id', 'path'):
            val = data.get(key)
            if val not in (None, ''):
                return [str(val)]
    return [fallback]
