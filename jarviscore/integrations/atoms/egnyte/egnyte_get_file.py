...
async def egnyte_get_file(file_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get file metadata (GET /pubapi/v1/fs/{path}). file_id is the Egnyte path. Bearer OAuth per Egnyte Public API. Official: https://developers.egnyte.com/docs/read/File_System_Management_API_Documentation"""
    try:
        if not file_id:
            return {'records': [], 'data_count': 0, 'status': 400, 'message': 'file_id is required (Egnyte file path, e.g. /Shared/docs/report.pdf)'}
        ...
        resp = await nexus_call('GET', _egnyte_fs_url(api, file_id), headers=headers)
