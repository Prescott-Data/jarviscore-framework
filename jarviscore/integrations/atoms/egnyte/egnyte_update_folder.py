...
async def egnyte_update_folder(folder_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Move/copy/rename folder (POST /pubapi/v1/fs/{path} with action). Not PATCH. Bearer OAuth per Egnyte Public API. Official: https://developers.egnyte.com/docs/read/File_System_Management_API_Documentation"""
    try:
        if not folder_id or not payload or (not isinstance(payload, dict)):
        ...
        body = dict(payload)
        body['action'] = action
        resp = await nexus_call('POST', _egnyte_fs_url(api, folder_id), headers=headers, json=body)
        if resp['status_code'] >= 400:
            return {'records': [], 'data_count': 0, 'status': resp['status_code'], 'message': resp['body'][:1000]}
        data = resp['json'] if resp['body'] else {'path': folder_id}
        records = [data] if isinstance(data, dict) else [{'path': folder_id}]
        dest = body.get('destination') or body.get('new_name') or folder_id
        return {'records': records, 'data_count': len(records), 'status': resp['status_code'], 'message': 'ok', 'provision_ids': [str(dest)]}
