...
def egnyte_list_files(auth_info: dict, folder_path: str = "/Shared", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        records, status, message = _egnyte_list_entries(
            api, headers, folder_path, limit, "files", timeout, verify_ssl
        )
        if message != "ok":
