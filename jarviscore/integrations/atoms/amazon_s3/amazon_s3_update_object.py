...
async def amazon_s3_update_object(bucket: str, object_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Replace object content (PUT /{key} — S3 has no PATCH; full overwrite). Official: https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutObject.html"""
    try:
        if not base_url:
            return _s3_provision([], 400, 'base_url is required')
        if not bucket or not object_id:
            return _s3_provision([], 400, 'bucket and object_id are required')
        if not isinstance(payload, dict):
            return _s3_provision([], 400, 'payload must be a dict')
        merged = dict(payload)
        key = merged.get('key') or merged.get('object_key') or object_id
        body = merged.get('body') if 'body' in merged else merged.get('content')
        if body is None and isinstance(merged.get('fields'), dict):
        ...
        headers = {}
        content_type = merged.get('content_type') or merged.get('ContentType')
        if content_type:
            headers['Content-Type'] = str(content_type)
        resp = await _s3_request('PUT', url, region, access_key, secret_key, session_token, headers=headers, data=data, timeout=timeout, verify_ssl=verify_ssl)
