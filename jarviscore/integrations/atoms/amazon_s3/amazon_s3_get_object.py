...
def amazon_s3_get_object(auth_info: dict, bucket: str, object_id: str, destination_path: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get object (GET /{key}, SigV4). Writes binary to destination_path when provided. Official: https://docs.aws.amazon.com/AmazonS3/latest/API/API_GetObject.html"""
    try:
        if not base_url:
            return {"records": [], "data_count": 0, "status": 400, "message": "base_url is required"}
        if not bucket or not object_id:
            return {"records": [], "data_count": 0, "status": 400, "message": "bucket and object_id are required"}
        ...
        key = _pct_enc(object_id.lstrip("/"), safe="/-_.~")
        url = base_url.rstrip("/") + "/" + key
        resp = _s3_request("GET", url, region, access_key, secret_key, session_token, timeout=timeout, verify_ssl=verify_ssl)
        ...
        if destination_path:
            try:
                ...
                return {"records": [], "data_count": 0, "status": 500, "message": str(exc)}
            record = {"key": object_id, "destination_path": destination_path, "size": len(resp.content), "etag": resp.headers.get("ETag")}
            return {"records": [record], "data_count": 1, "status": status, "message": "ok"}
        record = {"key": object_id, "size": len(resp.content), "etag": resp.headers.get("ETag"), "content_type": resp.headers.get("Content-Type")}
        return {"records": [record], "data_count": 1, "status": status, "message": "ok"}
...
# Amazon S3 REST API — https://docs.aws.amazon.com/AmazonS3/latest/API/Type_API_Reference.html
