...
def amazon_s3_list_objects(auth_info: dict, bucket: str, prefix: Optional[str] = None, limit: int = 1000, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        while len(records) < cap and pages < 100:
            ...
            resp = _s3_request("GET", url, region, access_key, secret_key, session_token, headers={"Accept": "application/xml"}, params=params, timeout=timeout, verify_ssl=verify_ssl)
            status = resp.status_code
            if status >= 400:
                return {"records": records, "data_count": len(records), "status": status, "message": resp.text[:1000]}
            batch, truncated, token = _s3_parse_list_xml(resp.text)
            for item in batch:
                records.append(item)
                if len(records) >= cap:
                    break
