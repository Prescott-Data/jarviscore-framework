...
def etsy_list_products(auth_info: dict, shop_id: str, state: str = "active", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List shop listings (GET /v3/application/shops/{shop_id}/listings). Catalog product → Etsy listing. Requires x-api-key (keystring:shared_secret) and OAuth Bearer token per Etsy Open API v3. Official: https://developers.etsy.com/documentation/reference#operation/getListingsByShop"""
    try:
        ...
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        headers, auth_err = _etsy_auth(auth_info)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        url = f"{api}/shops/{sid}/listings"
        params = {"state": state or "active"}
        records, status, message = _etsy_paginate(url, headers, params, limit, timeout, verify_ssl)
        if message != "ok":
