...
async def etsy_update_order(shop_id: str, order_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp = await nexus_call('PUT', f'{api}/shops/{sid}/receipts/{receipt_id}', headers=headers, data=_etsy_form_body(payload))
        if resp['status_code'] >= 400:
            return {'records': [], 'data_count': 0, 'status': resp['status_code'], 'message': resp['body'][:1000]}
        data = resp['json'] if resp['body'] else {}
        records = _etsy_results(data)
        return {'records': records, 'data_count': len(records), 'status': resp['status_code'], 'message': 'ok', 'provision_ids': _etsy_provision_id(data, ('receipt_id',))}
