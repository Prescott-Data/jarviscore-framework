...
async def etsy_get_customer(customer_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get user profile (GET /v3/application/users/{user_id}). customer_id is user_id. Requires x-api-key (keystring:shared_secret) and OAuth Bearer token per Etsy Open API v3. Official: https://developers.etsy.com/documentation/reference#operation/getUser"""
    try:
        if not customer_id:
        ...
        resp = await nexus_call('GET', f'{api}/users/{user_id}', headers=headers)
