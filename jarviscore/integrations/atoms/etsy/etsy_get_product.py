...
async def etsy_get_product(product_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get listing by ID (GET /v3/application/listings/{listing_id}). product_id is listing_id. Requires x-api-key (keystring:shared_secret) and OAuth Bearer token per Etsy Open API v3. Official: https://developers.etsy.com/documentation/reference#operation/getListing"""
    try:
        if not product_id:
        ...
        resp = await nexus_call('GET', f'{api}/listings/{listing_id}', headers=headers)
