...
def zendesk_chat_create_chat(auth_info: dict, name: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        visitor = (auth_info or {}).get("visitor")
        session = (auth_info or {}).get("session")
        timestamp = (auth_info or {}).get("timestamp")
        ...
        resp = requests.post(root + "/chats", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
