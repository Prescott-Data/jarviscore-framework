...
def zendesk_chat_update_chat(auth_info: dict, chat_id: str, name: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not chat_id: return _provision({}, 400, 'chat_id is required')
        headers, aerr = _auth(auth_info)
        if aerr: return _provision({}, 401, aerr)
        headers["Content-Type"] = "application/json"
        payload = {"visitor.name": name} if name else {}
        resp = requests.put(root + "/chats/" + str(chat_id), headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
