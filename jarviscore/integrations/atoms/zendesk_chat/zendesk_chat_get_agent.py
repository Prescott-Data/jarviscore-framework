...
def zendesk_chat_get_agent(auth_info: dict, agent_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """zendesk_chat REST: get agent. Official: https://developer.zendesk.com/api-reference/live-chat/introduction/"""
    try:
        root, err = _root(base_url, auth_info)
        if err: return _dataset([], 400, err)
        if not agent_id: return _dataset([], 400, 'agent_id is required')
        headers, aerr = _auth(auth_info)
        if aerr: return _dataset([], 401, aerr)
        resp = requests.get(root + "/agents/" + str(agent_id), headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _dataset([], resp.status_code, _err(resp))
        data = resp.json() if resp.content else {}
        if isinstance(data, list): records = data
        elif isinstance(data, dict): records = data.get('value') or data.get('items') or data.get('data') or [data]
        else: records = []
