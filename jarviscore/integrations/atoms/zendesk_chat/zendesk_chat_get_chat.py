...
def zendesk_chat_get_chat(auth_info: dict, chat_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not chat_id: return _dataset([], 400, 'chat_id is required')
        headers, aerr = _auth(auth_info)
        if aerr: return _dataset([], 401, aerr)
        resp = requests.get(root + "/chats/" + str(chat_id), headers=headers, timeout=timeout, verify=verify_ssl)
