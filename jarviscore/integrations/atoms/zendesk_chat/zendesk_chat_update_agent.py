...
def zendesk_chat_update_agent(auth_info: dict, agent_id: str, name: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not agent_id: return _provision({}, 400, 'agent_id is required')
        headers, aerr = _auth(auth_info)
        if aerr: return _provision({}, 401, aerr)
        headers["Content-Type"] = "application/json"
        payload = {"display_name": name} if name else {}
        resp = requests.put(root + "/agents/" + str(agent_id), headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _provision({}, resp.status_code, _err(resp))
        data = resp.json() if resp.content else {}
        return _provision(data if isinstance(data, dict) else {}, resp.status_code, 'ok', fallback_id=agent_id)
