...
def twilio_create_message(auth_info: dict, to: str, from_number: str, body: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not to or not from_number or not body: return _tw_provision({}, 400, "to, from_number, and body are required")
        sid, token, aerr = _tw_account(auth_info)
        if aerr: return _tw_provision({}, 401, aerr)
        resp = requests.post(f"{root}/Messages.json", auth=(sid, token), data={"To": to, "From": from_number, "Body": body}, timeout=timeout, verify=verify_ssl)
