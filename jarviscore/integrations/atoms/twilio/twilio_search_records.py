...
def twilio_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Twilio REST: search records. Official: https://www.twilio.com/docs/usage/api"""
    try:
        root, err = _tw_root(base_url, auth_info)
        if err: return _tw_dataset([], 400, err)
        if not query: return _tw_dataset([], 400, "query is required")
        sid, token, aerr = _tw_account(auth_info)
        if aerr: return _tw_dataset([], 401, aerr)
        resp = requests.get(f"{root}/Messages.json", auth=(sid, token), params={"To": query, "PageSize": limit}, timeout=timeout, verify=verify_ssl)
