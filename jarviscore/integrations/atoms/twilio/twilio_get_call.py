...
def twilio_get_call(auth_info: dict, call_sid: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not call_sid: return _tw_dataset([], 400, "call_sid is required")
        sid, token, aerr = _tw_account(auth_info)
        if aerr: return _tw_dataset([], 401, aerr)
        resp = requests.get(f"{root}/Calls/{call_sid}.json", auth=(sid, token), timeout=timeout, verify=verify_ssl)
