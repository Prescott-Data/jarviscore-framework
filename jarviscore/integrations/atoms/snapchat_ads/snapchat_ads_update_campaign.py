...
def snapchat_ads_update_campaign(auth_info: dict, ad_account_id: str, campaign_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Snapchat Marketing API: update campaign. Official: https://marketingapi.snapchat.com/docs/"""
    try:
        if not campaign_id: return _sc_provision({}, 400, "campaign_id is required")
        ...
        # Snapchat full-object update = PUT to the collection with a {"campaigns":[...]}
        # array (id in body); PATCH /campaigns/{id} exists but requires a JSON-Patch body.
        resp = requests.put(f"{root}/adaccounts/{aid}/campaigns", headers=headers, json={"campaigns": [camp]}, timeout=timeout, verify=verify_ssl)
