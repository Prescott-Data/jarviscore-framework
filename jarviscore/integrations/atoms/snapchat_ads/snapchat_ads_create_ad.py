...
def snapchat_ads_create_ad(auth_info: dict, ad_squad_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        ads = payload.get("ads") if isinstance(payload.get("ads"), list) else [payload]
        for ad in ads:
            if isinstance(ad, dict) and not ad.get("ad_squad_id"):
                ad["ad_squad_id"] = ad_squad_id
        body = payload if isinstance(payload.get("ads"), list) else {"ads": ads}
        resp = requests.post(f"{root}/adsquads/{ad_squad_id}/ads", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
