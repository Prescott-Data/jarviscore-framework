...
def snapchat_ads_get_campaign(auth_info: dict, campaign_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not campaign_id: return _sc_dataset([], 400, "campaign_id is required")
        headers, aerr = _sc_auth(auth_info)
        if aerr: return _sc_dataset([], 401, aerr)
        resp = requests.get(f"{root}/campaigns/{campaign_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _sc_dataset([], resp.status_code, _sc_err(resp))
        data = resp.json() if resp.content else {}
        records = _sc_extract_list(data, "campaigns")
        if not records and isinstance(data.get("campaign"), dict):
            records = [data["campaign"]]
