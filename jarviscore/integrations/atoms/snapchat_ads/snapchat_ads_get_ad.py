...
def snapchat_ads_get_ad(auth_info: dict, ad_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not ad_id: return _sc_dataset([], 400, "ad_id is required")
        headers, aerr = _sc_auth(auth_info)
        if aerr: return _sc_dataset([], 401, aerr)
        resp = requests.get(f"{root}/ads/{ad_id}", headers=headers, timeout=timeout, verify=verify_ssl)
