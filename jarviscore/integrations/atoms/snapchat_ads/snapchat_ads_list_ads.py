...
async def snapchat_ads_list_ads(ad_account_id: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        cap = _sc_cap(limit)
        resp = await nexus_call('GET', f'{root}/adaccounts/{aid}/ads', headers=headers, params={'limit': cap})
        if resp['status_code'] >= 400:
            return _sc_dataset([], resp['status_code'], _sc_err(resp))
        data = resp['json'] if resp['content'] else {}
        records = _sc_extract_list(data, 'ads')[:cap]
        records = await _sc_paging(resp, data, cap, records, 'ads')
