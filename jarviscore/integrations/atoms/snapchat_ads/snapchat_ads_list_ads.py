...
def snapchat_ads_list_ads(auth_info: dict, ad_account_id: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Snapchat Marketing API: list ads. Official: https://marketingapi.snapchat.com/docs/"""
    try:
        root, err = _sc_root(base_url)
        if err: return _sc_dataset([], 400, err)
        aid, err = _sc_ad_account(ad_account_id, auth_info)
        if err: return _sc_dataset([], 400, err)
        headers, aerr = _sc_auth(auth_info)
        if aerr: return _sc_dataset([], 401, aerr)
        cap = _sc_cap(limit)
        resp = requests.get(f"{root}/adaccounts/{aid}/ads", headers=headers, params={"limit": cap}, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400: return _sc_dataset([], resp.status_code, _sc_err(resp))
        data = resp.json() if resp.content else {}
        records = _sc_extract_list(data, "ads")[:cap]
        records = _sc_paging(resp, data, cap, records, "ads")
