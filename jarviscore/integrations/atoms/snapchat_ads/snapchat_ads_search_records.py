...
def snapchat_ads_search_records(auth_info: dict, ad_account_id: str, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Snapchat Marketing API: search campaigns and ads. Official: https://marketingapi.snapchat.com/docs/"""
    try:
        if not query: return _sc_dataset([], 400, "query is required")
        ...
        for path in (f"/adaccounts/{aid}/campaigns", f"/adaccounts/{aid}/ads"):
            ...
            if resp.status_code == 401: return _sc_dataset([], 401, _sc_err(resp))
            ...
            for item in _sc_extract_list(data, key):
                if _sc_match(item, query):
                    records.append(item)
                    if len(records) >= cap: return _sc_dataset(records[:cap], 200, "ok")
