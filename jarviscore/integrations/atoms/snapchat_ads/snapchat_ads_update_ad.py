...
async def snapchat_ads_update_ad(ad_squad_id: str, ad_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        if not ad_squad_id:
            return _sc_provision({}, 400, 'ad_squad_id is required')
        if not ad_id:
            return _sc_provision({}, 400, 'ad_id is required')
        ...
        resp = await nexus_call('PUT', f'{root}/adsquads/{ad_squad_id}/ads', headers=headers, json={'ads': [ad]})
        try:
