...
def snapchat_ads_update_ad(auth_info: dict, ad_squad_id: str, ad_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Snapchat Marketing API: update ad. Official: https://marketingapi.snapchat.com/docs/"""
    try:
        if not ad_squad_id: return _sc_provision({}, 400, "ad_squad_id is required")
        if not ad_id: return _sc_provision({}, 400, "ad_id is required")
        ...
        ad = dict(payload)
        ad.setdefault("id", ad_id)
        ad.setdefault("ad_squad_id", ad_squad_id)
        # Snapchat full-object update = PUT to the collection with an {"ads":[...]}
        # array (id in body); PATCH /ads/{id} exists but requires a JSON-Patch body.
        resp = requests.put(f"{root}/adsquads/{ad_squad_id}/ads", headers=headers, json={"ads": [ad]}, timeout=timeout, verify=verify_ssl)
        ...
        obj = records[0] if records else (data.get("ad") if isinstance(data, dict) else {})
        if not isinstance(obj, dict): obj = {}
        return _sc_provision(obj, resp.status_code, "ok", fallback_id=obj.get("id") or ad_id)
