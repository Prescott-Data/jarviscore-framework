...
def what3words_list_items(auth_info: dict, input_text: str, limit: int = 5, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not input_text: return _w3w_dataset([], 400, "input_text is required")
        resp = requests.get(f"{root}/autosuggest", params={"input": input_text, "n-results": limit, "key": key}, timeout=timeout, verify=verify_ssl)
