...
def what3words_get_record(auth_info: dict, lat: str, lng: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """what3words Public API: Convert coordinates to 3wa. Official: https://developer.what3words.com/public-api/docs"""
    ...
    try:
        ...
        if not lat or not lng: return _w3w_dataset([], 400, "lat and lng are required")
        resp = requests.get(f"{root}/convert-to-3wa", params={"coordinates": f"{lat},{lng}", "key": key}, timeout=timeout, verify=verify_ssl)
