...
async def what3words_list_records(bounding_box: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if not bounding_box:
            return _w3w_dataset([], 400, 'bounding_box is required (south_lat,west_lng,north_lat,east_lng)')
        resp = await nexus_call('GET', f'{root}/grid-section', params={'bounding-box': bounding_box, 'key': key})
