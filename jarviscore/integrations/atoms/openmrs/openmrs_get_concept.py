async def openmrs_get_concept(concept_uuid: str) -> dict:
    """Get concept via the openmrs API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    c = await _get(f'/concept/{concept_uuid}')
    return {'uuid': c.get('uuid'), 'name': c.get('name', {}).get('display'), 'datatype': c.get('datatype', {}).get('display'), 'concept_class': c.get('conceptClass', {}).get('display'), 'descriptions': [d.get('description') for d in c.get('descriptions', [])], 'answers': [a.get('display') for a in c.get('answers', [])]}
