...
def tawkto_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not query: return _tk_dataset([], 400, "query is required")
        headers, aerr = _tk_auth(auth_info)
        if aerr: return _tk_dataset([], 401, aerr)
        key = str((auth_info or {}).get("api_key") or "").strip()
        resp = requests.post(f"{root}/v1/chat.list", headers=headers, auth=(key, ""), json={"propertyId": pid, "size": 100}, timeout=timeout, verify=verify_ssl)
        ...
        q = query.lower()
        matched = [r for r in records if isinstance(r, dict) and any(q in str(r.get(k,"")).lower() for k in ("id","visitor","email","name"))]
        return _tk_dataset(matched[:limit], resp.status_code, "ok")
