...
# POST /identify — https://www.docs.developers.amplitude.com/analytics/apis/identify-api/
# Auth: api_key in form body (project API key)


def amplitude_create_user(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        ident = payload.get("identification") if "identification" in payload else payload
        ...
        try:
            data = resp.json() if resp.content else {"raw": resp.text[:1000]}
        except Exception:
        ...
        if isinstance(ident, list) and ident and isinstance(ident[0], dict):
            uid = ident[0].get("user_id") or ident[0].get("device_id")
        rec = data if isinstance(data, dict) else {}
        ids = [str(uid)] if uid not in (None, "") else []
