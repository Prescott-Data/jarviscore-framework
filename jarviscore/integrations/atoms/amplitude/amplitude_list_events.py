...
def amplitude_list_events(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        data, status, msg = _amp_get_json(f"{api_root}/events/list", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
