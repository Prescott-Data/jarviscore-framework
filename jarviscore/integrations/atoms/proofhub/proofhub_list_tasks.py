...
def proofhub_list_tasks(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        cap = _ph_cap(limit)
        project_id, todolist_id, _ = _ph_ids(auth_info)
        if project_id and todolist_id:
            records, status, msg = _ph_fetch_list_tasks(root, auth_info, project_id, todolist_id, cap, timeout, verify_ssl)
        else:
            records, status, msg = _ph_collect_tasks(root, auth_info, cap, timeout, verify_ssl, project_id=project_id)
