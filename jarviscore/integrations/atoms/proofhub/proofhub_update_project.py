...
def proofhub_update_project(auth_info: dict, project_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a project. Official: https://github.com/ProofHub/api_v3/blob/master/README.md"""
    try:
        if not project_id:
        ...
        resp, body, status, msg = _ph_request("put", root + "/projects/" + str(project_id), auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _ph_provision(body if isinstance(body, dict) else {}, status, msg, fallback_id=project_id)
        return _ph_provision(body if isinstance(body, dict) else {}, status, "ok", fallback_id=project_id)
    except Exception as e:
        return _ph_provision({}, 500, str(e), fallback_id=project_id)
