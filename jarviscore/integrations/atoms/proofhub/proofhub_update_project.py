...
async def proofhub_update_project(project_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = await _ph_request('put', root + '/projects/' + str(project_id), json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
