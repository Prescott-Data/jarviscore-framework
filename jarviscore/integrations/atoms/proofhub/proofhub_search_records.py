...
def proofhub_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        for row in _ph_rows(body):
            ...
            if len(out) >= cap:
                break
        if len(out) < cap:
            tasks, tstatus, tmsg = _ph_collect_tasks(root, auth_info, cap * 3, timeout, verify_ssl)
