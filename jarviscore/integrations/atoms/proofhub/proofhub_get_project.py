...
def proofhub_get_project(auth_info: dict, project_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a project by ID. Official: https://github.com/ProofHub/api_v3/blob/master/README.md"""
    try:
        if not project_id:
        ...
        resp, body, status, msg = _ph_request("get", root + "/projects/" + str(project_id), auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
