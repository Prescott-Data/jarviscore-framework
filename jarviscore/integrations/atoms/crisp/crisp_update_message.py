...
def crisp_update_message(auth_info: dict, message_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a message in crisp. HTTP Basic site_id:api_key. Official: https://docs.crisp.chat/references/rest-api/v1/"""
    try:
        if not message_id or not payload:
            return {"records": [], "data_count": 0, "status": 400, "message": "message_id and payload are required"}
        ...
        session_id, err = _crisp_session_id(auth_info, payload)
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        api = _crisp_api_root(base_url)
        url = f"{api}/website/{site_id}/conversation/{session_id}/message/{message_id}"
        resp = _crisp_patch(url, headers, basic, payload, timeout, verify_ssl)
        ...
        prov = _crisp_provision_ids(records[0]) if records else [message_id]
