...
def crisp_get_message(auth_info: dict, message_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a message by fingerprint from crisp. HTTP Basic site_id:api_key. Official: https://docs.crisp.chat/references/rest-api/v1/"""
    try:
        if not message_id:
            return {"records": [], "data_count": 0, "status": 400, "message": "message_id is required"}
        headers, basic, site_id = _crisp_auth(auth_info)
