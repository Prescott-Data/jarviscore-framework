def oracle_erp_list_invoices(auth_info: dict, instance_url: str, limit: int = 25, offset: int = 0) -> list:
    import requests
    ...
    def _delete(p, headers=None):
        ...
        _r.raise_for_status()
        return _r.json() if _r.content else {}
    resp = _get("/invoices", params={"limit": limit, "offset": offset})
    return [
        {
            "id": i.get("InvoiceId"),
            "number": i.get("InvoiceNumber"),
            "type": i.get("InvoiceType"),
            "supplier": i.get("Supplier"),
            "amount": i.get("InvoiceAmount"),
            "currency": i.get("InvoiceCurrencyCode"),
            "date": i.get("InvoiceDate"),
            "due_date": i.get("PaymentDueDate"),
