async def oracle_erp_list_expenses(instance_url: str, limit: int=25, offset: int=0) -> list:
    """Erp list expenses via the oracle_erp API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/expenseReports', params={'limit': limit, 'offset': offset})
    return [{'id': e.get('ExpenseReportId'), 'number': e.get('ReportNumber'), 'purpose': e.get('Purpose'), 'status': e.get('ExpenseStatusCode'), 'amount': e.get('TotalAmount'), 'currency': e.get('ReimbursementCurrencyCode'), 'submitted_by': e.get('EmployeeName'), 'submitted_date': e.get('SubmissionDate'), 'approved_date': e.get('ApprovedDate')} for e in resp.get('items', [])]
