async def oracle_erp_list_suppliers(instance_url: str, limit: int=25, offset: int=0) -> list:
    """Erp list suppliers via the oracle_erp API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/suppliers', params={'limit': limit, 'offset': offset})
    return [{'id': s.get('SupplierId'), 'name': s.get('Supplier'), 'number': s.get('SupplierNumber'), 'type': s.get('SupplierType'), 'status': s.get('Status'), 'tax_id': s.get('TaxpayerIdentificationNumber'), 'payment_terms': s.get('PaymentTerms'), 'created_at': s.get('CreationDate')} for s in resp.get('items', [])]
