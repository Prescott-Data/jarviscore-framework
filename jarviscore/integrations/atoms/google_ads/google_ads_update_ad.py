...
def google_ads_update_ad(auth_info: dict, customer_id: str, ad_id: str, payload: Dict[str, Any], resource_name: Optional[str] = None, update_mask: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update a ad in google ads. Official: https://developers.google.com/google-ads/api/docs/rest/common/search"""
    try:
        ...
        if isinstance(payload.get("operations"), list):
            ...
            if not rn and ad_id:
                ...
                rn = aga.get("resourceName") or aga.get("resource_name")
            if not rn:
...
def _gads_search(
    ...
    while len(records) < cap:
        body: Dict[str, Any] = {"query": query}  # pageSize deprecated; API returns 10k rows/page
        if page_token:
