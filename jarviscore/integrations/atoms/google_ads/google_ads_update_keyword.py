...
# Keywords are ad_group_criterion resources of type KEYWORD.
_GADS_API_ROOT = "https://googleads.googleapis.com/v24"


def google_ads_update_keyword(auth_info: dict, customer_id: str, criterion_id: str, payload: Dict[str, Any], resource_name: Optional[str] = None, update_mask: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if isinstance(payload.get("operations"), list):
            ...
            rn = resource_name or payload.get("resourceName") or payload.get("resource_name")
            if not rn and criterion_id:
                rn, status, msg = _gads_resolve_criterion(api, cid, headers, criterion_id, timeout, verify_ssl)
                if not rn:
            ...
            op: Dict[str, Any] = {"update": {"resourceName": rn, **update_fields}}
            ...
            body = {"operations": [op]}
        data, status, msg = _gads_mutate(api, cid, "adGroupCriteria", headers, body, timeout, verify_ssl)
...
def _gads_resolve_criterion(api, cid, headers, criterion_id, timeout, verify_ssl):
    query = (
