...
def google_ads_get_ad(auth_info: dict, customer_id: str, ad_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a ad by ID from google ads. Official: https://developers.google.com/google-ads/api/docs/rest/common/search"""
    try:
        ...
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        if not ad_id:
            return {"records": [], "data_count": 0, "status": 400, "message": "ad_id is required"}
        ...
        query = (
            ...
            "ad_group_ad.resource_name, ad_group.id, campaign.id "
            f"FROM ad_group_ad WHERE ad_group_ad.ad.id = {ad_id}"
        )
        records, status, msg = _gads_search(api, cid, headers, query, 1, timeout, verify_ssl)
