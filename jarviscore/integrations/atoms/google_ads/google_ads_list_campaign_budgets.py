...
async def google_ads_list_campaign_budgets(customer_id: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        query = f'SELECT campaign_budget.id, campaign_budget.name, campaign_budget.amount_micros, campaign_budget.delivery_method, campaign_budget.status, campaign_budget.resource_name FROM campaign_budget ORDER BY campaign_budget.id LIMIT {cap}'
