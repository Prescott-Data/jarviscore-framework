...
async def google_ads_list_campaigns(customer_id: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List campaigns via GAQL googleAds:search. Requires OAuth Bearer, developer-token, customer_id. Official: https://developers.google.com/google-ads/api/docs/rest/common/search"""
    try:
        ...
        query = f'SELECT campaign.id, campaign.name, campaign.status, campaign.resource_name, campaign.advertising_channel_type FROM campaign ORDER BY campaign.id LIMIT {cap}'
