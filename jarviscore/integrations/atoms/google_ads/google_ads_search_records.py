...
def google_ads_search_records(auth_info: dict, customer_id: str, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Run arbitrary GAQL query via googleAds:search. Official: https://developers.google.com/google-ads/api/docs/rest/common/search"""
    try:
        ...
        if err:
            return {"records": [], "data_count": 0, "status": 400, "message": err}
        if not query:
            return {"records": [], "data_count": 0, "status": 400, "message": "query is required (GAQL)"}
        headers, auth_err = _gads_auth(auth_info, json_body=True)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        records, status, msg = _gads_search(api, cid, headers, query, limit, timeout, verify_ssl)
