...
async def google_ads_list_ad_groups(customer_id: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List a ad group in google ads. Official: https://developers.google.com/google-ads/api/docs/rest/common/search"""
    try:
        ...
        query = f'SELECT ad_group.id, ad_group.name, ad_group.status, ad_group.type, ad_group.campaign, ad_group.resource_name FROM ad_group ORDER BY ad_group.id LIMIT {cap}'
