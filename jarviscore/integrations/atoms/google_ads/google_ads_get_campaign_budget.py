...
async def google_ads_get_campaign_budget(customer_id: str, budget_id: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get a campaign budget in google ads. Official: https://developers.google.com/google-ads/api/docs/rest/common/search"""
    try:
        ...
        if err:
            return {'records': [], 'data_count': 0, 'status': 400, 'message': err}
        if not budget_id:
            return {'records': [], 'data_count': 0, 'status': 400, 'message': 'budget_id is required'}
        (headers, auth_err) = _gads_auth(json_body=True)
        if auth_err:
            return {'records': [], 'data_count': 0, 'status': 401, 'message': auth_err}
        query = f'SELECT campaign_budget.id, campaign_budget.name, campaign_budget.amount_micros, campaign_budget.delivery_method, campaign_budget.status, campaign_budget.type, campaign_budget.resource_name FROM campaign_budget WHERE campaign_budget.id = {budget_id}'
