...
def prembly_get_record(auth_info: dict, record_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get verification status by verification_id. Official: https://docs.prembly.com/docs/get-verification-status"""
    try:
        if not record_id:
            return _pm_dataset([], 400, "record_id is required")
        root, err = _pm_root(base_url, auth_info)
        if err:
            return _pm_dataset([], 400, err)
        resp, body, status, msg = _pm_request("get", root + "/verification/" + str(record_id).strip() + "/status", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _pm_dataset([], status, msg)
        rows = _pm_rows(body)
        if isinstance(body, dict) and body.get("data") and not rows:
            rows = [body["data"]] if isinstance(body.get("data"), dict) else rows
        return _pm_dataset(rows if rows else ([body] if isinstance(body, dict) else []), status, msg)
