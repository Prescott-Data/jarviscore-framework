...
def prembly_create_item(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _pm_request("post", root + "/api/v1/api/bgc/packages/", auth_info, json_body=payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
