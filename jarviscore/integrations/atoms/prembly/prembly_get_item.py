...
def prembly_get_item(auth_info: dict, item_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp, body, status, msg = _pm_request("get", root + path, auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _pm_dataset([], status, msg)
        rows = _pm_rows(body)
        if not rows:
            rows = [x for x in _pm_rows(body) if str(x.get("id", "")) == check_id]
        return _pm_dataset(rows[:1] if rows else [], status, msg)
