...
def prembly_list_records(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List custom background-check packages. Official: https://docs.prembly.com/reference/list-custom-packages"""
    try:
        root, err = _pm_root(base_url, auth_info)
        if err:
            return _pm_dataset([], 400, err)
        resp, body, status, msg = _pm_request("get", root + "/api/v1/api/bgc/packages/", auth_info, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
