...
async def prembly_list_records(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        (resp, body, status, msg) = await _pm_request('get', root + '/api/v1/api/bgc/packages/', timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
