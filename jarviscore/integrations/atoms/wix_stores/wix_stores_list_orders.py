...
def wix_stores_list_orders(auth_info: dict, site_id: str = "", limit: int = 100, cursor: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        paging: Dict[str, Any] = {"limit": limit}
        if cursor:
            paging["cursor"] = cursor
        body = {"search": {"cursorPaging": paging}}
        resp = requests.post(f"{root}/ecom/v1/orders/search", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
