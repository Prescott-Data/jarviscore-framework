...
def wix_stores_get_order(auth_info: dict, order_id: str, site_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wix_stores API: get order. Official: https://dev.wix.com/docs/rest/business-solutions/stores"""
    try:
        if not order_id:
            return _wx_dataset([], 400, "order_id is required")
        ...
        resp = requests.get(f"{root}/ecom/v1/orders/{order_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _wx_dataset([], resp.status_code, _wx_err(resp))
        return _wx_dataset(_wx_obj(resp.json() if resp.content else {}, "order"), resp.status_code, "ok")
