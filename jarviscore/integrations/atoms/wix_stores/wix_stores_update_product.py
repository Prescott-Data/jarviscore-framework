...
def wix_stores_update_product(auth_info: dict, product_id: str, site_id: str = "", name: str = "", price: Optional[float] = None, description: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wix_stores API: update product. Official: https://dev.wix.com/docs/rest/business-solutions/stores"""
    try:
        if not product_id:
            return _wx_provision({}, 400, "product_id is required")
        ...
        if price is not None:
            product["priceData"] = {"price": price}
        if description:
            product["description"] = description
        resp = requests.patch(f"{root}/stores/v1/products/{product_id}", headers=headers, json={"product": product}, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _wx_provision({}, resp.status_code, _wx_err(resp))
        return _wx_provision(resp.json() if resp.content else {}, resp.status_code, "ok", key="product", fallback_id=product_id)
