...
def wix_stores_get_customer(auth_info: dict, contact_id: str, site_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """wix_stores API: get customer. Official: https://dev.wix.com/docs/rest/business-solutions/stores"""
    try:
        if not contact_id:
        ...
        resp = requests.get(f"{root}/contacts/v4/contacts/{contact_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _wx_dataset([], resp.status_code, _wx_err(resp))
        return _wx_dataset(_wx_obj(resp.json() if resp.content else {}, "contact"), resp.status_code, "ok")
