...
def monday_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        board_id, berr = _md_board_id(auth_info)
        if berr:
        ...
        query_params = {
            "rules": [
                {
                    "column_id": "name",
                    "compare_value": [str(query)],
                    "operator": "contains_text",
                }
            ]
        }
        records, status, msg = _md_items_page(base, headers, board_id, limit, query_params, timeout, verify_ssl)
