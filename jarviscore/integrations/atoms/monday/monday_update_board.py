...
def monday_update_board(auth_info: dict, board_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update board attribute via GraphQL update_board mutation. Official: https://developer.monday.com/api-reference/reference/boards"""
    try:
        if not board_id:
        ...
        attr = body.get("board_attribute") or body.get("attribute")
        val = body.get("new_value")
        if body.get("name") not in (None, ""):
            attr = attr or "name"
            val = val if val not in (None, "") else body.get("name")
        if body.get("description") not in (None, "") and not val:
            attr = attr or "description"
            val = body.get("description")
        if not attr or val in (None, ""):
        ...
        resp = _md_gql(base, headers, q, {"board_id": str(board_id), "board_attribute": str(attr), "new_value": str(val)}, timeout, verify_ssl)
