...
async def insightly_update_account(account_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Update Insightly account. Official: https://api.na1.insightly.com/v3.1/Help"""
    try:
        if not account_id:
        ...
        body = dict(payload)
        try:
            body['ORGANISATION_ID'] = int(account_id)
        except (TypeError, ValueError):
            body['ORGANISATION_ID'] = account_id
        resp = await _in_put(f'{api}/Organisations', headers, basic, body, timeout, verify_ssl)
        if resp['status_code'] >= 400:
            return {'records': [], 'data_count': 0, 'status': resp['status_code'], 'message': resp['body'][:1000]}
        records = _in_single(resp['json'] if resp['body'] else {})
...
async def _in_put(url, headers, basic, body, timeout, verify_ssl):
