...
# Get application — GET /api/v1/apps/{appId}. Official: https://developer.okta.com/docs/reference/api/apps/


def okta_get_app(auth_info: dict, domain: str, app_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Retrieve application by id. Official: https://developer.okta.com/docs/reference/api/overview/"""
    try:
        if not app_id:
        ...
        resp = requests.get(f"{root}/apps/{app_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        ...
        try:
            ...
            return _ok_dataset([], status, resp.text[:1000])
