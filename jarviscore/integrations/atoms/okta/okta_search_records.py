...
# Search users/groups/apps — GET with q or search params. Official: https://developer.okta.com/docs/reference/api/users/


def okta_search_records(auth_info: dict, domain: str, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search users/groups/apps via q or search param. Official: https://developer.okta.com/docs/reference/api/overview/"""
    try:
        if not query:
            return _ok_dataset([], 400, "query is required")
        root, err = _ok_root(base_url, domain, auth_info)
        if err:
            return _ok_dataset([], 400, err)
        headers, aerr = _ok_auth(auth_info)
        if aerr:
            return _ok_dataset([], 401, aerr)
        auth_info = auth_info or {}
        resource = str(auth_info.get("resource") or auth_info.get("search_type") or "users").lower()
        if resource in ("group", "groups"):
            path = "groups"
        elif resource in ("app", "apps", "application", "applications"):
            path = "apps"
        else:
            path = "users"
        params = {}
        if auth_info.get("search"):
            params["search"] = str(auth_info.get("search"))
        elif auth_info.get("filter"):
            params["filter"] = str(auth_info.get("filter"))
        else:
            params["q"] = str(query)
        records, status, msg = _ok_paginate(f"{root}/{path}", headers, params, limit, timeout, verify_ssl)
