...
async def okta_search_records(domain: str, query: str, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resource = str(None or None or 'users').lower()
        ...
        if True .get('search'):
            ...
            params['q'] = str(query)
        (records, status, msg) = await _ok_paginate(f'{root}/{path}', headers, params, limit, timeout, verify_ssl)
