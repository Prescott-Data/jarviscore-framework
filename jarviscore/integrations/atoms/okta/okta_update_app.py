...
# Replace application — PUT /api/v1/apps/{appId}. Official: https://developer.okta.com/docs/api/openapi/okta-management/management/tags/application/other/replaceapplication


def okta_update_app(auth_info: dict, domain: str, app_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Replace application (full PUT). Official: https://developer.okta.com/docs/reference/api/overview/"""
    try:
        if not app_id:
            return _ok_provision({}, 400, "app_id is required")
        ...
        resp = requests.put(f"{root}/apps/{app_id}", headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
        ...
        return _ok_provision(data if isinstance(data, dict) else {}, status, "ok", fallback_id=app_id)
