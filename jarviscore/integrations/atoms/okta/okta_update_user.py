...
# Update user — POST /api/v1/users/{userId} (partial update). Official: https://developer.okta.com/docs/api/openapi/okta-management/management/tags/user/other/updateuser


def okta_update_user(auth_info: dict, domain: str, user_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Partial user update (POST not PUT). Official: https://developer.okta.com/docs/reference/api/overview/"""
    try:
        if not user_id:
            return _ok_provision({}, 400, "user_id is required")
        ...
        resp = requests.post(f"{root}/users/{user_id}", headers=headers, json=payload, timeout=timeout, verify=verify_ssl)
        ...
        return _ok_provision(data if isinstance(data, dict) else {}, status, "ok", fallback_id=user_id)
