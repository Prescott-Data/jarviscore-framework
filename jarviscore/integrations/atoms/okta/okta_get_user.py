...
# Get user — GET /api/v1/users/{userId}. Official: https://developer.okta.com/docs/reference/api/users/


def okta_get_user(auth_info: dict, domain: str, user_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.get(f"{root}/users/{user_id}", headers=headers, timeout=timeout, verify=verify_ssl)
