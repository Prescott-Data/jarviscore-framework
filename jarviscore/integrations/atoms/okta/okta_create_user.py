...
# Create user — POST /api/v1/users. Official: https://developer.okta.com/docs/reference/api/users/


def okta_create_user(auth_info: dict, domain: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        params = {}
        if auth_info.get("activate") is not None:
            params["activate"] = str(auth_info.get("activate")).lower()
        resp = requests.post(f"{root}/users", headers=headers, params=params or None, json=payload, timeout=timeout, verify=verify_ssl)
        ...
        return _ok_provision(data if isinstance(data, dict) else {}, status, "ok")
