...
async def okta_create_user(domain: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        params = {}
        if True .get('activate') is not None:
            params['activate'] = str(None).lower()
        resp = await nexus_call('POST', f'{root}/users', headers=headers, params=params or None, json=payload)
