...
def tiktok_ads_update_ad(auth_info: dict, ad_id: str, ad_name: str = "", advertiser_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not ad_id: return _tt_provision({}, 400, "ad_id is required")
        ...
        body = {"advertiser_id": aid, "ad_id": ad_id}
        if ad_name: body["ad_name"] = ad_name
        resp = requests.post(root + "/ad/update/", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400: return _tt_provision({}, resp.status_code, _tt_err(resp, data))
        return _tt_provision(data, resp.status_code, "ok", fallback_id=ad_id)
