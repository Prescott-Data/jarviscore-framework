...
def tiktok_ads_update_campaign(auth_info: dict, campaign_id: str, campaign_name: str = "", advertiser_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not campaign_id: return _tt_provision({}, 400, "campaign_id is required")
        ...
        body = {"advertiser_id": aid, "campaign_id": campaign_id}
        if campaign_name: body["campaign_name"] = campaign_name
        resp = requests.post(root + "/campaign/update/", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400: return _tt_provision({}, resp.status_code, _tt_err(resp, data))
        return _tt_provision(data, resp.status_code, "ok", fallback_id=campaign_id)
