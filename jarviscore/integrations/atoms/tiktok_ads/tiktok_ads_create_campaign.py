...
def tiktok_ads_create_campaign(auth_info: dict, campaign_name: str, advertiser_id: str = '', timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not campaign_name: return _tt_provision({}, 400, "campaign_name is required")
        ...
        body = {"advertiser_id": aid, "campaign_name": campaign_name, "objective_type": "TRAFFIC", "budget_mode": "BUDGET_MODE_INFINITE"}
        resp = requests.post(root + "/campaign/create/", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
