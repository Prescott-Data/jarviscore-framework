...
def tiktok_ads_list_ads(auth_info: dict, advertiser_id: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.get(root + "/ad/get/", headers=headers, params={"advertiser_id": aid, "page_size": limit}, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400: return _tt_dataset([], resp.status_code, _tt_err(resp, data))
        return _tt_dataset(_tt_list(data, "ads"), resp.status_code, "ok")
