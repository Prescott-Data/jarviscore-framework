...
def tiktok_ads_get_campaign(auth_info: dict, campaign_id: str, advertiser_id: str = '', timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """TikTok Marketing API: Get campaign. Official: https://business-api.tiktok.com/portal/docs"""
    try:
        root, err = _tt_root(base_url, auth_info)
        if err: return _tt_dataset([], 400, err)
        if not campaign_id: return _tt_dataset([], 400, 'campaign_id is required')
        aid, err = _tt_advertiser(advertiser_id, auth_info)
        if err: return _tt_dataset([], 400, err)
        headers, aerr = _tt_auth(auth_info)
        if aerr: return _tt_dataset([], 401, aerr)
        resp = requests.get(root + '/campaign/get/', headers=headers, params={'advertiser_id': aid, 'filtering': '{"campaign_ids": ["' + str(campaign_id) + '"]}'}, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400: return _tt_dataset([], resp.status_code, _tt_err(resp, data))
        items = _tt_list(data, 'campaigns')
        rec = next((x for x in items if str(x.get('campaign_id'))==str(campaign_id)), items[0] if items else {})
        return _tt_dataset([rec] if rec else [], resp.status_code, 'ok')
    except Exception as e: return _tt_dataset([], 500, str(e))
