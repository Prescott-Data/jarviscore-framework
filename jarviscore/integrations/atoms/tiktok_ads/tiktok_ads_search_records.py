...
def tiktok_ads_search_records(auth_info: dict, query: str, advertiser_id: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not query: return _tt_dataset([], 400, "query is required")
        aid, err = _tt_advertiser(advertiser_id, auth_info)
        if err: return _tt_dataset([], 400, err)
        headers, aerr = _tt_auth(auth_info)
        if aerr: return _tt_dataset([], 401, aerr)
        resp = requests.get(root + "/campaign/get/", headers=headers, params={"advertiser_id": aid, "page_size": 100}, timeout=timeout, verify=verify_ssl)
