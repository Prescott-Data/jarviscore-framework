...
def tiktok_ads_list_campaigns(auth_info: dict, advertiser_id: str = '', limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """TikTok Marketing API: List campaigns. Official: https://business-api.tiktok.com/portal/docs"""
    try:
        ...
        resp = requests.get(root + '/campaign/get/', headers=headers, params={'advertiser_id': aid, 'page_size': limit}, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400 or (isinstance(data, dict) and data.get('code') not in (0, None)): return _tt_dataset([], resp.status_code, _tt_err(resp, data))
        return _tt_dataset(_tt_list(data, 'campaigns'), resp.status_code, 'ok')
