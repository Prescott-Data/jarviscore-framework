...
def tiktok_ads_get_ad(auth_info: dict, ad_id: str, advertiser_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """TikTok Marketing API: Get ad. Official: https://business-api.tiktok.com/portal/docs"""
    try:
        root, err = _tt_root(base_url, auth_info)
        if err: return _tt_dataset([], 400, err)
        if not ad_id: return _tt_dataset([], 400, "ad_id is required")
        aid, err = _tt_advertiser(advertiser_id, auth_info)
        if err: return _tt_dataset([], 400, err)
        headers, aerr = _tt_auth(auth_info)
        if aerr: return _tt_dataset([], 401, aerr)
        resp = requests.get(root + "/ad/get/", headers=headers, params={"advertiser_id": aid, "filtering": '{"ad_ids": ["' + str(ad_id) + '"]}'}, timeout=timeout, verify=verify_ssl)
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400: return _tt_dataset([], resp.status_code, _tt_err(resp, data))
        items = _tt_list(data, "ads")
        rec = next((x for x in items if str(x.get("ad_id"))==str(ad_id)), items[0] if items else {})
