...
def jenkins_update_pipeline(auth_info: dict, job_name: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not config: return {"records": [], "data_count": 0, "status": 400, "message": "payload.config_xml is required"}
        ...
        resp = _jk_post(f"{base}{path}/config.xml", basic, headers, config, timeout, verify_ssl)
