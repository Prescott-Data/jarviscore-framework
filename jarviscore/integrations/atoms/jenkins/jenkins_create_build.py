...
def jenkins_create_build(auth_info: dict, job_name: str, timeout: int = 30, verify_ssl: bool = True, payload: Optional[Dict[str, Any]] = None, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = _jk_post(endpoint, basic, headers, data, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        info_resp = _jk_get(f"{base}{path}/api/json", basic, {"tree": "lastBuild[number,url,result]"}, timeout, verify_ssl)
        build = info_resp.json().get("lastBuild") if info_resp.text else {}
