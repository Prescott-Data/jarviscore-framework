...
def jenkins_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = _jk_get(f"{base}/api/json", basic, {"tree": "jobs[name,url,color,_class]"}, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        needle = query.lower()
        matched = [j for j in _jk_jobs(resp.json() if resp.text else {}) if isinstance(j, dict) and needle in str(j.get("name", "")).lower()][:limit]
