...
def jenkins_list_builds(auth_info: dict, job_name: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = _jk_get(f"{base}{path}/api/json", basic, {"tree": "builds[number,url,result,timestamp]"}, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        records = _jk_builds(resp.json() if resp.text else {})[:limit]
