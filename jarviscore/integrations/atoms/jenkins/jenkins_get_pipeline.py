...
async def jenkins_get_pipeline(job_name: str, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """Get Jenkins pipeline job details. Official: https://www.jenkins.io/doc/book/using/remote-access-api/"""
    try:
        ...
        resp = await _jk_get(f'{base}{path}/api/json', basic, None, timeout, verify_ssl)
        if resp['status_code'] >= 400:
            return {'records': [], 'data_count': 0, 'status': resp['status_code'], 'message': resp['body'][:1000]}
        records = _jk_single_job(resp['json'] if resp['body'] else {})
