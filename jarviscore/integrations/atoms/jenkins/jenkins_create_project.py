...
def jenkins_create_project(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        if not name: return {"records": [], "data_count": 0, "status": 400, "message": "payload.name is required"}
        ...
        if not config:
            config = """<?xml version='1.0' encoding='UTF-8'?><project><builders/><publishers/><buildWrappers/></project>"""
        resp = _jk_post(f"{base}/createItem?name={name}", basic, headers, config, timeout, verify_ssl)
