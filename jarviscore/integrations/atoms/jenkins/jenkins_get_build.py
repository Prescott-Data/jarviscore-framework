...
async def jenkins_get_build(job_name: str, build_number: int, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        if build_number in (None, ''):
            return {'records': [], 'data_count': 0, 'status': 400, 'message': 'build_number is required'}
        basic, auth_err = _jk_auth()
        if auth_err:
            return {'records': [], 'data_count': 0, 'status': 401, 'message': auth_err}
        resp = await _jk_get(f'{base}{path}/{build_number}/api/json', basic, None, timeout, verify_ssl)
        if resp['status_code'] >= 400:
            return {'records': [], 'data_count': 0, 'status': resp['status_code'], 'message': resp['body'][:1000]}
        records = _jk_single_build(resp['json'] if resp['body'] else {})
