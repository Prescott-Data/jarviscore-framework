...
def jenkins_get_build(auth_info: dict, job_name: str, build_number: int, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get Jenkins build details. Official: https://www.jenkins.io/doc/book/using/remote-access-api/"""
    try:
        ...
        if build_number in (None, ""): return {"records": [], "data_count": 0, "status": 400, "message": "build_number is required"}
        basic, auth_err = _jk_auth(auth_info)
        if auth_err:
            return {"records": [], "data_count": 0, "status": 401, "message": auth_err}
        resp = _jk_get(f"{base}{path}/{build_number}/api/json", basic, None, timeout, verify_ssl)
        if resp.status_code >= 400:
            return {"records": [], "data_count": 0, "status": resp.status_code, "message": resp.text[:1000]}
        records = _jk_single_build(resp.json() if resp.text else {})
