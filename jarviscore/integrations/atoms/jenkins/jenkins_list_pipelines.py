...
def jenkins_list_pipelines(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        jobs = [j for j in _jk_jobs(resp.json() if resp.text else {}) if isinstance(j, dict) and "workflow" in str(j.get("_class", "")).lower()]
