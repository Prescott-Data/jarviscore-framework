...
def xero_get_payment(auth_info: dict, payment_id: str, tenant_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """xero REST: get payment. Official: https://developer.xero.com/documentation/api/accounting/overview"""
    try:
        if not payment_id:
        ...
        resp = requests.get(f"{root}/Payments/{payment_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _x_dataset([], resp.status_code, _x_err(resp))
        return _x_dataset(_x_records(resp.json() if resp.content else {}, "Payments"), resp.status_code, "ok")
