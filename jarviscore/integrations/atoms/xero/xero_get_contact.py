...
def xero_get_contact(auth_info: dict, contact_id: str, tenant_id: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """xero REST: get contact. Official: https://developer.xero.com/documentation/api/accounting/overview"""
    try:
        if not contact_id:
            return _x_dataset([], 400, "contact_id is required")
        root, err = _x_root(base_url, auth_info)
        if err:
            return _x_dataset([], 400, err)
        headers, aerr = _x_headers(auth_info, tenant_id)
        if aerr:
            return _x_dataset([], 401, aerr)
        resp = requests.get(f"{root}/Contacts/{contact_id}", headers=headers, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _x_dataset([], resp.status_code, _x_err(resp))
        return _x_dataset(_x_records(resp.json() if resp.content else {}, "Contacts"), resp.status_code, "ok")
