...
# Xero Accounting API 2.0 — Official: https://developer.xero.com/documentation/api/accounting/payments
...
def xero_create_payment(auth_info: dict, invoice_id: str, account_id: str, amount: float, tenant_id: str = "", date: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """xero REST: create payment. Official: https://developer.xero.com/documentation/api/accounting/overview"""
    try:
        if not invoice_id:
            return _x_provision({}, "Payments", "PaymentID", 400, "invoice_id is required")
        if not account_id:
        ...
        if date:
            payment["Date"] = date
        resp = requests.put(f"{root}/Payments", headers=headers, json={"Payments": [payment]}, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _x_provision({}, "Payments", "PaymentID", resp.status_code, _x_err(resp))
        return _x_provision(resp.json() if resp.content else {}, "Payments", "PaymentID", resp.status_code, "ok")
    except Exception as e:
        return _x_provision({}, "Payments", "PaymentID", 500, str(e))
