...
# Xero Accounting API 2.0 — Official: https://developer.xero.com/documentation/api/accounting/contacts
...
def xero_update_contact(auth_info: dict, contact_id: str, tenant_id: str = "", name: str = "", email: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.post(f"{root}/Contacts/{contact_id}", headers=headers, json={"Contacts": [contact]}, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _x_provision({}, "Contacts", "ContactID", resp.status_code, _x_err(resp), fallback_id=contact_id)
        return _x_provision(resp.json() if resp.content else {}, "Contacts", "ContactID", resp.status_code, "ok", fallback_id=contact_id)
    except Exception as e:
        return _x_provision({}, "Contacts", "ContactID", 500, str(e), fallback_id=contact_id)
