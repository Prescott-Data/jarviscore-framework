...
def xero_update_payment(auth_info: dict, payment_id: str, tenant_id: str = "", status: str = "DELETED", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        # Payments are immutable except for status (e.g. Status=DELETED to reverse a payment).
        payment = {"PaymentID": str(payment_id), "Status": status}
        resp = requests.post(f"{root}/Payments/{payment_id}", headers=headers, json={"Payments": [payment]}, timeout=timeout, verify=verify_ssl)
