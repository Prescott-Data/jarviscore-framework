...
def xero_create_contact(auth_info: dict, name: str, tenant_id: str = "", email: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """xero REST: create contact. Official: https://developer.xero.com/documentation/api/accounting/overview"""
    try:
        if not name:
            return _x_provision({}, "Contacts", "ContactID", 400, "name is required")
        ...
        resp = requests.put(f"{root}/Contacts", headers=headers, json={"Contacts": [contact]}, timeout=timeout, verify=verify_ssl)
        if resp.status_code >= 400:
            return _x_provision({}, "Contacts", "ContactID", resp.status_code, _x_err(resp))
        return _x_provision(resp.json() if resp.content else {}, "Contacts", "ContactID", resp.status_code, "ok")
    except Exception as e:
        return _x_provision({}, "Contacts", "ContactID", 500, str(e))
