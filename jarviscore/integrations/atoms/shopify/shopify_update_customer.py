...
def shopify_update_customer(auth_info: dict, customer_id: str, payload: Dict[str, Any], shop: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.put(root + "/customers/{customer_id}.json".format(**locals()), headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
            data = resp.json() if resp.content else {}
        except Exception:
            data = {}
        if resp.status_code >= 400:
            return _sh_provision(data, resp.status_code, _sh_err(resp), "customer", fallback_id=customer_id)
        return _sh_provision(data, resp.status_code, "ok", "customer", fallback_id=customer_id)
    except Exception as e:
        return _sh_provision({}, 500, str(e), "customer")
