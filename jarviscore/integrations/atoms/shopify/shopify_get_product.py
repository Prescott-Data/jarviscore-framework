...
def shopify_get_product(auth_info: dict, product_id: str, shop: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shopify Admin REST: get product. Official: https://shopify.dev/docs/api/admin-rest/latest/resources/product#get-products-product-id"""
    try:
        if not product_id:
            return _sh_dataset([], 400, "product_id is required")
        ...
        resp = requests.get(root + "/products/{product_id}.json".format(**locals()), headers=headers, timeout=timeout, verify=verify_ssl)
        try:
        ...
        return _sh_dataset(_sh_rows(data, "products"), resp.status_code, "ok")
