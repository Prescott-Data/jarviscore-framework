...
def shopify_list_orders(auth_info: dict, shop: str = "", limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shopify Admin REST: list orders. Official: https://shopify.dev/docs/api/admin-rest/latest/resources/order#get-orders"""
    try:
        root, err = _sh_root(base_url, shop, auth_info)
        if err:
            return _sh_dataset([], 400, err)
        records, status, msg = _sh_paginate(root, "/orders.json", auth_info, "orders", limit, timeout, verify_ssl)
