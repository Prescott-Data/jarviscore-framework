...
async def shopify_search_records(query: str, shop: str='', limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        cap = _sh_cap(limit)
        resp = await nexus_call('GET', root + '/customers/search.json', headers=headers, params={'query': str(query), 'limit': min(cap, 250)})
