...
def shopify_get_order(auth_info: dict, order_id: str, shop: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Shopify Admin REST: get order. Official: https://shopify.dev/docs/api/admin-rest/latest/resources/order#get-orders-order-id"""
    try:
        if not order_id:
        ...
        resp = requests.get(root + "/orders/{order_id}.json".format(**locals()), headers=headers, timeout=timeout, verify=verify_ssl)
        try:
        ...
        return _sh_dataset(_sh_rows(data, "orders"), resp.status_code, "ok")
