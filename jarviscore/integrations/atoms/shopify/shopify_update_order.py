...
def shopify_update_order(auth_info: dict, order_id: str, payload: Dict[str, Any], shop: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.put(root + "/orders/{order_id}.json".format(**locals()), headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
