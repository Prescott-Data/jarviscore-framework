...
def shopify_create_customer(auth_info: dict, payload: Dict[str, Any], shop: str = "", timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        resp = requests.post(root + "/customers.json", headers=headers, json=body, timeout=timeout, verify=verify_ssl)
        try:
            data = resp.json() if resp.content else {}
        except Exception:
            data = {}
        if resp.status_code >= 400:
            return _sh_provision(data, resp.status_code, _sh_err(resp), "customer")
        return _sh_provision(data, resp.status_code, "ok", "customer")
