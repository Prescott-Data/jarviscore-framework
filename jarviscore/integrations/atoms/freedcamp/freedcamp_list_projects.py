...
def freedcamp_list_projects(auth_info: dict, limit: int = 200, offset: int = 0, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List projects. Official: https://freedcamp.com/help_/tutorials/wiki/wiki_public/view/DFaab#/projects"""
    try:
        ...
        resp = requests.get(f"{api}/projects", headers=headers, params=params, timeout=timeout, verify=verify_ssl)
        status = resp.status_code
        if status >= 400:
            return {"records": [], "data_count": 0, "status": status, "message": resp.text[:1000]}
        records = _fc_records(resp.json() if resp.text else {})
