...
async def freedcamp_list_projects(limit: int=200, offset: int=0, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp = await nexus_call('GET', f'{api}/projects', headers=headers, params=params)
        status = resp['status_code']
        if status >= 400:
            return {'records': [], 'data_count': 0, 'status': status, 'message': resp['body'][:1000]}
        records = _fc_records(resp['json'] if resp['body'] else {})
