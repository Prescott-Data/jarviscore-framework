...
def freedcamp_get_task(auth_info: dict, task_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get task by id. Official: https://freedcamp.com/help_/tutorials/wiki/wiki_public/view/DFaab#/tasks"""
    try:
        ...
        resp = requests.get(f"{api}/tasks/{task_id}", headers=headers, params=auth, timeout=timeout, verify=verify_ssl)
        ...
        return {"records": records[:1] if records else _fc_records(body), "data_count": len(records[:1]) if records else len(_fc_records(body)), "status": status, "message": "ok"}
