...
def activecampaign_update_deal(auth_info: dict, deal_id: str, payload: Dict[str, Any], account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        ids = _ac_v3_provision_ids(data, "deal") or [str(deal_id)]
        return _ac_v3_provision(records, status, "ok", ids)
    except Exception as e:
        return _ac_v3_provision([], 500, str(e))


# PUT /deals/{id} — https://developers.activecampaign.com/reference/update-a-deal-new
