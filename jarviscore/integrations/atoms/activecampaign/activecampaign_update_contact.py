...
def activecampaign_update_contact(auth_info: dict, contact_id: str, payload: Dict[str, Any], account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update contact via PUT /contacts/{id}. Official: https://developers.activecampaign.com/reference/update-a-contact-new"""
    try:
        if not contact_id:
            return _ac_v3_provision([], 400, "contact_id is required")
        ...
        resp = requests.put(
            f"{api_root}/contacts/{contact_id}",
            headers=headers,
            json=body,
            timeout=timeout,
            verify=verify_ssl,
        ...
        ids = _ac_v3_provision_ids(data, "contact") or [str(contact_id)]
        return _ac_v3_provision(records, status, "ok", ids)
    except Exception as e:
        return _ac_v3_provision([], 500, str(e))


# PUT /contacts/{id} — https://developers.activecampaign.com/reference/update-a-contact-new
