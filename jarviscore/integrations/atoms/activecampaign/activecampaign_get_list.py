...
def activecampaign_get_list(auth_info: dict, list_id: str, account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Retrieve list via GET /lists/{id}. Official: https://developers.activecampaign.com/reference/retrieve-a-list"""
    try:
        if not list_id:
            return _ac_v3_dataset([], 400, "list_id is required")
        ...
        if not isinstance(obj, dict):
            return _ac_v3_dataset([], status, "missing list object in response")
        return _ac_v3_dataset([obj], status, "ok")
    except Exception as e:
        return _ac_v3_dataset([], 500, str(e))


# GET /lists/{id} — https://developers.activecampaign.com/reference/retrieve-a-list
