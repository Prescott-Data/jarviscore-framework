...
def activecampaign_list_lists(auth_info: dict, account: Optional[str] = None, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        while len(records) < cap:
            ...
            batch = data.get("lists") if isinstance(data, dict) else None
            if not isinstance(batch, list):
                return _ac_v3_dataset(records, status, "missing lists array in response")
            for item in batch:
...
# GET /lists — https://developers.activecampaign.com/reference/retrieve-all-lists
