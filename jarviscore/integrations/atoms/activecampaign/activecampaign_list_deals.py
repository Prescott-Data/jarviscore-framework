...
async def activecampaign_list_deals(account: Optional[str]=None, limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List deals via GET /deals (limit/offset). Official: https://developers.activecampaign.com/reference/list-all-deals"""
    try:
        ...
        while len(records) < cap:
            ...
            resp = await nexus_call('GET', f'{api_root}/deals', headers=headers, params={'limit': page_size, 'offset': offset})
            ...
            batch = data.get('deals') if isinstance(data, dict) else None
            if not isinstance(batch, list):
                return _ac_v3_dataset(records, status, 'missing deals array in response')
            for item in batch:
