...
def activecampaign_create_list(auth_info: dict, payload: Dict[str, Any], account: Optional[str] = None, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        body = _ac_v3_wrap_resource("list", payload)
        resp = requests.post(
            f"{api_root}/lists",
            headers=headers,
            json=body,
        ...
        obj = data.get("list") if isinstance(data, dict) else None
        records: List[Dict[str, Any]] = [obj] if isinstance(obj, dict) else []
        return _ac_v3_provision(records, status, "ok", _ac_v3_provision_ids(data, "list"))
    except Exception as e:
        return _ac_v3_provision([], 500, str(e))


# POST /lists — https://developers.activecampaign.com/reference/create-new-list
