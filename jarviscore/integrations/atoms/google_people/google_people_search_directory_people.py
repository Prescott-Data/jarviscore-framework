...
def google_people_search_directory_people(auth_info: dict, query: str, read_mask: str = "names,emailAddresses,phoneNumbers,organizations", max_results: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        records, status, msg = _people_page(
            url, headers, {"query": query, "readMask": read_mask, "sources": ["DIRECTORY_SOURCE_TYPE_DOMAIN_PROFILE"]},
            "people", max_results, timeout, verify_ssl
        )
