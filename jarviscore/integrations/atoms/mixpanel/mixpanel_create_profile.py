...
def mixpanel_create_profile(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create/set user profile via Ingestion API profile-set. Official: https://developer.mixpanel.com/reference/profile-set"""
    try:
        ...
        distinct_id = body.get("distinct_id") or body.get("$distinct_id")
        if not distinct_id:
            return {"records": [], "data_count": 0, "status": 400, "message": "payload.distinct_id is required", "provision_ids": []}
        ...
        resp = requests.post(f"{root}/engage", params={"verbose": "1", "ip": "0"}, json=update, timeout=timeout, verify=verify_ssl)
