...
async def drift_create_message(conversation_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        resp = await _drift_post(f'{root}/conversations/{conversation_id}/messages', headers, body, timeout, verify_ssl)
        ...
        out = {'records': [obj] if isinstance(obj, dict) else [], 'data_count': 1 if isinstance(obj, dict) else 0, 'status': resp['status_code'], 'message': 'ok'}
        if prov:
...
async def _drift_post(url, headers, body, timeout, verify_ssl):
