...
async def africas_talking_create_message(payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        form = {'username': str(username), 'to': to_numbers, 'message': str(message), 'bulkSMSMode': '1'}
        ...
        if enqueue in (True, 1, '1', 'true', 'True'):
            form['enqueue'] = '1'
        resp = await nexus_call('POST', f'{api_root}{_AT_MESSAGING_PATH}', headers=headers, data=form)
        ...
        records: List[Dict[str, Any]] = [item for item in recipients or [] if isinstance(item, dict)]
