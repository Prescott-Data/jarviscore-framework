...
def prestashop_update_product(auth_info: dict, product_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Partially update a product (XML body). Official: https://devdocs.prestashop-project.org/9/webservice/getting-started/"""
    try:
        if not product_id:
            return _ps_provision({}, 400, "product_id is required", "products")
        if not isinstance(payload, dict) or not payload:
            return _ps_provision({}, 400, "payload is required", "products")
        root, err = _ps_root(base_url, auth_info)
        if err:
            return _ps_provision({}, 400, err, "products")
        xml_body = _ps_build_xml("products", payload, resource_id=product_id)
        resp, body, status, msg = _ps_request("patch", root + "/products/" + str(product_id), auth_info, xml_body=xml_body, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _ps_provision(body if isinstance(body, dict) else {}, status, msg, "products", fallback_id=product_id)
        return _ps_provision(body if isinstance(body, dict) else {}, status, "ok", "products", fallback_id=product_id)
    except Exception as e:
        return _ps_provision({}, 500, str(e), "products", fallback_id=product_id)
