...
def prestashop_create_product(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    ...
    try:
        ...
        xml_body = _ps_build_xml("products", payload)
        resp, body, status, msg = _ps_request("post", root + "/products", auth_info, xml_body=xml_body, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _ps_provision(body if isinstance(body, dict) else {}, status, msg, "products")
        return _ps_provision(body if isinstance(body, dict) else {}, status, "ok", "products")
    except Exception as e:
        return _ps_provision({}, 500, str(e), "products")
