...
def prestashop_update_customer(auth_info: dict, customer_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Partially update a customer (XML body). Official: https://devdocs.prestashop-project.org/9/webservice/getting-started/"""
    try:
        if not customer_id:
            return _ps_provision({}, 400, "customer_id is required", "customers")
        if not isinstance(payload, dict) or not payload:
            return _ps_provision({}, 400, "payload is required", "customers")
        root, err = _ps_root(base_url, auth_info)
        if err:
            return _ps_provision({}, 400, err, "customers")
        xml_body = _ps_build_xml("customers", payload, resource_id=customer_id)
        resp, body, status, msg = _ps_request("patch", root + "/customers/" + str(customer_id), auth_info, xml_body=xml_body, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _ps_provision(body if isinstance(body, dict) else {}, status, msg, "customers", fallback_id=customer_id)
        return _ps_provision(body if isinstance(body, dict) else {}, status, "ok", "customers", fallback_id=customer_id)
    except Exception as e:
        return _ps_provision({}, 500, str(e), "customers", fallback_id=customer_id)
