...
def prestashop_search_records(auth_info: dict, query: str, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Search products/customers/orders using filter LIKE. Official: https://devdocs.prestashop-project.org/9/webservice/getting-started/"""
    try:
        if not query:
            return _ps_dataset([], 400, "query is required")
        auth_info = auth_info or {}
        resource = (auth_info.get("resource") or auth_info.get("search_resource") or "products").strip().lower()
        if resource not in ("products", "customers", "orders"):
            resource = "products"
        cap = _ps_cap(limit)
        extra = {}
        if resource == "products":
            extra["filter[name]"] = f"[{query}]%"
        elif resource == "customers":
            extra["filter[email]"] = f"[{query}]%"
        else:
            extra["filter[reference]"] = f"[{query}]%"
        records, status, msg = _ps_list(resource, base_url, auth_info, cap, timeout, verify_ssl, extra_params=extra)
        if status >= 400:
            return _ps_dataset([], status, msg)
        filtered = [row for row in records if _ps_match(row, query)]
        out = filtered[:cap] if filtered else records[:cap]
        return _ps_dataset(out, status, msg)
