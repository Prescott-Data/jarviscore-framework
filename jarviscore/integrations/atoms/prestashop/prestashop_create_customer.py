...
def prestashop_create_customer(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create a customer (XML body). Official: https://devdocs.prestashop-project.org/9/webservice/getting-started/"""
    try:
        ...
        xml_body = _ps_build_xml("customers", payload)
        resp, body, status, msg = _ps_request("post", root + "/customers", auth_info, xml_body=xml_body, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _ps_provision(body if isinstance(body, dict) else {}, status, msg, "customers")
        return _ps_provision(body if isinstance(body, dict) else {}, status, "ok", "customers")
    except Exception as e:
        return _ps_provision({}, 500, str(e), "customers")
