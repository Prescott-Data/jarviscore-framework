...
def prestashop_get_customer(auth_info: dict, customer_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get a customer by ID. Official: https://devdocs.prestashop-project.org/9/webservice/getting-started/"""
    try:
        if not customer_id:
        ...
        resp, body, status, msg = _ps_request("get", root + "/customers/" + str(customer_id), auth_info, params={"display": "full"}, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
