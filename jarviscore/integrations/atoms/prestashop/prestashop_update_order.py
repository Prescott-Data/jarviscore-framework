...
async def prestashop_update_order(order_id: str, payload: Dict[str, Any], timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    ...
    try:
        ...
        (resp, body, status, msg) = await _ps_request('patch', root + '/orders/' + str(order_id), xml_body=xml_body, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
