...
def prestashop_update_order(auth_info: dict, order_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Partially update an order (XML body). Official: https://devdocs.prestashop-project.org/9/webservice/getting-started/"""
    try:
        if not order_id:
            return _ps_provision({}, 400, "order_id is required", "orders")
        ...
        xml_body = _ps_build_xml("orders", payload, resource_id=order_id)
        resp, body, status, msg = _ps_request("patch", root + "/orders/" + str(order_id), auth_info, xml_body=xml_body, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _ps_provision(body if isinstance(body, dict) else {}, status, msg, "orders", fallback_id=order_id)
        return _ps_provision(body if isinstance(body, dict) else {}, status, "ok", "orders", fallback_id=order_id)
    except Exception as e:
        return _ps_provision({}, 500, str(e), "orders", fallback_id=order_id)
