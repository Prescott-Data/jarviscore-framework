...
def prestashop_get_order(auth_info: dict, order_id: str, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Get an order by ID. Official: https://devdocs.prestashop-project.org/9/webservice/getting-started/"""
    try:
        if not order_id:
            return _ps_dataset([], 400, "order_id is required")
        root, err = _ps_root(base_url, auth_info)
        if err:
            return _ps_dataset([], 400, err)
        resp, body, status, msg = _ps_request("get", root + "/orders/" + str(order_id), auth_info, params={"display": "full"}, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _ps_dataset([], status, msg)
        return _ps_dataset(_ps_rows(body, "orders"), status, msg)
