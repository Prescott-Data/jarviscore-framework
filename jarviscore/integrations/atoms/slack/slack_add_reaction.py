def slack_add_reaction(auth_info: dict, channel: str, timestamp: str, emoji: str) -> dict:
    import requests
    ...
    def _delete(p, headers=None):
        ...
        _r.raise_for_status()
        return _r.json() if _r.content else {}
    resp = _post("/reactions.add", data={"channel": channel, "timestamp": timestamp, "name": emoji})
    if not resp.get("ok"):
