async def slack_get_messages(channel: str, limit: int=20) -> list:
    """Get messages via the slack API."""
    ...
    async def _delete(p, headers=None):
        ...
        if not _r['ok']:
            return {'success': False, 'error': _r['body']}
        return _r['json'] if _r['content'] else {}
    resp = await _get('/conversations.history', params={'channel': channel, 'limit': limit})
    if not resp.get('ok'):
