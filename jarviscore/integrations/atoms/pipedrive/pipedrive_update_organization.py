...
def pipedrive_update_organization(auth_info: dict, organization_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update organization. Official: https://developers.pipedrive.com/docs/api/v1/Organizations"""
    try:
        data, status, msg = _pi_write("patch", "/organizations", base_url, auth_info, payload, obj_id=organization_id, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _pi_provision(data, status, msg, fallback_id=organization_id)
        return _pi_provision(data, status, "ok", fallback_id=organization_id)
