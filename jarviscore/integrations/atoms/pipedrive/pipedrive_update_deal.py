...
def pipedrive_update_deal(auth_info: dict, deal_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update deal. Official: https://developers.pipedrive.com/docs/api/v1/Deals"""
    try:
        data, status, msg = _pi_write("patch", "/deals", base_url, auth_info, payload, obj_id=deal_id, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
