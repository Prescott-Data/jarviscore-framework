...
def pipedrive_create_person(auth_info: dict, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Create person. Official: https://developers.pipedrive.com/docs/api/v1/Persons"""
    try:
        data, status, msg = _pi_write("post", "/persons", base_url, auth_info, payload, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
