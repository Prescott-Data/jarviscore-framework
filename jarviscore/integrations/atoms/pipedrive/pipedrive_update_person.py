...
def pipedrive_update_person(auth_info: dict, person_id: str, payload: Dict[str, Any], timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """Update person. Official: https://developers.pipedrive.com/docs/api/v1/Persons"""
    try:
        data, status, msg = _pi_write("patch", "/persons", base_url, auth_info, payload, obj_id=person_id, timeout=timeout, verify_ssl=verify_ssl)
        if status >= 400:
            return _pi_provision(data, status, msg, fallback_id=person_id)
        return _pi_provision(data, status, "ok", fallback_id=person_id)
