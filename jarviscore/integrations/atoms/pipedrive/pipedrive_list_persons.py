...
async def pipedrive_list_persons(limit: int=25, timeout: int=30, verify_ssl: bool=True, base_url: str=None) -> dict:
    """List persons with cursor pagination. Official: https://developers.pipedrive.com/docs/api/v1/Persons"""
    try:
        extra = None or None
        if not isinstance(extra, dict):
            extra = {}
        records, status, msg = await _pi_list('/persons', base_url, limit, timeout, verify_ssl, extra_params=extra)
