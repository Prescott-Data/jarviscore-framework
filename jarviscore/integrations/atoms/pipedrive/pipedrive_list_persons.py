...
def pipedrive_list_persons(auth_info: dict, limit: int = 25, timeout: int = 30, verify_ssl: bool = True, base_url: str = None) -> dict:
    """List persons with cursor pagination. Official: https://developers.pipedrive.com/docs/api/v1/Persons"""
    try:
        auth_info = auth_info or {}
        extra = auth_info.get("filters") or auth_info.get("params")
        if not isinstance(extra, dict):
            extra = {}
        records, status, msg = _pi_list("/persons", base_url, auth_info, limit, timeout, verify_ssl, extra_params=extra)
